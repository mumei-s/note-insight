...
公開確認：main `e5deb8801ba48881b1764f973be121c2e8a19ff6`、Actions `35692419759` の141件テスト・build・Pages deploy成功。公開ページ13ファイル、raw GitHubの通知wrapper/network・Dashboard userscript、クエリなし更新パネルのHTTP200と内容完全一致を確認。接続エラーによる公開待ちは解消。実機の通知Storageエラー解消と本物のDM本文保存は確認待ちを維持する。

## 24. 2026-09-22 DMパネルがメッセージ入力を妨げる問題

追加画像1000015189.jpgでDM固定パネルが入力欄に重なることを確認。入力欄・送信ボタンの位置を避け、文字入力とキーボード表示中は非表示にする。本人の会話の履歴スクロールは入力中に止め、取得済み本文の保存と再開を維持する。詳細は [WORK_FIXES_20260922_DM_COMPOSER.md](WORK_FIXES_20260922_DM_COMPOSER.md)。本体2026.09.22.5／DM1.4.3。通知3.6.3とDashboard1.4.5は変更しない。

公開確認：機能main `db6a9af198f230d761b41ac0275679c543630896`、Actions `35693554319` のbuild・152件テスト・Pages deploy成功。公開manifest、本体JS、DM更新ページ、DM wrapper、実際の@require先のraw GitHub Reader `?v=143` のHTTP200・内容完全一致を確認。旧DMスクリプトが動作中の端末ではv1.4.3へ更新後、DMページを開き直す。各ブラウザの実機キーボードとDM本文の全件保存は未検証であり、自動テスト成功で代替しない。

## 25. 2026-09-22 運営向け分類案内・フィルター・DM自動差分

最新画像1000015191〜1000015194と「DMも初回全部→以後追加のみ」を受け、本体2026.09.22.6／通知3.6.4／DM1.4.4を実装。未知形式の案内は全参加者分を運営者だけへ集約し、既読／未読を保存する。stock_photoを画像の使用、指摘のembed_noteを話題へ本番補修。新着取得でアイコンを消さず、旧その他・引用を自動再判定する。フィルター登録の共有プロフィール欠落を修正し、重複する分析設定を除去。

DMは実際のdm-api.note.comの取得形式に対応。会話ごとの保存確認済み境界から自動で過去を取得し、初回完了後は新着だけ追加。停止／画面移動の途中区間も再開する。入力を妨げない小さい1行表示とし、続き読込ボタンは不要。詳細な保存条件・配備情報・178件の検証・実機未確認事項は [WORK_FIXES_20260922_AUTO_DM_FORMATS.md](WORK_FIXES_20260922_AUTO_DM_FORMATS.md) を参照。

公開確認：機能main `40320b6e90b660496833f2f6b14b173143f4136a`、Actions `35700371912` の178件テスト・build・Pages deploy成功。Pages17ファイル、実際の@require URLを含むraw GitHub15ファイルでHTTP200・内容完全一致を確認。Supabase4関数の配備後ソースも一致。利用者の端末には通知3.6.4／DM1.4.4への更新が必要。実機の全件取得と各ブラウザのキーボード確認は未完了として引き継ぐ。
...
公開実画面で通知・DMインストール画面の「戻る」が自分自身になる問題も発見し、空・自己参照URLを除外。回帰テストは221件へ追加。公開確認の履歴は上記の全体点検ドキュメントを参照。

## 2026-09-22 注意事項直上の点滅を最優先で修正

実機で「インストール / 更新」が点滅。旧v13表示補正と現行上段が相互にリンクを生成・削除していた。本体2026.09.22.10で旧処理を停止し、更新案内の無条件再生成も止める。詳細は [WORK_FIXES_20260922_TOP_BLINK.md](WORK_FIXES_20260922_TOP_BLINK.md)。点滅を先に公開後、そのまま通常データ下の本体更新案内と↑表記の誤解を解消し、全体動作点検を継続する。各ツールの版数は変えない。

## 2026-09-22 note公式🔔パネルOFFとDashboard連携失効

点滅の先行公開後、ユーザーの追加指示を継続。本体2026.09.22.11／本人通知3.6.6／Dashboard1.4.8。表記を「note公式🔔パネル ON/OFF」とし、既存のnoteタブへ状態を共有してOFFで追加パネルと読取を止める。全件フィルター時にも解除操作を保持。本体更新は通常データから独立。

公式Dashboardは折りたたみ内を展開し通信完了後に保存、日別未取得を同期完了にしない。本番では通知再連携が全用途のトークンを失効させる不具合も確認。通知トークン／ペアコードの用途を限定したサーバーv10を配備し、実ソース一致を確認。既存失効トークンは復活させず、正常な読取操作から再連携する。詳細と実機未確認範囲は [WORK_FIXES_20260922_PANEL_OFF_DASHBOARD.md](WORK_FIXES_20260922_PANEL_OFF_DASHBOARD.md)。本人ログイン後の最新データ取得は未確認のまま扱う。

公開確認：機能main `d4a88b61af5f54e93eca20a170b3ee1c704102a6`、Actions `35728808727` の241件回帰・build・Pages deploy成功。Pages13／raw GitHub7、計20ファイルでHTTP200・内容完全一致。サーバーv10も実ソース一致。本人通知3.6.6／Dashboard1.4.8へ端末更新後、noteを開き直してDashboardを再連携・読取する。実アカウントの最新保存を確認済みと報告しない。


## 2026-09-22 追加：料金表記と分析・Dashboardの実操作

最新指示：「各ブラウザで料金がかかるやつのインストールの動線のところのモンキーのとこにかけばいーだけ」。共通注意枠は削除し、有料Tampermonkeyのリンクだけに別料金を表示する。分析の更新案内は分析パネル自身の点灯と更新リンクへまとめ、誤った統合案内を廃止。公式の未連携パネルには「連携して読み込む」を設け、設定の先頭に読込操作を移動。長文案内とパネルを小型化。本体2026.09.22.12／Dashboard1.4.9。実操作回帰・原因・実機未確認範囲は [WORK_FIXES_20260922_DASHBOARD_ENTRY.md](WORK_FIXES_20260922_DASHBOARD_ENTRY.md)。本人ログイン後の最新保存は確認済みと報告しない。
...
設定で@ss_yr確認後もnoteでは未連携。新しいペアコード未使用／有効トークン0を本番で確認。1.5.3で移動前の拡張機能共有保存と読返し確認、到着時URL保持、クエリ消失後の共有情報受取を実装。通常記事・編集ページでは保留連携を消費しない。詳細は上記の9/26修正文書。端末更新後の最新実データ保存は未確認。
