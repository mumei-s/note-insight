...
Updated: 2026-09-02 11:44 JST

## 2026-09-02 11:44 ACCOUNT-ID ISOLATION / PRIVATE NOTIFICATION 2.2 — LATEST CHECKPOINT

This is the newest checkpoint. Future work must determine source-of-truth by actual timestamp first, then fetch the current GitHub `main`. Do not overwrite newer 巡回BOOST or other userscript work from another chat.
...
- `src/access-portal-v4.tsx` is the active participant access/switch UI. `src/App.tsx` routes `#access/insight` to `AccessPortalV4`.
- Opening the switch page first hydrates the currently valid `X-Insight-Token` session and saves that account before showing the list. This specifically prevents an already-active `ss_yr` session from disappearing while another account application is in progress.
- Primary switch UI is now the saved-account list:
  - active saved participant → `切替` and direct dashboard open;
  - pending/approved application → `参加の続き`;
  - `＋ 新しく参加する` is separate;
  - individual code input is hidden under `この端末に既存参加アカウントを追加` and is only needed once when importing an already-verified participant from another device/browser.
...
- Database state checked during this fix: `ss_yr` is `active` and profile-verified. `fuku444` is OWNER-approved but still `verified_at IS NULL`; therefore it must complete the one-time profile verification before it can become an active tap-to-switch account. Until then it correctly appears as `参加の続き`.
...
- `public/note-insight-notification-sync.user.js` is now **v2.2.0**.
- v2.2 reads the actual note login identity from `/api/v2/current_user`, reusing the same proven real-account check used by 巡回BOOST.
- Private notification token, last-notification signature, and magazine-mute settings are all stored under the actual note ID. Switching note accounts loads only that note ID's token; an unpaired note account never reuses another account's token.
- Pairing URLs include `mumei_account=<expected note id>`. The userscript refuses pairing if the expected INSIGHT note ID and note's actual logged-in ID differ.
- Supabase Edge Function `insight-notification-import-token` **v4 ACTIVE** supports both participant `X-Insight-Token` and explicit OWNER `X-Owner-Token`, issues pair codes/tokens scoped to the authenticated member ID, and returns the expected note ID.
- Supabase Edge Function `insight-notification-ingest-v2` **v1 ACTIVE** provides a second server-side guard: every private ingest must send `noteId`, and it is rejected with `NOTIFICATION_ACCOUNT_MISMATCH` unless that note ID exactly matches the ingest token's notification profile. A wrong-account client therefore cannot store notifications in another account.
- Supabase Edge Function `insight-self-account` **v2 ACTIVE** additionally revokes that participant's private-notification tokens and unused pair codes on `退会`. Normal account switching and ordinary logout do not mix or transfer notification identities.
- `public/notification-import.html` is participant-aware. Normal participant setup authenticates with the current INSIGHT member token; `role=owner` keeps OWNER notification setup separate. Magazine notification mute lists are also stored per note ID.
- `public/notification-setup.html` forwards participant/OWNER role explicitly. The OWNER tool-row link in `index.html` uses `role=owner`.
- Pages CI now runs `node --check public/note-insight-notification-sync.user.js` before the production build, so an invalid notification userscript cannot be deployed by the Pages workflow.
...
- Each note account needs a one-time update/re-pair to v2.2. Old v1 global notification tokens are not silently reused for another account.

### Release / coexistence
- Service Worker generation is `mumei-note-insight-v22` for this account-isolation release.
- 巡回BOOST v4.6 work saved by the newer parallel chat is part of current `main` and must be preserved. Do not roll it back while working on INSIGHT access/notification code.

## 2026-09-02 10:28 CODE-ONLY LOGIN / NON-BLOCKING INITIAL LOAD — PREVIOUS CHECKPOINT

- Participant login/import on a new device requires only the individual `INSIGHT-XXXXXXXX` code. A note ID is not entered again for that import.
...
- Member API calls have a 20-second client timeout. A timeout never leaves the user on an infinite spinner; it shows a retry button or leaves the already-rendered INSIGHT usable while sync can be retried manually.
...
- v2.1 removed dependence on finding or programmatically clicking a labelled bell button.
...
- The list of creators whose magazine-add notifications should be organized is transferred to the note-side userscript without sending note passwords or login cookies to INSIGHT.
...
## Participant flow
1. Participant opens the fixed root URL and chooses participation/account switch.
2. For a new participant, submit the note ID/profile URL once.
...
8. Verification activates the account, issues a long-lived hashed session token, saves the account on the device and adds the creator to the public participant list.
9. Participant restores the original bio.
10. Normal switching among saved active accounts is tap-only. Adding an already-verified account to a new device/browser requires its individual code once.
...
- `insight-code-login`: code-only import/login for an already-verified participant and new session issuance.
- `insight-self-account`: participant self-service long-session refresh, explicit logout and explicit leave; v2 also revokes private notification pairing on leave.
...
- `insight-notification-import-token`: participant/OWNER private-notification pairing and token issuance, currently v4 ACTIVE.
- `insight-notification-ingest-v2`: account-verified private notification ingestion, currently v1 ACTIVE.
...
Private note-account bell notifications remain an optional userscript extension. They must be account-isolated and must never block the core product.
