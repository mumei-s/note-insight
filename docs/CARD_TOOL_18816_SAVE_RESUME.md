...
- Later the user reported successful saving through タンポポはる. This is user-confirmed progress; the workspace still has not independently inspected that private draft.
...
- The standalone 18.8.16 file is published at main `47323f1dcd5a7a0d1906048f7ddc7a66ef4d93d7`. Both canonical Raw and `?v=18.8.16` returned HTTP 200 and exactly matched local bytes (SHA-256 `6e3c1b42f27eb5d516c264d4cfa33b3e55d3818490ebe2ab093622eb072ec7af`). The first Pages run passed 109/110 tests: the expanded 313-card test still expected 312 total embeds, overlooking the two new cards. Correct that assertion to 313 tool-owned plus the one preserved original card; do not weaken the preservation assertions. No distributed userscript code changes are needed for this test correction.
