...
function errorText(code: string) {
  const messages: Record<string, string> = {
    ...
    ALREADY_ACTIVE: "このnote IDは参加中です。ログイン画面から個別コードでログインしてください。",
    ...
    INSIGHT_SESSION_INVALID: "このINSIGHTログインは失効しています。個別コードで再ログインしてください。",
...
export function AccessPortalV3() {
  ...
  async function openSaved(account: StoredInsightAccount) {
    ...
    try {
      if (account.memberToken) {
        ...
        setApplication(payload.application); setStage("active");
        setMessage(`@${payload.application.noteId} に切り替えました。ほかの保存済みアカウントはログアウトしていません。`);
  ...
  async function checkExisting() {
    ...
    if (member) {
      try {
        const payload = await callAccess("session", {}, { "X-Insight-Token": member });
        rememberMemberSession(payload.application, member, currentStoredInsightAccount()?.passcode);
        setApplication(payload.application); setStage("active"); refreshAccounts(); return;
      } catch (reason) {
        const code = reason instanceof Error ? reason.message : "ACCESS_ERROR";
    ...
    if (applicant) {
      ...
      } catch { /* saved applicant state remains */ }
    }
  ...
  async function verifyProfile() {
    ...
    try {
      ...
      setApplication(payload.application); setStage("active"); refreshAccounts();
  ...
  async function login(event: FormEvent) {
    ...
    try {
      ...
      setApplication(payload.application); setPasscode(normalized); setStage("active"); refreshAccounts();
      setMessage(`@${payload.application.noteId} へログインしました。次回からこの端末では保存済みアカウントから直接切り替えられます。`);
  ...
  return <div className="access2"><main className="access2-main">
    ...
    <header className="access2-head"><small>MEMBER ACCESS</small><h1>INSIGHT 参加・ログイン</h1><p>参加申請、本人確認、個別コードでのログイン、保存済みアカウント切替をここで行います。</p></header>
    ...
    {stage === "login" ? <section className="access2-card"><h2>ログイン / アカウント切替</h2><p><strong style={{ color: "#fff" }}>ログインに必要なのは個別コードだけです。</strong> 保存済みアカウントは下からワンタップで切り替えられます。</p><SavedAccounts accounts={accounts} onUse={(account) => void openSaved(account)} /><form onSubmit={login}><input className="access2-input" value={passcode} onChange={(event) => setPasscode(event.target.value.toUpperCase())} placeholder="INSIGHT-XXXXXXXX" autoComplete="current-password" inputMode="text" required /><div className="access2-actions"><button className="access2-btn" disabled={saving}>{saving ? "確認中…" : "コードでINSIGHTへログイン"}</button><button type="button" className="access2-btn secondary" onClick={() => setStage("apply")}>新しく参加申請する</button></div></form></section> : null}
