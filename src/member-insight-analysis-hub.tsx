...
async function loadNotificationSummary(period=0){
  ...
  try{
    const response=await fetch(NOTIFICATION_ANALYSIS,{method:"POST",headers:{"Content-Type":"application/json","X-Insight-Token":token},body:JSON.stringify({period}),cache:"no-store",signal:controller.signal});
    const payload=await response.json().catch(()=>({}));
    if(!response.ok||payload?.ok===false)throw new Error(payload?.error||"本人通知分析データを取得できませんでした");
    if(localStorage.getItem(INSIGHT_TOKEN_KEY)!==token)throw new Error("アカウントが切り替わりました");return payload;
  }catch(error){
...
function NotificationDeepAnalysis(){
  ...
  async function load(){const seq=++requestSeq.current;setLoading(true);setError("");try{const next=await loadNotificationSummary(period);if(seq===requestSeq.current)setData(next)}catch(e){if(seq===requestSeq.current)setError(e instanceof Error?e.message:"本人通知分析に失敗しました")}finally{if(seq===requestSeq.current)setLoading(false)}}
  useEffect(()=>{void load();return()=>{requestSeq.current++}},[period]);
  ...
  if(!data?.sample)return <section className="miah-notification"><b>🔔 本人通知分析</b><span>選択期間の本人通知はありません。</span><label>対象期間 <select value={period} onChange={e=>setPeriod(Number(e.target.value))}><option value={0}>全保存履歴</option><option value={7}>直近7日</option><option value={30}>直近30日</option><option value={90}>直近90日</option></select></label><button onClick={()=>void load()}>↻ 再集計</button></section>;
...
export function MemberInsightAnalysisHub({revision=0,onBack,noteId="",dashboardInstalled="",notificationInstalled="",dashboardLatest="",notificationLatest=""}:{revision?:number;onBack?:()=>void;noteId?:string;dashboardInstalled?:string;notificationInstalled?:string;dashboardLatest?:string;notificationLatest?:string}){
  ...
  return <section className="miah">{!allReady?<div className="miah-paths"><article><a className="miah-path-main" href={setupHref}><strong>📊 Dashboard分析を準備 / 更新</strong><small>不足・更新がある時だけ表示</small><span>Dashboard同期＋本人通知をそろえると公式値とのクロス分析を使えます。</span></a><span className="miah-ready">{notificationUsable?`✓ 本人通知 v${notificationInstalled} 利用可能`:"ツール状態を確認してください"}</span></article></div>:null}<p className="miah-rule"><b>数値の正本：</b>PV・スキ・コメント・売上・流入などDashboardで取得できる値は公式Dashboardを最優先。公開データと差がある場合は公式値を採用します。</p>
    <section className="miah-analysis-layer base"><header className="miah-layer-head"><div><small>DASHBOARD ANALYSIS V3</small><h2>📊 公式Dashboard＋INSIGHT Pro分析</h2><p>公式Dashboardの生値を正本にし、データ品質を判定してから本人内ベンチマーク・流入・収益・通知とのクロス分析まで行います。</p></div><span>{allReady?"2ツール準備済み":"Dashboardクロス分析は2ツール"}</span></header><div className="miah-layer-tags"><span>公式値</span><span>データ品質</span><span>成長推移</span><span>本人内ベンチマーク</span><span>流入</span><span>収益</span><span>曜日</span><span>通知×PV</span><span>記事指数</span></div>{allReady?<MemberInsightAnalyticsProV3 key={cleanNoteId} revision={revision} onBack={onBack}/>:<div className="miah-notification-locked"><b>公式Dashboardクロス分析は2つのツールを準備してください。</b><span>本人通知だけ導入済みなら、下の人物・交流分析はそのまま使えます。</span><a href={setupHref}>ブラウザ別パネルを開く</a></div>}</section>
    <details className="miah-analysis-layer notice miah-fold" open><summary><b>🔔 本人通知の人物・交流分析</b><span>人物別反応・14日推移・時間帯・コメント・メンシプ・購入/支援</span></summary><div className="miah-fold-body"><NotificationDeepAnalysis key={cleanNoteId+revision}/></div></details>
  </section>;
}