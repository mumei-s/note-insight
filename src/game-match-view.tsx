...
export function MatchBattle({onFinish}:{onFinish?:(result:BattleResult,score:number)=>void}={}){
 ...
 useEffect(()=>{if((state!=="clear"&&state!=="over")||sent.current)return;sent.current=true;onFinish?.(state==="clear"?"win":"lose",score)},[state,score,onFinish]);
 function start(){sent.current=false;setBoard(settle(fresh()).board);setSelected(null);setScore(0);setCombo(0);setTime(SECONDS);setState("live");setFlash("")}
 ...
 return <div className={`rune-game state-${state}`}><div className="rune-hud"><div><small>TIME</small><b>{time.toFixed(1)}</b></div><div className="rune-score"><small>RUNE ENERGY</small><strong>{score.toLocaleString()} / {TARGET.toLocaleString()}</strong><i><span style={{width:`${progress}%`}}/></i></div><div><small>CHAIN</small><b>×{combo}</b></div></div>{state==="ready"?<div className="rune-intro"><span className="rune-orb">✦</span><h3>RUNE BREAK</h3><p>隣り合うルーンを入れ替え、3個以上そろえてエネルギーを集める。45秒以内に目標へ到達。</p><button onClick={start}>START PUZZLE</button></div>:null}{state==="live"?<><div className="rune-board">{board.map((x,i)=><button key={x.id} className={selected===i?"selected":""} data-rune={runes.indexOf(x.rune)} onClick={()=>choose(i)}><span>{x.rune}</span></button>)}</div>{flash?<div className="rune-flash">{flash}</div>:null}<p className="rune-help">タップで1つ選択 → 隣のルーンをタップして交換</p></>:null}{state==="clear"?<div className="rune-result clear"><small>MISSION COMPLETE</small><h3>RUNE CORE CLEARED</h3><strong>{score.toLocaleString()} pts</strong><div className="reward"><span>✦</span><div><small>REWARD</small><b>ARCANE CHEST ×1</b></div></div><p>目標エネルギー到達。クリア報酬を獲得。</p><button onClick={start}>PLAY AGAIN</button></div>:null}{state==="over"?<div className="rune-result"><small>TIME UP</small><h3>CORE NOT FILLED</h3><strong>{score.toLocaleString()} pts</strong><p>あと {Math.max(0,TARGET-score).toLocaleString()} pt。連鎖でボーナスが増える。</p><button onClick={start}>RETRY</button></div>:null}</div>;
}
