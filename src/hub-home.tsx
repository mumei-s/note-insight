...
export function HubHome() {
  ...
  const primaryLabel = memberReady ? "自分のINSIGHTを開く →" : "参加申請・ログイン →";

  return <div className="hub-page"><header className="hub-header"><div className="hub-wrap"><a href="#"><span>無名S note</span>INSIGHT</a></div></header><main>
    ...
    <section className="hub-wrap" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(230px,1fr))", gap: 10, padding: "8px 0 24px" }}>
      {[["01","購入・参加申請","販売用URLは固定。購入者が自分のnote IDで参加申請します。"],["02","OWNER承認","申請はOWNER専用管理ページに届き、購入確認後に承認します。"],["03","noteで本人確認","発行コードは申請者側に表示され、自己紹介欄へ一時掲載して本人確認します。"],["04","利用開始","確認後は自己紹介を元に戻せます。本人アイコンがTOP参加者一覧へ追加されます。"]].map(([no,title,copy]) => <article key={no} style={{ border: "1px solid #273446", borderRadius: 16, background: "#0d141d", padding: 16 }}><small style={{ color: "#b6ff38", fontWeight: 950 }}>{no}</small><strong style={{ display: "block", margin: "5px 0", fontSize: 16 }}>{title}</strong><p style={{ color: "#8d9caf", fontSize: 12, lineHeight: 1.7, margin: 0 }}>{copy}</p></article>)}
    </section>
