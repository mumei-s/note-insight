...
function mergePair(a:Row,b:Row){const p=richness(b)>richness(a)?b:a,o=p===b?a:b,pi=safeActorImage(p.actor_image_url)||safeActorImage(o.actor_image_url)||null;return{...o,...p,actor_url:p.actor_url||o.actor_url||null,actor_image_url:pi,target_url:p.target_url||o.target_url||null,target_title:p.target_title||o.target_title||null,source_url:p.source_url||o.source_url||null,occurred_at:p.occurred_at||o.occurred_at||null,captured_at:p.captured_at||o.captured_at||null}}
...
async function enrich(rows:Row[]){const ids=[...new Set(rows.map(r=>noteId(r.actor_url)).filter(Boolean))];if(!ids.length)return rows.map(r=>({...r,actor_image_url:safeActorImage(r.actor_image_url)||null}));try{const res=await fetch(ICON,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({noteIds:ids}),cache:"no-store"}),p=await res.json(),m=new Map((p.items||[]).map((x:any)=>[String(x.noteId||"").toLowerCase(),x.image]));return rows.map(r=>{const fresh=String(m.get(noteId(r.actor_url))||"");return{...r,actor_image_url:fresh||safeActorImage(r.actor_image_url)||null}})}catch{return rows.map(r=>({...r,actor_image_url:safeActorImage(r.actor_image_url)||null}))}}
