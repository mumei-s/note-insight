...
function rawFallback(article:HTMLElement){
  const raw=clean(article.querySelector('.minf-main small')?.textContent||'');if(!raw)return'';
  const markers=['さんがあなたの記事にコメントしました','さんがあなたのコメントに返信しました','さんがあなたのコメントにスキしました','さんがあなたのコメントに「スキ」しました','掲示板に返信しました'];
  for(const marker of markers){const i=raw.indexOf(marker);if(i>=0){const tail=raw.slice(i+marker.length).trim();if(tail)return tail}}
  return''
}
function bestComment(article:HTMLElement,rows:any[]){
  ...
  for(const r of rows){let s=0;const rid=profileId(String(r.actor_url||'')),rk=articleKey(String(r.article_url||'')),body=clean(r.body);if(actor&&rid===actor)s+=45;if(akey&&rk===akey)s+=35;if(body&&raw.includes(body.slice(0,Math.min(18,body.length))))s+=80;const rat=Date.parse(String(r.occurred_at||''))||0,gap=at&&rat?Math.abs(at-rat):Infinity;if(gap<=15*60000)s+=30;else if(gap<=2*3600000)s+=15;if(s>score){score=s;best=r}}
...
async function fillBody(article:HTMLElement){
  ...
  if(box.dataset.loaded==='1'||box.dataset.loading==='1')return;box.dataset.loading='1';box.innerHTML='<b>コメント内容</b><p>本文を照合中…</p>';
  try{const day=jstDay(article.querySelector('time')?.textContent||''),rows=day?await loadDay(day):[],hit=bestComment(article,rows),fallback=rawFallback(article),body=clean(hit?.body)||fallback;if(body){box.innerHTML='';const b=document.createElement('b');b.textContent='コメント内容';const p=document.createElement('p');p.textContent=body;const small=document.createElement('small');small.textContent=hit?'コメント履歴から本文を照合':'通知文から本文部分を抽出';box.append(b,p,small)}else{box.innerHTML='<b>コメント内容</b><p>保存通知だけでは本文を特定できませんでした。対象記事を開くと全文を確認できます。</p><small>通知概要は上に常時表示しています。</small>'}box.dataset.loaded='1'}catch{const fallback=rawFallback(article);box.innerHTML=`<b>コメント内容</b><p>${fallback||'本文を取得できませんでした。対象記事を開いて確認してください。'}</p><small>${fallback?'通知文から本文部分を抽出':'コメント履歴との照合に失敗'}</small>`;box.dataset.loaded='1'}finally{delete box.dataset.loading}
}
function bindComments(){
  document.querySelectorAll<HTMLElement>('#minf-notifications .mumei-comment-toggle').forEach(toggle=>{if(toggle.dataset.mumeiBodyBound==='1')return;toggle.dataset.mumeiBodyBound='1';toggle.setAttribute('aria-label',`${clean(toggle.textContent)}のコメント本文を開閉`);toggle.addEventListener('click',()=>window.setTimeout(()=>{const article=toggle.closest('article') as HTMLElement|null;if(article?.classList.contains('mumei-comment-open'))void fillBody(article)},30))})
}
