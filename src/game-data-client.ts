...
export async function loadCreatorGame(){
 ...
 const ownCreator=player&&player.status==="approved"?{
  id:player.id,note_id:player.note_id,display_name:player.display_name,status:player.status,
  images:player.cards.filter(x=>x.url).sort((a,b)=>a.position-b.position).map(x=>({position:x.position,url:x.url}))
 }:null;
 const data:CreatorGameData={opponents:(payload.opponents??[]).slice(0,3),creators:ownCreator?[ownCreator]:[]};
