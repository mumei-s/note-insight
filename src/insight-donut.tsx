...
export function InsightDonut({items,label,unit="件"}:{items:{label:string;value:number}[];label:string;unit?:string}){
 ...
 return <figure className="insight-donut">
  <svg viewBox="0 0 200 208" role="img" aria-label={`${label} 合計${total.toLocaleString()}${unit}`}>
   ...
   {slices.map((x,i)=>x.value>0?<g key={x.label} className={selected===i?"donut-slice selected":"donut-slice"} style={{opacity:selected==null||selected===i?1:.45}}><circle cx="100" cy="96" r="65" fill="none" stroke={`url(#${uid}-face-${i%colors.length})`} strokeWidth="30" strokeDasharray={`${x.share*perimeter} ${perimeter-x.share*perimeter}`} strokeDashoffset={-x.start*perimeter} transform="rotate(-90 100 96)" onClick={()=>setSelected(selected===i?null:i)}><title>{`${x.label}：${x.value.toLocaleString()}${unit}（${(x.share*100).toFixed(1)}%）`}</title></circle></g>:null)}
   ...
   <text x="100" y="114" textAnchor="middle" fill="#a3cddd" fontSize="12">{active?`${(active.share*100).toFixed(1)}%`:`合計・${unit}`}</text>
  </svg>
  <figcaption><strong>{label}</strong><small>項目をタップして内訳を確認</small>{slices.map((x,i)=><button type="button" className="insight-donut-legend" key={x.label} aria-pressed={selected===i} onClick={()=>setSelected(selected===i?null:i)}><i style={{background:x.color}}/><span>{x.label}</span><b>{x.value.toLocaleString()}{unit}<em>{(x.share*100).toFixed(1)}%</em></b></button>)}</figcaption>
 </figure>
}
