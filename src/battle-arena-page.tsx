...
import { CreatorQuestBattle } from "./game-quest";
import { StarCircuitBattle } from "./game-race";
...
const games = [
  ...
  ["quest", "CREATOR QUEST", "ACTION RPG", "ATTACK・GUARD・SKILLでSPを操り、3WAVEの最終ボスへ挑む"],
  ["race", "STAR CIRCUIT", "ARCADE RACE", "3LANEを駆け、障害を回避。ENERGYとBOOSTで1000mを先取"],
] as const;

type Foe = { id: string; type: "official" | "creator"; name: string; rarity: string; image_url: string | null; cards: GameCard[] };
const modeName: Record<GameMode, string> = { choice: "COMMAND", tap: "TAP RUSH", puzzle: "ARCANE PUZZLE", shoot: "STAR SHOOTER", quest: "CREATOR QUEST", race: "STAR CIRCUIT" };
...
function QuestPreview({ playerArt, enemyArt }: { playerArt?: string | null; enemyArt?: string | null }) {
  return <div className="premium-screen quest-premium"><div className="premium-topline"><span>WAVE 3 / 3</span><b>CREATOR QUEST</b><span>SP 100%</span></div><div className="quest-preview-stage"><div className="premium-card quest-hero"><Art src={playerArt} label="HERO" /><span className="card-glass" /></div><div className="quest-preview-slash"><small>LIMIT ARTS</small><strong>CREATOR<br />DRIVE</strong><em>12,480</em></div><div className="premium-card quest-boss"><Art src={enemyArt} label="BOSS" /><span className="card-glass" /></div></div><div className="quest-preview-dock"><span>⚔ ATTACK</span><span>⬡ GUARD</span><strong>✦ SKILL</strong></div></div>;
}
function RacePreview({ playerArt, enemyArt }: { playerArt?: string | null; enemyArt?: string | null }) {
  return <div className="premium-screen race-premium"><div className="premium-topline"><span>POS 1ST</span><b>STAR CIRCUIT</b><span>428 km/h</span></div><div className="race-preview-cast"><div><Art src={playerArt} label="YOU" /></div><strong>BOOST!</strong><div><Art src={enemyArt} label="RIVAL" /></div></div><div className="race-preview-track"><i /><i /><span className="preview-machine rival" /><span className="preview-machine player" /><b className="preview-energy">✦</b><em>FINISH 182m</em></div><div className="race-preview-footer"><span>ENERGY 82%</span><strong>✦ STAR BOOST</strong></div></div>;
}

function ModePreview({ mode, playerArt, enemyArt }: { mode: GameMode; playerArt?: string | null; enemyArt?: string | null }) {
  if (mode === "choice") return <CommandPreview playerArt={playerArt} enemyArt={enemyArt} />;
  if (mode === "tap") return <TapPreview art={enemyArt} />;
  if (mode === "puzzle") return <PuzzlePreview playerArt={playerArt} enemyArt={enemyArt} />;
  if (mode === "shoot") return <ShootPreview art={enemyArt} />;
  if (mode === "quest") return <QuestPreview playerArt={playerArt} enemyArt={enemyArt} />;
  return <RacePreview playerArt={playerArt} enemyArt={enemyArt} />;
}
...
function GameModeCard({ mode, title, tag, desc, playerArt, enemyArt, onPlay, ranked }: { mode: GameMode; title: string; tag: string; desc: string; playerArt?: string | null; enemyArt?: string | null; onPlay: () => void; ranked: boolean }) {
  return <article className={`game-concept premium-concept concept-${mode}`}><header className="concept-caption"><small>{tag} · PLAYABLE</small><strong>{title}</strong><span>{desc}</span></header><ModePreview mode={mode} playerArt={playerArt} enemyArt={enemyArt} /><button className="concept-enter" onClick={onPlay}>{ranked ? "RANKED MATCH  →" : "TRIAL PLAY  →"}</button></article>;
}
...
function AccessGate({ error, loading, onRetry }: { error: string; loading: boolean; onRetry: () => void }) {
  ...
  const message = loading ? "プロフィール画像ではなく、名鑑へ登録した本人カードだけを確認しています。" : card ? "本人の名鑑でカードを1〜3枚登録し、ゲーム対戦をONにすると戦績付き対戦が開きます。" : "下の6ゲームはTRIALで今すぐ操作できます。TRIALは参加者データを取得せず、戦績にも保存しません。";
...
export function BattleArenaPage() {
  ...
  return <div className="creator-world"><link rel="stylesheet" href={`${import.meta.env.BASE_URL}creator-world.css`} /><link rel="stylesheet" href={`${import.meta.env.BASE_URL}creator-world-premium.css`} /><link rel="stylesheet" href={`${import.meta.env.BASE_URL}creator-world-concept-v2.css`} /><link rel="stylesheet" href={`${import.meta.env.BASE_URL}creator-world-game-v4.css?v=4`} /><link rel="stylesheet" href={`${import.meta.env.BASE_URL}creator-world-game-v5.css?v=5`} /><link rel="stylesheet" href={`${import.meta.env.BASE_URL}creator-world-game-v9-six.css?v=9`} /><main>
    ...
    {!mode ? <section className="mode-select g5-mode-select"><div className="world-heading"><div><small>CHOOSE GAME MODE</small><h2>6つのゲーム</h2><p>{canPlay ? "選択カードでRANKED MATCH。勝敗・スコア・カード履歴を保存します。" : "参加前でも全ゲームをTRIAL操作できます。参加者情報・戦績保存は使いません。"}</p></div></div><div className="concept-grid premium-grid">{games.map((game) => <GameModeCard key={game[0]} mode={game[0]} title={game[1]} tag={game[2]} desc={game[3]} playerArt={canPlay ? playerCard?.url : null} enemyArt={canPlay ? enemyCard?.url : null} onPlay={() => start(game[0])} ranked={canPlay} />)}</div></section> : null}
    {mode && session ? <section className="active-game g4-active g5-active" ref={gameFrame}><div className="g5-game-toolbar"><button className="mode-back" onClick={() => setMode(null)}>← 6ゲームへ</button><div><small>{demo ? "TRIAL · NO RECORD" : "RANKED MATCH"}</small><strong>{modeName[mode]}</strong></div><button onClick={toggleFullscreen}>{fullscreen ? "全画面終了" : "全画面"}</button></div><div className="g4-match-label"><span>{session.playerName}</span><b>VS</b><span>{session.enemyName}</span></div><div className="game-console g4-console" key={`${demo ? "demo" : "ranked"}-${mode}-${foe?.type}-${foe?.id}-${playerCard?.position}-${enemyCard?.position}`}>{mode === "choice" ? <CommandBattle {...session} onComplete={(result, score) => demo ? Promise.resolve() : complete("choice", result, score)} /> : mode === "tap" ? <TapRushBattle {...session} onComplete={(result, score) => demo ? Promise.resolve() : complete("tap", result, score)} /> : mode === "puzzle" ? <MatchBattle {...session} onComplete={(result, score) => demo ? Promise.resolve() : complete("puzzle", result, score)} /> : mode === "shoot" ? <TargetRushBattle {...session} onComplete={(result, score) => demo ? Promise.resolve() : complete("shoot", result, score)} /> : mode === "quest" ? <CreatorQuestBattle {...session} onComplete={(result, score) => demo ? Promise.resolve() : complete("quest", result, score)} /> : <StarCircuitBattle {...session} onComplete={(result, score) => demo ? Promise.resolve() : complete("race", result, score)} />}</div></section> : null}
