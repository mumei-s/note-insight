...
type NamedLoss={name:string;url:string|null;image:string|null;personKey:string};
type Loss={at:string;previousTotal:number;currentTotal:number;officialNetLoss:number;namedRemoved:NamedLoss[];namedRemovedCount:number;unidentifiedNetLoss:number;complete:boolean;receivedCount:number};
type FollowData={officialTotal:number;namedScopeCount:number;complete:boolean;scannedAt:string|null;losses:Loss[]};
...
function FollowLossSummary(){
  ...
  if(!data)return null;
  return <section className="iv8-loss"><div className="iv8-loss-head"><div><b>フォロワー減少・直近7日</b><span>公式総数 {fmt(data.officialTotal)}人 / 名前追跡 {fmt(data.namedScopeCount)}人</span></div><button onClick={()=>void load()}>再読込</button></div><small>{data.complete?"全件の名前を照合済み":"noteの人物一覧は1,000人まで。1,000人内は名前で追跡し、外側の減少は公式総数差で残します。"}</small>{data.losses.length?<div className="iv8-loss-list">{data.losses.map((x,i)=><article key={`${x.at}-${i}`}><div className="iv8-loss-row"><time>{when(x.at)}</time><strong>{fmt(x.previousTotal)} → {fmt(x.currentTotal)} <em>-{fmt(x.officialNetLoss)}</em></strong></div>{x.namedRemoved?.length?<div className="iv8-known"><span>名前特定：</span>{x.namedRemoved.map((p,j)=>p.url?<a href={p.url} target="_blank" rel="noreferrer" key={`${p.personKey}-${j}`}>{p.image?<img src={p.image} alt="" referrerPolicy="no-referrer"/>:null}{p.name}</a>:<b key={`${p.personKey}-${j}`}>{p.name}</b>)}</div>:null}{x.unidentifiedNetLoss>0?<div className="iv8-unknown">未特定 -{fmt(x.unidentifiedNetLoss)}人 <small>（1,000人範囲外を含む公式総数の純減）</small></div>:null}</article>)}</div>:<div className="iv8-no-loss">保存済み履歴では直近7日に公式総数の純減はありません。</div>}<div className="iv8-loss-foot">最終照合 {when(data.scannedAt)}</div></section>;
}

export function FastInsightV8(){
  const[magTarget,setMagTarget]=useState<HTMLElement|null>(null),[followTarget,setFollowTarget]=useState<HTMLElement|null>(null);
  useEffect(()=>{
    const tune=()=>{
      ...
      if(notice){const link=notice.querySelector<HTMLAnchorElement>("a");if(link){link.style.display="";link.textContent="自動同期設定";link.href="./notification-import.html?v=2"}}
      ...
      if(followHead){const panel=followHead.closest<HTMLElement>(".iv6-panel");if(panel){let mount=panel.querySelector<HTMLElement>("[data-iv8-follow-loss]");if(!mount){mount=document.createElement("div");mount.dataset.iv8FollowLoss="1";followHead.insertAdjacentElement("afterend",mount)}setFollowTarget(mount)}}
    };
