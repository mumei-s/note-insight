...
function ensureOne(kind:"dashboard"|"notice"){
  ...
  if(!card.classList.contains("needs-update")&&!card.classList.contains("needs-install")){card.querySelectorAll(":scope > .miv5-install-link,:scope > .mumei-top-tool-install").forEach(el=>el.remove());return}
