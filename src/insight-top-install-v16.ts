...
function ensureNoticeControls(){
  ...
  link.className=`miv5-install-link ${TOOL_CLASS} ${CANONICAL}`;link.href=hrefFor("notice");link.textContent="設定 / 更新";link.title="本人通知の設定・更新";link.onclick=e=>e.stopPropagation();
...
function ensureOne(kind:"dashboard"|"notice"){
  ...
  keep.className=`miv5-install-link ${TOOL_CLASS} ${CANONICAL}`;keep.href=hrefFor(kind);keep.textContent="設定 / 更新";keep.title=kind==="dashboard"?"Dashboard同期の設定・更新":"本人通知の設定・更新";keep.onclick=e=>e.stopPropagation();
