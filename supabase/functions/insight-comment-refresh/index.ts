...
async function noteJson(path:string){
  ...
  try{
    const r=await fetch(NOTE+path,{headers:{Accept:"application/json","User-Agent":"Mumei-S-note-INSIGHT/3.3 (+comment-refresh)"},signal:c.signal});
...
function normalize(r:any,parent:string|null=null){
  const row=o(r),u=o(row.user??row.author),urlname=s(u.urlname)||null,key=s(row.key??row.comment_key);if(!key)return null;
  return{key,parent:s(row.parent_key??row.parentKey)||parent,urlname,name:s(u.nickname??u.name,"noteユーザー"),url:urlname?`${NOTE}/${urlname}`:null,image:s(u.user_profile_image_url??u.profile_image_url??u.profileImageUrl)||null,body:commentText(row.comment??row.body??row.text).replace(/\s+/g," ").trim().slice(0,1000),at:s(row.created_at??row.createdAt)||null,liked:Boolean(row.is_creator_liked??row.isCreatorLiked??row.is_liked_by_note_owner),likeCount:n(row.like_count??row.likeCount)};
}
async function commentPages(articleKey:string,parentKey:string|null=null){
  ...
  for(let page=1;page<=20;page++){
    ...
    rows.push(...cp.rows);if(cp.last)break;await sleep(60);
  }
  return rows;
}
async function comments(articleKey:string){
  ...
  for(const raw of roots){
    const root=o(raw),base=normalize(root,null);if(!base)continue;all.set(base.key,base);
    const embedded=normalize(root.latest_creator_reply,base.key);if(embedded)all.set(embedded.key,embedded);
    const replyCount=n(root.reply_count??root.replyCount),known=embedded?1:0;
    if(replyCount>known){try{const replies=await commentPages(articleKey,base.key);for(const rawReply of replies){const reply=normalize(rawReply,base.key);if(reply&&reply.key!==base.key)all.set(reply.key,reply)}}catch(e){console.error("reply-fetch",articleKey,base.key,e instanceof Error?e.message:e)}}
  }
  return[...all.values()];
}
...
async function pendingArticles(dataMember:string){
  ...
  const keys=[...new Set((data||[]).map((r:any)=>String(r.article_key||"")).filter(Boolean))];if(!keys.length)return[];
  const {data:rows,error:re}=await db.from("insight_public_articles").select("article_key,title,url,publish_at,comment_count").eq("member_id",dataMember).in("article_key",keys);if(re){console.error("pending-articles",re);return[]}
...
async function missingArticles(dataMember:string){
  const {data,error}=await db.rpc("insight_missing_comment_articles",{p_member:dataMember,p_limit:30});
  if(error){console.error("missing-comment-rpc",error);return[] as Article[]}
  return(data||[]).map((r:any)=>({key:String(r.article_key),title:String(r.title||"記事"),url:String(r.url||""),publishAt:r.publish_at?String(r.publish_at):null,commentCount:Number(r.comment_count||0)}));
}
async function refreshProfile(profile:any){
  ...
  const [recent,pending,missing]=await Promise.all([recentArticles(noteId),pendingArticles(dataMember),missingArticles(dataMember)]),map=new Map<string,Article>();
  ...
  for(const art of map.values()){
    try{
      ...
      for(const x of rows){
        ...
        if(!before.has(x.key)&&!creator){const fp=`comment-refresh|${art.key}|${x.key}`,text=`${x.name}さんが「${art.title}」に${x.parent?"返信":"コメント"}しました`,{error:ne}=await db.from("insight_notifications").insert({member_id:notifyMember,fingerprint:fp,notification_type:x.parent?"reply":"comment",raw_text:text,actor_name:x.name,actor_url:x.url,target_title:art.title,target_url:art.url,source_url:art.url,occurred_at:row.occurred_at,meta:{source:"comment-refresh",derived:true,articleKey:art.key,commentKey:x.key,parentKey:x.parent}});if(!ne)inserted++;else if(ne.code!=="23505")console.error("comment-notice",ne)}
      }
...
Deno.serve(async req=>{
  ...
  try{
    ...
    const {data:profiles,error}=await db.from("insight_notification_profiles").select("member_id,note_urlname").eq("public_watch_enabled",true).not("verified_at","is",null).limit(100);if(error)throw error;
    const results=[] as any[];for(const p of profiles||[]){results.push(await refreshProfile(p));await sleep(120)}
