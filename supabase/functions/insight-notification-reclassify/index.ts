...
Deno.serve(async req=>{if(req.method==="OPTIONS")return new Response("ok",{headers:H});try{if(req.method!=="POST")return out({ok:false,error:"METHOD_NOT_ALLOWED"},405);const m=await auth(req),ids=m.scope===m.id?[m.id]:[m.scope,m.id],now=new Date().toISOString();const{data,error}=await db.from("insight_notifications").select("id,notification_type,raw_text,target_url,occurred_at,captured_at,meta").in("member_id",ids).order("captured_at",{ascending:false}).limit(6000);if(error)throw error;let checked=0,moved=0,dayStamped=0,pending=0;for(const r of data||[]){const type=String(r.notification_type||"other"),meta=r.meta&&typeof r.meta==="object"?r.meta:{},day=jstDay(r.occurred_at||r.captured_at),nextType=type==="other"?classify(String(r.raw_text||""),r.target_url?String(r.target_url):null):type,needsDay=String((meta as any).event_day_jst||"")!==day,needsType=nextType!==type;if(type==="other")checked++;if(nextType==="other")pending++;if(!needsDay&&!needsType&&type!=="other")continue;const nextMeta={...meta,event_day_jst:day,last_reclassified_at:now,reclassify_version:"daily-v2-exact-kind",reclassify_pending:nextType==="other"};const{error:up}=await db.from("insight_notifications").update({notification_type:nextType,meta:nextMeta}).eq("id",r.id);if(up)throw up;if(needsType)moved++;if(needsDay)dayStamped++}return out({ok:true,noteId:m.noteId,checked,moved,dayStamped,pending,classifiedAt:now})}catch(e){const msg=e instanceof Error?e.message:String(e);console.error(msg);return out({ok:false,error:msg},/LOGIN|SESSION|INACTIVE/.test(msg)?401:500)}});