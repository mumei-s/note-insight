...
function classify(text:string,targetUrl:string|null){const t=text.replace(/\s+/g," ").trim(),target=targetUrl||"";
 ...
 if(/(?:あなたの記事にスキしました|「[^」]{0,500}」にスキしました|新しいスキが\d*件?増えました|さん他?\d*名?があなたの記事にスキしました)/.test(t))return"like";
 ...
 if(/メンバーシップ.{0,80}掲示板.{0,40}投稿/.test(t))return"membership_board";
 if(/メンバーシップを(?:はじめ|始め|開始し)ました/.test(t))return"membership_started";
 if(/メンバーシップ.{0,80}(?:新しいプラン|プラン.{0,30}(?:追加|公開))/.test(t))return"membership_plan";
...
Deno.serve(async(req)=>{if(req.method==="OPTIONS")return new Response("ok",{headers:H});if(req.method!=="POST")return out({ok:false,error:"METHOD_NOT_ALLOWED"},405);try{const who=await identity(req),body=await req.json().catch(()=>({})),suppliedNoteId=String(body?.noteId||"").trim().replace(/^@/,"").toLowerCase();if(!suppliedNoteId||suppliedNoteId!==who.noteId)return out({ok:false,error:"NOTIFICATION_ACCOUNT_MISMATCH",expectedNoteId:who.noteId},409);const incoming=Array.isArray(body?.notifications)?body.notifications.slice(0,1000):[];let inserted=0,updated=0;for(const item of incoming){const raw=clean(item?.raw_text??item?.text,3000);if(!raw||raw.length<2)continue;const sourceUrl=cleanTarget(item?.source_url),targetUrl=cleanTarget(item?.target_url),actorUrl=cleanTarget(item?.actor_url),occurred=clean(item?.occurred_at,80),type=classify(raw,targetUrl),actorName=clean(item?.actor_name,200)||actorFromText(raw),at=occurred&&!Number.isNaN(Date.parse(occurred))?new Date(occurred).toISOString():null,bucket=at?new Date(Math.floor(Date.parse(at)/60000)*60000).toISOString():"no-time",fingerprint=clean(item?.fingerprint,128)||await sha(`${type}|${canonicalText(raw)}|${actorUrl||""}|${targetUrl?.split("?")[0]||""}|${bucket}`);const row={member_id:who.memberId,fingerprint,notification_type:type,raw_text:raw,actor_name:actorName,actor_url:actorUrl,target_title:clean(item?.target_title,500),target_url:targetUrl,source_url:sourceUrl,occurred_at:at,meta:{...(item?.meta&&typeof item.meta==="object"?item.meta:{}),synced_note_id:who.noteId,classifier:"action-v4-exact"}};const {data,error}=await db.from("insight_notifications").upsert(row,{onConflict:"member_id,fingerprint"}).select("id").single();if(error)throw error;if(data?.id){const {count}=await db.from("insight_notifications").select("id",{count:"exact",head:true}).eq("member_id",who.memberId).eq("fingerprint",fingerprint);if(Number(count||0)===1)inserted++;else updated++}}await db.from("insight_notification_sync_runs").insert({member_id:who.memberId,inserted_count:inserted,received_count:incoming.length,source:"browser-account-verified"});return out({ok:true,noteId:who.noteId,received:incoming.length,inserted,updated})}catch(e){const message=e instanceof Error?e.message:"INGEST_ERROR";return out({ok:false,error:message},/REQUIRED|INVALID|UNKNOWN/.test(message)?401:500)}});
