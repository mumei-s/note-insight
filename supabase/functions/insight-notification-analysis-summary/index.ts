...
function displayType(r:any,noteId:string){const rawType=String(r.notification_type||"other"),t=rawType==="other"&&String(r?.meta?.kind||"")==="qa_answer"?"question_answer":rawType,u=String(r.target_url||"");if(t==="membership_reaction"){const owner=/^https:\/\/note\.com\/[^/]+\/membership/.test(u)?targetOwner(u):"";return owner===noteId?"membership_reaction_self":owner?"membership_reaction_joined":"membership_reaction_unknown"}if(t==="reply"){const owner=/\/n\//.test(u)?targetOwner(u):"";return owner===noteId?"reply_self":owner?"reply_other":"reply_unknown"}return t}
function useful(r:any){const type=String(r.notification_type||"other"),src=String(r?.meta?.source||"");if(r?.meta?.noise_reason==="non-notification-api-capture")return false;return !["capture_noise","like","follow","comment","creator_article_posted"].includes(type)&&/^note-notification-/.test(src)&&Boolean(clean(r.raw_text))}
async function notificationPage(ids:string[],start:number){return db.from("insight_notifications").select("id,notification_type,raw_text,actor_name,actor_url,target_url,occurred_at,captured_at,meta",{count:start===0?"exact":undefined}).in("member_id",ids).order("captured_at",{ascending:false}).order("id").range(start,start+999)}
async function notificationRows(ids:string[]){
  const first=await notificationPage(ids,0);if(first.error)throw first.error;const rows=[...(first.data||[])],count=Number(first.count||rows.length),pages=Math.max(1,Math.ceil(count/1000));
...
function summarize(rows:any[],noteId:string,truncated:boolean,sourceTotal:number,period=0,comparisonRows=rows){
  ...
  for(const r of rows){const t=displayType(r,noteId),at=ts(r);types.set(t,(types.get(t)||0)+1);if(t==="other")other++;if(/^(comment|reply|reply_self|reply_other|reply_unknown|membership_board_reply)$/.test(t))comments++;if(t.startsWith("membership_"))membership++;if(t==="purchase"||t==="tip")money++;if(t==="like"||t==="comment_like"||t.startsWith("membership_reaction"))likes++;if(t==="follow")follows++;
    ...
    if(at){const dk=jstDay(at);days.set(dk,(days.get(dk)||0)+1);const parts=new Intl.DateTimeFormat("en-US",{timeZone:"Asia/Tokyo",hour:"2-digit",hourCycle:"h23",weekday:"short"}).formatToParts(new Date(at)),h=Number(parts.find(x=>x.type==="hour")?.value||0),wd=parts.find(x=>x.type==="weekday")?.value||"Sun",wi=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].indexOf(wd);if(h>=0&&h<24)hours[h]++;if(wi>=0)week[wi]++}}
  for(const r of comparisonRows){const at=ts(r);if(at<=now&&at>=now-7*day)recent7++;else if(at<=now&&at>=now-14*day)prev7++;}
  const sample=rows.length,topTypes=[...types.entries()].sort((a,b)=>b[1]-a[1]).slice(0,10),topActors=[...actors.values()].sort((a,b)=>b.count-a.count||b.last-a.last).slice(0,12),peakHour=hours.indexOf(Math.max(...hours)),peakWeek=week.indexOf(Math.max(...week)),weekName=["日","月","火","水","木","金","土"][Math.max(0,peakWeek)],topActorShare=sample&&topActors[0]?topActors[0].count/sample*100:0,last14=Array.from({length:14},(_,i)=>{const ms=now-(13-i)*day,k=jstDay(ms);return{k,label:k.slice(5).replace("-","/"),v:days.get(k)||0}}),timeBands=[{k:"深夜 0–5",v:hours.slice(0,6).reduce((a,b)=>a+b,0)},{k:"朝 6–11",v:hours.slice(6,12).reduce((a,b)=>a+b,0)},{k:"昼 12–17",v:hours.slice(12,18).reduce((a,b)=>a+b,0)},{k:"夜 18–23",v:hours.slice(18,24).reduce((a,b)=>a+b,0)}],classifiedRate=sample?Math.max(0,100-other/sample*100):0;
  return{ownArticleAdds:types.get("my_article_magazine_added")||0,membershipJoins:types.get("membership_join")||0,repeatPeople:[...actors.values()].filter(a=>a.count>1).length,activeDays:days.size,week:week.map((v,i)=>({k:["日","月","火","水","木","金","土"][i]+"曜",v})),period,exactTimeCount:rows.filter(r=>Boolean(r.occurred_at)&&!r.meta?.time_estimated).length,sample,total:sample,sourceTotal,truncated,recent7,prev7,comments,membership,money,likes,follows,people:actors.size,topTypes,topActors,peakHour,weekName,topActorShare,last14,timeBands,coverage:truncated?Math.min(99.9,sample/Math.max(1,sourceTotal)*100):100,classifiedRate,other};
}
Deno.serve(async req=>{if(req.method==="OPTIONS")return new Response("ok",{headers:H});try{if(req.method!=="POST")return out({ok:false,error:"METHOD_NOT_ALLOWED"},405);const m=await auth(req),b=await req.json().catch(()=>({})),period=[7,30,90].includes(Number(b.period))?Number(b.period):0,ids=m.scope===m.id?[m.id]:[m.scope,m.id],n=await notificationRows(ids),seen=new Set<string>(),unique=n.rows.filter(r=>{const k=String(r.meta?.client_signature||r.id);if(seen.has(k))return false;seen.add(k);return true}),rows=period?unique.filter(r=>ts(r)>=Date.now()-period*86400000&&ts(r)<=Date.now()):unique,summary=summarize(rows,m.noteId,n.truncated,rows.length,period,unique);return out({ok:true,noteId:m.noteId,...summary})}catch(e){const msg=e instanceof Error?e.message:String(e);return out({ok:false,error:msg},/LOGIN|SESSION|INACTIVE/.test(msg)?401:500)}});
