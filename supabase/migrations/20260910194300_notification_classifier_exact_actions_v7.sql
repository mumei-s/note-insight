...
set notification_type = case
  when coalesce(target_url,'') like '%kind=circle_plan_join%' then 'membership_join'
  when coalesce(target_url,'') ~ 'kind=board_like_(comment|post)' then 'membership_reaction'
  when coalesce(target_url,'') ~ 'kind=board_reply_(comment|post)' then 'membership_board_reply'
  when raw_text ~ 'さんが.*記事を更新しました' then 'creator_article_posted'
  else notification_type
end
where notification_type='other'
  and (
    coalesce(target_url,'') like '%kind=circle_plan_join%'
    or coalesce(target_url,'') ~ 'kind=board_like_(comment|post)'
    or coalesce(target_url,'') ~ 'kind=board_reply_(comment|post)'
    or raw_text ~ 'さんが.*記事を更新しました'
  );