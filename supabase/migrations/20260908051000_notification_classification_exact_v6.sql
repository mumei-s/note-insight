...
begin
  ...
  elsif target like '%kind=circle_plan_join%' or t like '%あなたのメンバーシップ%参加しました%' or t like '%あなたのメンバーシップ%メンバーになりました%' or t like '%メンバーシップに参加しました%' then
  ...
  elsif t ~ 'さん(他[0-9]+名)?が(新しい)?記事を投稿しました( |$)' then
...
where notification_type='other'
  and regexp_replace(coalesce(raw_text,''), '[[:space:]]+', ' ', 'g') ~ 'さん(他[0-9]+名)?がフォローしました( |$)'
  and coalesce(target_url,'') !~ '/m/';

update public.insight_notifications
set notification_type='creator_article_posted'
where notification_type='other'
  and regexp_replace(coalesce(raw_text,''), '[[:space:]]+', ' ', 'g') ~ 'さん(他[0-9]+名)?が(新しい)?記事を投稿しました( |$)';