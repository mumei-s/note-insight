...
begin
  ...
  if t like '%あなたの記事にスキしました%' or t like '%」にスキしました%' or t like '%新しいスキが%増えました%' then
    new.notification_type := 'like';
  ...
  elsif t like '%メンバーシップをはじめました%' then
...
-- Re-run the corrected trigger over historical rows so title/name words no longer
-- masquerade as notification actions (e.g. チップル, 高評価 in a title, 返信 in a display name).
