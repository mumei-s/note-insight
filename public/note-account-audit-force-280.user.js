...
// @downloadURL https://raw.githubusercontent.com/mumei-s/note-insight/main/public/note-account-audit-force-280.user.js
// @grant none
// @run-at document-start
// ==/UserScript==
(function(){
'use strict';window.__noteAccountAuditV1=1;window.__noteAccountAuditCanonical='2.8.0';
...
function analyzeLikeGraph(outgoing,followers,incomingRows,fallbackCandidates){
 ...
 outgoing.forEach(function(x){
  if(!x.creator)return;
  var id=c(x.creator).toLowerCase(),q=cmap.get(id)||{id:id,count:0,follower:fs.has(id),incoming:incoming.get(id)||0};
  q.count++;q.incoming=incoming.get(id)||0;cmap.set(id,q)
 });
 ...
 return{
  ...
  incomingCreators:incoming.size,top:creators.slice(0,30),mutual:mutual.slice(0,30),oneWay:followerOneWay.slice(0,50)
 };
}
