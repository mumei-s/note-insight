...
// @downloadURL https://raw.githubusercontent.com/mumei-s/note-insight/main/public/note-account-audit-force-290.user.js
// @grant none
// @run-at document-start
// ==/UserScript==
(function(){
'use strict';window.__noteAccountAuditV1=1;window.__noteAccountAuditCanonical='2.9.0';
...
function analyzeLikeGraph(outgoing,followers,incomingRows,fallbackCandidates){
 ...
 outgoing.forEach(function(x,idx){
  if(!x.creator)return;
  var id=c(x.creator).toLowerCase(),q=cmap.get(id)||{id:id,count:0,follower:fs.has(id),incoming:incoming.get(id)||0,likedArticleNewestAt:null,likedArticleOldestAt:null,latestLikeRank:0};
...
function render(r){
 ...
 card('相互スキ',(lg.incomingComplete?'':'>=')+lg.followerMutualCreators+'人',lg.incomingComplete?'まこの全'+lg.incomingArticles+'記事で照合':'全記事照合は未完了')+
 card('全員走査',network.scanned+'/'+network.total,network.backend==='server'?'専用サーバー':'端末フォールバック')+
 card('販売型一致',market.score+'/100',market.label)+
 '</div>'+
 '<div class="naa-tabs">'+
 '<button data-tab="answer" class="on">答え</button><button data-tab="likes">スキ</button><button data-tab="activity">停止</button><button data-tab="net">関係網</button><button data-tab="market">購入</button><button data-tab="ext">外部</button>'+
 '</div>'+
 ...
 '</details></section>'+'<section class="naa-pane" data-pane="net">'+
