...
// @version      3.0.0
// @description  note画像ツールバーを操作せず、ProseMirror編集データへ直接リンクを設定するテスト
...
(function(){
'use strict';
if(window.__MUMEI_DIRECT_URL_300__) return;
window.__MUMEI_DIRECT_URL_300__=true;
...
function hideOld(){
  for(const el of [...document.querySelectorAll('[id]')]){
    if(OLD_PREFIXES.some(p=>el.id.startsWith(p))){el.style.setProperty('display','none','important');el.style.setProperty('pointer-events','none','important')}
  }
}
function cards(){
...
function mount(){
  ...
  if(!document.getElementById(PANEL)){
    const p=document.createElement('div');p.id=PANEL;p.textContent='DIRECT URL 3.0';
    Object.assign(p.style,{position:'fixed',left:'8px',bottom:'58px',zIndex:'2147483646',maxWidth:'340px',padding:'6px 8px',borderRadius:'8px',background:'#065f46',color:'#fff',fontSize:'11px',lineHeight:'1.3',boxShadow:'0 4px 12px rgba(0,0,0,.25)',pointerEvents:'none'});document.body.appendChild(p)
  }
  if(!document.getElementById(BTN)){
    const b=document.createElement('button');b.id=BTN;b.type='button';b.textContent='DIRECT URL 3.0';
    Object.assign(b.style,{position:'fixed',left:'8px',bottom:'10px',zIndex:'2147483647',border:'0',borderRadius:'10px',padding:'10px 13px',background:'#059669',color:'#fff',fontSize:'13px',fontWeight:'800',boxShadow:'0 4px 14px rgba(0,0,0,.28)',touchAction:'manipulation'});b.addEventListener('click',run);document.body.appendChild(b)
  }
}
