...
(function(){
'use strict';
if(window.__MUMEI_CARD_COMPLETE_403__) return;
window.__MUMEI_CARD_COMPLETE_403__=true;
const ITEMS=[
...
const CREATOR='無名S note',CARD_W=860,CARD_H=140;
const BTN='mumei-direct-url-300-btn',PANEL='mumei-direct-url-300-panel';
...
function status(t,bad=false){const p=document.getElementById(PANEL);if(!p)return;p.textContent=t;p.style.background=bad?'#991b1b':'#065f46';p.style.display='block'}
function showButton(show=true){const b=document.getElementById(BTN);if(b)b.style.display=show?'block':'none'}
function hideOld(){for(const el of [...document.querySelectorAll('[id]')]){if((OLD_PREFIXES.some(p=>el.id.startsWith(p))||OLD_EXACT.includes(el.id))&&el.id!==BTN&&el.id!==PANEL){el.style.setProperty('display','none','important');el.style.setProperty('pointer-events','none','important')}}}
...
function fitLines(c,text,maxWidth,maxLines){const chars=[...text],lines=[];let line='',used=0;for(let i=0;i<chars.length;i++){const test=line+chars[i];if(c.measureText(test).width>maxWidth&&line){lines.push(line);used+=line.length;line=chars[i];if(lines.length===maxLines-1)break}else line=test}if(lines.length<maxLines&&line){let rest=[...text].slice(used).join('');if(c.measureText(rest).width>maxWidth){while(rest&&c.measureText(rest+'…').width>maxWidth)rest=rest.slice(0,-1);rest+='…'}lines.push(rest)}return lines.slice(0,maxLines)}
async function makeCard(item){const thumbUrl=await getThumb(item.url),thumbBlob=await xhr(thumbUrl,'blob'),bmp=await blobToBitmap(thumbBlob),canvas=document.createElement('canvas');canvas.width=CARD_W;canvas.height=CARD_H;const c=canvas.getContext('2d');c.fillStyle='#fff';c.fillRect(0,0,CARD_W,CARD_H);c.strokeStyle='#d9dde3';c.lineWidth=1.5;roundedRect(c,1,1,CARD_W-2,CARD_H-2,12);c.stroke();const thumbW=220,thumbH=124,thumbX=CARD_W-thumbW-8,thumbY=8,textX=16,textW=thumbX-textX-14;c.fillStyle='#171b21';c.font='700 20px system-ui,-apple-system,sans-serif';c.textBaseline='top';fitLines(c,item.title,textW,2).forEach((line,i)=>c.fillText(line,textX,14+i*27));c.fillStyle='#626975';c.font='15px system-ui,-apple-system,sans-serif';c.fillText(CREATOR,textX,108);const iw=bmp.width||bmp.naturalWidth,ih=bmp.height||bmp.naturalHeight,scale=Math.min(thumbW/iw,thumbH/ih),dw=iw*scale,dh=ih*scale;c.fillStyle='#f7f8fa';roundedRect(c,thumbX,thumbY,thumbW,thumbH,8);c.fill();c.save();roundedRect(c,thumbX,thumbY,thumbW,thumbH,8);c.clip();c.drawImage(bmp,thumbX+(thumbW-dw)/2,thumbY+(thumbH-dh)/2,dw,dh);c.restore();if(bmp.close)bmp.close();const blob=await new Promise((resolve,reject)=>canvas.toBlob(b=>b?resolve(b):reject(new Error('カード生成失敗')),'image/png',1));return new File([blob],String(item.index).padStart(2,'0')+'_thin.png',{type:'image/png'})}
async function prepare(){const out=[];for(let i=0;i<ITEMS.length;i++){status(`カード生成 ${i+1}/10…`);out.push(await makeCard(ITEMS[i]))}return out}
...
function findView(){const root=editor();if(!root)return null;const seeds=[];let n=root;for(let i=0;i<6&&n;i++,n=n.parentElement)seeds.push(n);const seen=new Set(),q=[];const push=(v,d=0)=>{if(!v||seen.has(v)||d>7)return;if(typeof v!=='object'&&typeof v!=='function')return;seen.add(v);q.push([v,d])};seeds.forEach(s=>push(s,0));let steps=0;while(q.length&&steps<12000){steps++;const[v,d]=q.shift();if(looksLikeView(v))return v;let keys=[];try{keys=Object.getOwnPropertyNames(v)}catch(_){continue}for(const k of keys){if(['window','document','ownerDocument','parentNode','children','childNodes','style'].includes(k))continue;let x;try{x=v[k]}catch(_){continue}if(looksLikeView(x))return x;if(d<7&&x&&(typeof x==='object'||typeof x==='function')&&x!==window&&x!==document)push(x,d+1)}}return null}
...
function trySetAttr(view,pos,node,url){for(const key of attrKeys(node)){try{const attrs={...node.attrs,[key]:url};view.dispatch(view.state.tr.setNodeMarkup(pos,node.type,attrs,node.marks));if(nodeHasLink(view.state.doc.nodeAt(pos),url))return true}catch(_){}}return false}
...
async function directAll(imgs){status('DIRECT URLを書き込み中…');const view=findView();if(!view)throw new Error('EditorView取得失敗');let ok=0;for(let i=0;i<Math.min(10,imgs.length);i++){status(`DIRECT URL ${i+1}/10…`);if(setDirect(view,imgs[i],ITEMS[i].url))ok++;else throw new Error(`${i+1}枚目URL設定失敗`);await sleep(80)}return ok}
async function waitInserted(timeout=45000){const root=editor();if(!root)return[];const end=Date.now()+timeout;while(Date.now()<end){const added=[...root.querySelectorAll('img')].filter(x=>!beforeImages.has(x));if(added.length>=10)return added;await sleep(350)}return[...root.querySelectorAll('img')].filter(x=>!beforeImages.has(x))}
async function inject(input){if(!armed||consumed||!preparedFiles.length||!imageInput(input)||beforeInputs.has(input))return false;consumed=true;armed=false;if(timer)clearTimeout(timer);timer=null;const send=preparedFiles;preparedFiles=[];try{const dt=new DataTransfer();send.forEach(f=>dt.items.add(f));input.files=dt.files;input.dispatchEvent(new Event('input',{bubbles:true}));input.dispatchEvent(new Event('change',{bubbles:true}));status('10枚を一括挿入中…');const imgs=await waitInserted();if(imgs.length<10)throw new Error(`画像 ${imgs.length}/10`);const ok=await directAll(imgs.slice(0,10));status(`完成：画像 10/10・URL ${ok}/10 ✅`);showButton(true);consumed=false;return true}catch(e){consumed=false;status('停止：'+(e?.message||String(e)),true);showButton(true);return false}}
