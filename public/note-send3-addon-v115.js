...
if(page.__MUMEI_SEND3_V115__)return;
page.__MUMEI_SEND3_V115__=true;
...
const BTN='mumei-send3-v115',STATUS='mumei-send3-status-v115';
const ACTIVE='mumei_send3_active_v115',ROWS='mumei_send3_rows_v115';
...
function webpackRequire(){const chunks=page.webpackChunk_N_E;if(!chunks||typeof chunks.push!=='function')return null;let req=null;const id=990000000+Math.floor(Math.random()*9000000);try{chunks.push([[id],{},r=>{req=r}])}catch(_){}return req}
...
function shutdownForManual(){
 ...
 page.alert('画像🔗3枚を先に保存しました。\n\n3件URLをコピー済みです。\n本文最下部へ1回貼付 → 各URL末尾で実Enter → 公開→更新 → サブ垢で通知確認。\n\n確認後、編集画面を再読み込みして「削」。カード3件だけ消し、画像🔗3枚は残します。');
}

async function inject(input){const a=imageArm;if(!a||a.used||!imageInput(input))return;a.used=true;try{const dt=new page.DataTransfer();a.files.forEach(f=>dt.items.add(f));input.files=dt.files;input.dispatchEvent(new page.Event('input',{bubbles:true}));input.dispatchEvent(new page.Event('change',{bubbles:true}));status('画像3枚を一括アップロード中…');const created=await waitImages(a);let tr=a.view.state.tr;created.forEach((h,i)=>{tr=tr.setNodeMarkup(h.pos,h.node.type,{...h.node.attrs,link:URLS[i]},h.node.marks);a.rows[i].nodeId=String(h.node.attrs?.id||'')});a.view.dispatch(tr);ensureEnd(a.view);setRows(a.rows);await saveDraft(a.view,'画像🔗3枚を通常保存中…',()=>verifyImages(a.view,a.rows));status('画像🔗3枚 完成 ✅ URL一覧をコピーして通知工程へ');shutdownForManual()}catch(e){status('3送停止：'+(e?.message||String(e)),true);cancelArm();busy=false}}

async function runSend3(){if(busy||!isEdit())return;busy=true;try{const v=findView();if(!v)throw new FatalError('EditorViewなし');core();const hits=[];for(const u of URLS){hits.push(...officialCards(v,u),...exactUrlParagraphs(v,u));const im=findImage(v,u);if(im)hits.push(im)}hits.push(...targetUrlTextblocks(v));deleteHits(v,hits);ensureEnd(v);await saveDraft(v,'3送の旧3件データを初期化して保存中…',()=>{for(const u of URLS)if(officialCards(v,u).length||exactUrlParagraphs(v,u).length||findImage(v,u))throw new FatalError('初期化残り');return serializedBody(v)});status('10と同じ方式で3件サムネイル準備中…');const made=[];for(let i=0;i<3;i++){made.push(await makeFile(URLS[i],i+1));status(`画像準備 ${i+1}/3…`)}const rows=made.map((m,i)=>({index:i+1,url:URLS[i],title:m.title,nodeId:''}));setRows(rows);imageArm={view:v,rows,files:made.map(x=>x.file),used:false,beforeIds:new Set(imageNodes(v).map(e=>String(e.node.attrs?.id||'')).filter(Boolean)),beforeInputs:new Set(document.querySelectorAll('input[type="file"]')),timer:null};install();imageArm.timer=setTimeout(()=>{status('画像選択待機が3分を超えました',true);cancelArm();busy=false},180000);status('準備OK｜本文をタップ→「＋」→「画像」を1回だけ')}catch(e){status('3送停止：'+(e?.message||String(e)),true);busy=false}}
...
function mount(){if(!document.body||!isEdit())return;const old=document.getElementById('mumei-notify-panel-v1100');if(old&&!document.getElementById(BTN)){const b=document.createElement('button');b.id=BTN;b.type='button';b.textContent='3送';b.title='10と同じ方式：画像先行→URLコピー→実Enter';b.style.background='#d97706';b.addEventListener('click',e=>{e.stopPropagation();runSend3()});old.insertBefore(b,old.firstChild)}if(old){const d=old.querySelector('[data-action="delete"]');if(d&&!d.dataset.send3v115){d.dataset.send3v115='1';d.addEventListener('click',e=>{if(localStorage.getItem(ACTIVE)==='1'){e.preventDefault();e.stopImmediatePropagation();deleteCardsOnly()}},true)}}if(!document.getElementById(STATUS)){const s=document.createElement('div');s.id=STATUS;Object.assign(s.style,{position:'fixed',right:'4px',bottom:'156px',zIndex:'2147483647',maxWidth:'320px',padding:'5px 7px',borderRadius:'7px',background:'#92400e',color:'#fff',font:'700 10px/1.35 system-ui',boxShadow:'0 2px 8px rgba(0,0,0,.25)'});document.body.appendChild(s);status(`3送 ${VERSION}｜10と同方式`)}}
