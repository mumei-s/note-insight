...
function candidateNodes(root){
 if(!root)return[];
 const selector='[class*="MessageContent" i] [class*="text" i],[class*="MessageBubble" i],[class*="message_body" i],[class*="message-content" i],[data-message-id],[data-testid*="message" i],[class*="messageItem" i],[class*="chatItem" i],[class*="bubble" i],[class*="messageBody" i],[class*="messageText" i],[class*="message_content" i],[class*="message_text" i],[class*="message-body" i],[class*="message-text" i],li,[role="listitem"],article';
...
function showStatus(state){
 if(window.top!==window.self||!dmRoute()||!document.body)return;
...
async function syncRoomsInBackground(a,rooms){
 ...
 finally{bgRunning=false;showStatus(await get(key(CHECK,a.id),{}))}
}
...
async function run(){
 ...
 roomRunning=true;
 try{
  const r=await scanRoom(a,threadKey);await reportRoom(a,threadKey,r);
...
{let timer=0,running=false;const schedule=(ms=180)=>{clearTimeout(timer);timer=setTimeout(()=>{if(running)return;running=true;Promise.resolve(run()).catch(()=>{}).finally(()=>{running=false})},ms)};schedule(220);const observe=()=>{if(!document.documentElement){document.addEventListener('DOMContentLoaded',observe,{once:true});return}new MutationObserver(records=>{if(records.some(r=>!r.target.closest?.('#mumei-dm-reader-status')))schedule(180)}).observe(document.documentElement,{subtree:true,childList:true})};observe();addEventListener('pageshow',()=>{manualPaused=false;schedule(180)});addEventListener('focus',()=>schedule(180));window.__mumeiInsightDmReaderV1Api={version:VERSION,stop,scanRoom,syncRoomsInBackground,candidateNodes,messageData,roomAnchors,run:()=>{if(running)return false;running=true;Promise.resolve(run()).catch(()=>{}).finally(()=>{running=false});return true}}}
})();