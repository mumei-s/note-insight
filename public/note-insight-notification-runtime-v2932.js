...
const SRC='note-notification-manual-sync-v2932';
...
function knownLike(el){try{return el.matches(KNOWN)}catch{return false}}
function rowLike(el){if(!el||el.closest?.('[id^="mumei-v29"]'))return false;const t=text(el);if(t.length<5||t.length>900||!ACTION.test(t))return false;if(knownLike(el))return true;if(el.matches?.('li,[role="listitem"],article'))return true;if(links(el).some(a=>{try{return new URL(a.getAttribute('href'),location.href).hostname.endsWith('note.com')}catch{return false}}))return true;return Boolean(el.querySelector?.('img,time,[datetime]'))}
function rawRows(root){if(!root)return[];let xs=[...root.querySelectorAll(KNOWN)].filter(rowLike);if(!xs.length)xs=[...root.querySelectorAll('li,[role="listitem"],article')].filter(rowLike);if(!xs.length)xs=[...root.querySelectorAll('div')].filter(el=>el.children.length<=20&&rowLike(el)&&![...el.children].some(ch=>rowLike(ch)));return[...new Set(xs)].filter(rowLike)}
...
function rootHintOk(el){return Boolean(el&&el.isConnected&&shown(el)&&(rawRows(el).length||commonShell(exact('通知'),exact('お知らせ'))===el||el.hasAttribute?.(HOST_ATTR)))}
function notificationRoot(hint=null){if(rootHintOk(hint))return hint;const marked=document.querySelector(`[${HOST_ATTR}="1"]`);if(rootHintOk(marked))return marked;const shell=commonShell(exact('通知'),exact('お知らせ'));if(shell)return shell;const candidates=[...document.querySelectorAll(ROOT)].filter(el=>shown(el)&&el!==document.body&&el!==document.documentElement&&!el.closest?.('[id^="mumei-v29"]'));
const ranked=candidates.map(el=>({el,n:rawRows(el).length,h:/(通知|お知らせ)/u.test(text(el).slice(0,220))||/notification|notice/i.test(String(el.id||'')+' '+String(el.className||''))})).filter(x=>x.n||x.h).sort((a,b)=>(b.n*100+(b.h?50:0))-(a.n*100+(a.h?50:0)));return ranked[0]?.el||null}
...
function row(el){const raw=text(el);if(!raw||raw.length>900)return null;const ls=noteLinks(el),target=targetLink(ls),actor=actorLink(ls,raw),tm=el.querySelector('time[datetime]')?.getAttribute('datetime')||el.querySelector('[datetime]')?.getAttribute('datetime')||null,m=stripTime(raw).match(/^(.{1,180}?)\s*さん(?:他\d+名)?(?:が|の|から|より|に)/u);return{raw_text:raw,actor_name:actor?.t||m?.[1]||null,actor_url:actor?.u||null,actor_image_url:actorImage(actor,el),target_title:target?.t||null,target_url:target?.u||null,source_url:target?.u||actor?.u||'https://note.com/',occurred_at:tm,meta:{source:SRC,via:'notification-panel-manual-v2932',userscript:V}}}
const signature=r=>[stripTime(r.raw_text),String(r.target_url||'').split('#')[0],String(r.actor_url||'').split('?')[0]].join('|');
function ordered(root){return rawRows(root).map(el=>({el,r:row(el)})).filter(x=>x.r).sort((a,b)=>a.el.getBoundingClientRect().top-b.el.getBoundingClientRect().top)}
...
async function sendBatch(rows,a){if(!rows.length)return{confirmed:[],inserted:0,updated:0,skipped:0,blocked:0};const token=String(await get(key(TOK,a.id),'')||'');if(!token)throw new Error('PAIR_REQUIRED');let confirmed=[],inserted=0,updated=0,skipped=0,blocked=0;for(let i=0;i<rows.length;i+=BATCH){const part=rows.slice(i,i+BATCH),payload=part.map(r=>({...r,meta:{...(r.meta||{}),client_signature:signature(r)}})),p=await request(ING,{noteId:a.id,notifications:payload},{'X-Ingest-Token':token});inserted+=Number(p.inserted||0);updated+=Number(p.updated||0);skipped+=Number(p.skipped||0);blocked+=Number(p.blocked||0);const cs=Array.isArray(p.confirmedClientSignatures)?p.confirmedClientSignatures:[];if(cs.length)confirmed.push(...cs.map(String));else if(!Number(p.skipped||0)&&!Number(p.blocked||0))confirmed.push(...part.map(signature))}return{confirmed:[...new Set(confirmed)],inserted,updated,skipped,blocked}}
function markerStyle(){if(document.getElementById('mumei-v2932-marker-style'))return;const s=document.createElement('style');s.id='mumei-v2932-marker-style';s.textContent='[data-mumei-insight-boundary-v2932="1"]{position:relative!important;border-top:3px solid #69db91!important}[data-mumei-insight-boundary-v2932="1"]::before{content:attr(data-mumei-insight-boundary-text-v2932)!important;display:block!important;margin:5px 7px;padding:6px!important;border:1px solid #55b97b!important;border-radius:7px!important;background:#0b3320!important;color:#d5ffe2!important;font:900 9px/1.35 system-ui!important;text-align:center!important}';document.documentElement.append(s)}
async function mark(id,rootHint=null){markerStyle();for(const el of document.querySelectorAll('[data-mumei-insight-boundary-v2932]')){delete el.dataset.mumeiInsightBoundaryV2932;delete el.dataset.mumeiInsightBoundaryTextV2932}const cp=await checkpoint(id);if(!cp.boundary)return;const root=notificationRoot(rootHint);if(!root)return;for(const el of rawRows(root)){const r=row(el);if(r&&signature(r)===cp.boundary){el.dataset.mumeiInsightBoundaryV2932='1';el.dataset.mumeiInsightBoundaryTextV2932='✓ 保存済み境界｜次回はここから上だけ保存';break}}}
async function manualResume(rootHint=null){const now=Date.now();if(busy)return;if(now-lastManualAt<COOLDOWN){emit('error',`連続操作防止中。${Math.ceil((COOLDOWN-now+lastManualAt)/1000)}秒待ってください`);return}lastManualAt=now;const root=notificationRoot(rootHint);if(!root){emit('error',friendly('NOTIFICATION_PANEL_NOT_FOUND'));return}if(!rawRows(root).length){emit('error',friendly('NOTIFICATION_ROWS_WAITING'));return}busy=true;let box=null,startScroll=0,a=null;try{a=await account();if(!a)throw new Error('NOTE_LOGIN_REQUIRED');const saved=await loadSaved(a.id),cp=await checkpoint(a.id),oldBoundary=String(cp.boundary||''),seenRun=new Set(),host=root;box=scrollBox(host);startScroll=box?.scrollTop||0;let boundaryFound=!oldBoundary,newestCheckpoint='',newCount=0,checked=0,skipped=0,blocked=0,stall=0;emit('saving','手動保存中…新しい通知だけ確認します');for(let step=0;step<MAX_STEPS&&newCount<MAX_NEW;step++){
 ...
 const savedHere=unique.filter(x=>saved.has(signature(x.r))).length;if(boundaryFound||savedHere>=4)break;
