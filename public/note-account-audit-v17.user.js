// ==UserScript==
// @name note こあく まこ調査 v1.7
// @namespace https://github.com/mumei-s/note-insight
// @version 1.7.0
// @description 公開コメント・返信・スキ・フォロー関係とフォロワー構成を確認。本人同定は公開明示だけを根拠にします。
// @match https://note.com/*
// @updateURL https://raw.githubusercontent.com/mumei-s/note-insight/main/public/note-account-audit-v17.user.js
// @downloadURL https://raw.githubusercontent.com/mumei-s/note-insight/main/public/note-account-audit-v17.user.js
// @grant none
...
var SNAP='note-account-audit:snapshots',NET='note-account-audit:all-followers:v17:koakumako',last=null;
var PRESET_TARGET='koakumako',AUTO_KEY='note-account-audit:auto:koakumako:v170';
var PRESET_ARTS=[
...
var all=followers.slice(0,1000),tf=new Set(targetFollowings.map(function(x){return x.id})),state={v:17,done:{},hubs:{},total:all.length,completed:false};
try{var old=JSON.parse(localStorage.getItem(NET)||'null');if(old&&old.v===17&&old.done&&old.hubs)state=old}catch(e){}
if(state.completed&&state.total!==all.length)state={v:17,done:{},hubs:{},total:all.length,completed:false};
state.total=all.length;
function save(){try{localStorage.setItem(NET,JSON.stringify(state))}catch(e){}}
var stopped=false,stopReason='';
for(var i=0;i<all.length;i++){
 var f=all[i];
 if(state.done[f.id])continue;
 set('全フォロワー走査 '+Object.keys(state.done).length+'/'+all.length+'｜@'+f.id);
 try{
  var fp=await profile(f.id);
  var pages=Math.max(1,Math.min(10,Math.ceil(Math.max(0,fp.followings)/20)));
  var fl=await relLimited(fp,'followings',pages);
  state.done[f.id]={notes:fp.notes,bioThin:(!fp.bio||fp.bio.length<4),mass:(fp.followings>=300&&fp.followings>=Math.max(500,fp.followers*10)),followings:fp.followings,followers:fp.followers,read:fl.length};
  fl.forEach(function(x){
   if(!x.id||x.id===PRESET_TARGET)return;
   var q=state.hubs[x.id]||{id:x.id,name:x.name||x.id,image:x.image||'',count:0,followers:[],targetFollows:false};
   q.count++;if(q.followers.length<10)q.followers.push(f.id);q.targetFollows=tf.has(x.id);state.hubs[x.id]=q;
  });
  save();
 }catch(e){
  var msg=String(e&&e.message||e);
  if(/403|429/.test(msg)){stopped=true;stopReason=msg;save();break}
  state.done[f.id]={error:msg};save();
 }
 await wait(250);
}
var ids=new Set(all.map(function(x){return x.id})),rows=Object.keys(state.done).filter(function(id){return ids.has(id)}).map(function(id){return state.done[id]}).filter(function(x){return x&&!x.error});
var known=rows.filter(function(x){return x.notes>=0}),low=known.filter(function(x){return x.notes<=1}).length,empty=rows.filter(function(x){return x.bioThin}).length,mass=rows.filter(function(x){return x.mass}).length;
var scanned=Object.keys(state.done).filter(function(id){return ids.has(id)}).length;
if(!stopped&&scanned>=all.length){state.completed=true;save()}
var hubs=Object.keys(state.hubs).map(function(k){return state.hubs[k]}).filter(function(x){return x.count>=2}).sort(function(a,b){return b.count-a.count}).slice(0,100);
return{sample:all.length,scanned:scanned,total:all.length,completed:state.completed,stopped:stopped,stopReason:stopReason,hubs:hubs,quality:{sample:rows.length,low:pct(low,known.length),empty:pct(empty,rows.length),mass:pct(mass,rows.length)}}
}
...
+'<h3>フォロワー全員の共通フォロー先</h3><div class="naa-ev"><b>走査 '+network.scanned+'/'+network.total+'人'+(network.completed?'｜全員完了':'｜途中保存済み')+'</b>'+(network.stopped?'<br>制限検知で停止：次回続きから再開':'')+'</div><small>取得できる公開フォロワーを全員対象。各フォロワーの公開フォロー先は最大200件まで確認し、共通先を集計します。</small>'
