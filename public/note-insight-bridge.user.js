...
async function run(name,transform){if(loaded.has(name))return;let s=stripHeader(await source(name));if(transform)s=transform(s);eval(s+'\n//# sourceURL='+(BASE+name));loaded.add(name)}
async function notification(){if(document.documentElement.getAttribute('data-mumei-bridge-notice')==='1')return;document.documentElement.setAttribute('data-mumei-bridge-notice','1');try{await run('note-insight-notification-v3.user.js')}catch(e){document.documentElement.removeAttribute('data-mumei-bridge-notice');console.warn('[INSIGHT Bridge] notification retry',e);setTimeout(notification,1500)}}
async function dashboard(){if(document.documentElement.getAttribute('data-mumei-bridge-dash')==='1')return;document.documentElement.setAttribute('data-mumei-bridge-dash','1');try{const path=location.pathname.toLowerCase(),direct=new URLSearchParams(location.search).get('mumei_dashboard_sync')==='1';if(path.includes('/sitesettings/stats')||direct)await run('note-insight-dashboard-sync-core-v1.1.0.js');await run('note-insight-dashboard-sync.user.js')}catch(e){document.documentElement.removeAttribute('data-mumei-bridge-dash');console.warn('[INSIGHT Bridge] dashboard retry',e);setTimeout(dashboard,1500)}}
