...
const LEGACY='https://mumei-s.github.io/note-insight/note-insight-notification-reader-v323-legacy.js?v=resume-20260917';
...
function status(message,label='下から自動読込'){document.dispatchEvent(new CustomEvent('mumei-v3-reader-status',{detail:{message,kind:'saving',label}}))}
async function findCheckpoint(host,boundary){if(!boundary)return false;let stable=0,last=-1;for(let i=0;i<80;i++){if(rows().some(r=>legacySig(r)===boundary))return true;host.scrollTop=Math.max(0,host.scrollHeight-host.clientHeight);await sleep(280);const h=host.scrollHeight;if(h===last)stable++;else stable=0;last=h;if(stable>=4)break}return rows().some(r=>legacySig(r)===boundary)}
...
try{
 const src=await (await fetch(LEGACY,{cache:'no-store'})).text();
...
const original=api.scan.bind(api);let resumeRunning=false;
api.scan=async function(){
 ...
 resumeRunning=true;const host=scrollHost(),restore=Number(host?.scrollTop||0);
 try{
  ...
  status('前回保存位置から新しい通知へ読み上げます…','下から自動読込');
  let last=Infinity,guard=0;
  while(host&&host.scrollTop>1&&guard++<80){
   await original();
  ...
  await original();
 }finally{try{if(host)host.scrollTop=restore}catch{}resumeRunning=false}
};
