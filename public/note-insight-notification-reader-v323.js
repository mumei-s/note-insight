...
function progressNode(){let el=document.getElementById(PROGRESS_ID);if(el)return el;el=document.createElement('div');el.id=PROGRESS_ID;el.setAttribute('role','status');el.style.cssText='position:fixed;left:8px;right:8px;bottom:calc(env(safe-area-inset-bottom,0px) + 104px);z-index:2147483646;display:none;padding:7px 10px;border:1px solid #3c5d72;border-radius:9px;background:rgba(7,18,27,.96);color:#dff7ff;font:900 11px/1.35 system-ui,-apple-system,sans-serif;box-shadow:0 -2px 8px rgba(0,0,0,.28);pointer-events:none;text-align:center';(document.body||document.documentElement).appendChild(el);return el}
let progressTimer=0;
