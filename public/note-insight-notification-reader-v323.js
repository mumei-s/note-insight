...
const VERSION='3.2.83',PROTOCOL='3.2.83';
...
async function collectFullHistory(p,host,a,saved){
 ...
 for(let step=0;step<FULL_MAX_STEPS&&seen.size<FULL_MAX_ROWS;step++){
  ...
  if(nextPanel!==p){p=nextPanel;host=scrollHost(p)}
  let added=0;
  ...
  if(step%5===0)status(`全読み中… ${seen.size}件確認 / ${confirmed}件保存確認`,'saving','全読み',{readCount:seen.size,savedCount:confirmed,mode:'full'});
  if(!host)break;
  const max=Math.max(0,host.scrollHeight-host.clientHeight),before=host.scrollTop;
 ...
 return{confirmed,seenCount:seen.size}
}
async function scanFull(){
 ...
 try{
  ...
  else status(`✓ 全読み完了｜${result.seenCount}件照合・${count}件保存確認`,'done','✓ 全読み完了',{readCount:result.seenCount,savedCount:count,mode:'full'});
  return count
 }catch(e){
  ...
  status(`⚠ 全読み失敗：${msg}${pending?`｜${pending}件は次回保存待ち`:''}`,'error','全読み再試行',{pendingCount:pending,mode:'full'});
  return 0
 }finally{scanning=false;document.dispatchEvent(new CustomEvent('mumei-v3-reader-stopped'))}
}
...
window.__mumeiV3Reader323={scan,scanFull,isScanning:()=>scanning,readyToScan,version:VERSION};
