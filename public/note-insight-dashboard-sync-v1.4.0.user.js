...
// @version      1.4.0
// @description  INSIGHTの既存ボタンから本人確認・公式Dashboard読込・INSIGHT復帰まで自動実行。通常時パネル非表示、失敗時のみ復旧表示。
...
(() => {
  ...
  function hidePanel(){const p=panel();if(p){p.style.display='none';p.setAttribute('aria-hidden','true')}}
  function showPanel(){const p=panel();if(p){p.style.display='block';p.removeAttribute('aria-hidden')}}
  ...
  function watchCompletion(){const done=()=>{const s=document.querySelector('#mumei-dashboard-sync .status'),t=s?.textContent||'';if(!/同期完了/.test(t))return false;sessionStorage.removeItem(FLOW_KEY);markVersion();hidePanel();const back=safeReturn(localStorage.getItem(RETURN_KEY)||'');localStorage.removeItem(RETURN_KEY);if(back){const u=new URL(back);u.searchParams.set('dashboardSync','ok');u.searchParams.set('dashboardVersion',VERSION);u.searchParams.set('dashboardAt',new Date().toISOString());setTimeout(()=>location.assign(u.href),450)}return true};if(done())return;const o=new MutationObserver(()=>{if(done())o.disconnect()});o.observe(document.documentElement,{subtree:true,childList:true,characterData:true});setTimeout(()=>o.disconnect(),90000)}
  async function startRead(){if(!await waitPanel())throw new Error('DASHBOARD_TOOL_CORE_NOT_READY');hidePanel();watchCompletion();await sleep(1300);const btn=document.getElementById('mumei-dash-read');if(!btn)throw new Error('DASHBOARD_READ_BUTTON_NOT_FOUND');btn.click();hidePanel()}
  async function boot(){try{const pairedNow=await pairPending(),flow=sessionStorage.getItem(FLOW_KEY)==='1';if(!pairedNow&&!flow){panel()?.remove();return}if(!looksDashboard()){setCoreStatus('公式Dashboardへ移動中…');if(location.pathname!=='/sitesettings/stats'){location.assign('https://note.com/sitesettings/stats');return}}await startRead()}catch(e){ensureCorePanel();showPanel();setCoreStatus(`⚠ ${e?.message||e}｜このパネルは復旧時だけ表示されます`,'warn')}}
