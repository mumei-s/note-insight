...
async function safeScan(){const net=window.__mumeiNotificationNetwork3300;if(net&&typeof net.syncCurrent==='function'){const r=await net.syncCurrent({waitMs:2600});if(r?.handled)return{source:'network',saved:Number(r.saved||0),received:Number(r.received||0)}}const current=await waitForRows();if(!current)return{source:'none',saved:0,received:0};if(current!==shell)markSurface(current);await window.__mumeiV3Checkpoint325?.restore?.();const again=await waitForRows(1200);if(!again)return{source:'none',saved:0,received:0};const api=reader();if(!api||typeof api.scan!=='function')throw new Error('通知Readerを起動できませんでした');return{source:'dom',saved:Number(await api.scan()||0)}}
async function manualRead(btn){btn.classList.remove('err','done');btn.textContent='読込中';try{const r=await safeScan();if(r?.source==='network'){btn.classList.add('done');btn.textContent='✓通信';btn.title=`通知通信を直接確認｜${Number(r.received||0)}件確認`}else if(r?.source==='dom'){btn.classList.add('done');btn.textContent='✓完了'}else btn.textContent='読込'}catch(e){btn.classList.add('err');btn.textContent='再読込';btn.title=String(e?.message||e)}finally{keepDockVisible()}}
function hideProgress(){const p=document.getElementById(PROGRESS);if(p)p.style.setProperty('display','none','important')}
function onStatus(e){if(suppressUntilBell||!featureEnabled){document.getElementById(ROOT)?.remove();hideProgress();return}const actual=findSurface();if(!actual&&!bellIntent()&&!panelSessionActive){hideProgress();scheduleInspect(20);return}const r=document.getElementById(ROOT);if(!r)return;const b=r.querySelector('[data-a="read"]'),d=e.detail||{};if(!b)return;if(d.network&&d.kind==='done')autoDoneForSession=true;b.classList.toggle('err',d.kind==='error');b.classList.toggle('done',d.kind==='done');const doneLabel=d.mode==='network-full'?'✓全読':d.network?'✓通信':'✓完了';b.textContent=d.scanning?'読込中':d.kind==='error'?'再読込':d.kind==='done'?doneLabel:'読込';if(d.message){b.title=String(d.message);b.setAttribute('aria-label',String(d.message))}}
