...
const ROOT='mumei-v325-dock',DOCKHOST='mumei-v325-dock-toplayer',SETTINGS='mumei-v325-filter-settings',HIDE='mumei-v325-filter-hide';
...
let dockVisible=false,inspectTimer=0,hideTimer=0,filterTimer=0,intentUntil=0,suppressUntilBell=false,settingsNavStarted=false,notificationSessionActive=notificationRoute();
...
function rows(root){if(!root?.querySelectorAll)return[];const specific=noticeRows(root);if(specific.length)return specific;const xs=[...root.querySelectorAll('li,[role="listitem"]')].filter(rowish);return xs.filter(el=>!xs.some(o=>o!==el&&o.contains(el)))}
...
function rememberedSurface(){return notificationSessionActive&&shell&&shell.isConnected&&visible(shell)?shell:null}
function findSurface(){return popupSurface()||routeSurface()||rememberedSurface()}
...
function bellHit(el){if(!(el instanceof Element)||el.closest('#'+ROOT)||el.closest('#'+SETTINGS))return false;const href=String(el.getAttribute('href')||''),meta=bellMeta(el);if(/\/notifications(?:[/?#]|$)/i.test(href))return true;const chrome=el.closest('header,nav,[role="navigation"]');if(!chrome)return false;return /(?:notification|notice|通知|お知らせ|bell)/iu.test(meta)&&!/setting|filter/i.test(meta)}
...
function openNotificationBell(){
  suppressUntilBell=false;notificationSessionActive=true;
  if(findSurface()){autoDoneForSession=false;return true}
  const el=notificationBellElement();if(!el){notificationSessionActive=false;return false}
...
function detachedUiHost(){
  let host=document.getElementById(DOCKHOST);
  if(host)return host;
  host=document.createElement('div');host.id=DOCKHOST;
  try{if('popover' in HTMLElement.prototype)host.setAttribute('popover','manual')}catch{}
  (document.body||document.documentElement).appendChild(host);
  return host
}
function openDetachedUiHost(promote=false){
  const host=detachedUiHost();
  try{
...
function closeDetachedUiHost(){
  const host=document.getElementById(DOCKHOST);if(!host)return;
  try{if(typeof host.hidePopover==='function'&&host.matches(':popover-open'))host.hidePopover()}catch{}
}
function mountUiInSurface(promote=false){const host=openDetachedUiHost(promote);if(!host)return;const r=document.getElementById(ROOT);if(r&&r.parentElement!==host)host.appendChild(r);const p=document.getElementById(SETTINGS);if(p&&p.parentElement!==host)host.appendChild(p)}
function markSurface(next){if(shell&&shell!==next)shell.removeAttribute('data-mumei-notice-shell-v3');const changed=shell!==next;shell=next;if(shell){shell.setAttribute('data-mumei-notice-shell-v3','1');mountUiInSurface(changed)}}
...
function ensureRoot(){let r=document.getElementById(ROOT);if(r){mountUiInSurface();return r}r=document.createElement('div');r.id=ROOT;r.innerHTML='<button type="button" data-a="read">読込</button><button type="button" class="mode auto" data-a="mode">自動</button><button type="button" data-a="filter">フィルター</button><button type="button" data-a="settings">設定</button><button type="button" class="ins" data-a="ins">INSIGHT</button>';(document.body||document.documentElement).appendChild(r);
...
r.addEventListener('pointerdown',e=>{primary(e)},true);
r.addEventListener('pointerup',e=>{const b=primary(e);if(b)runPrimaryDockAction(b)},true);
r.addEventListener('click',e=>{const b=primary(e);if(b)return;const other=e.target instanceof Element?e.target.closest('button[data-a]'):null;if(!other)return;e.preventDefault();e.stopPropagation();const a=other.dataset.a;if(a==='settings')void openSettings();else if(a==='ins')void openInsight()},true);
return r}
function showRoot(on){if(readerScanning()&&on!==false){notificationSessionActive=true;suppressUntilBell=false}if(editingOutsideNotification()&&!readerScanning()){hideImmediately();return}const want=Boolean((on||readerScanning())&&featureEnabled&&!suppressUntilBell);dockVisible=want;if(!featureEnabled||suppressUntilBell){if(readerScanning()){keepDockDuringScan();return}document.getElementById(ROOT)?.remove();document.getElementById(SETTINGS)?.remove();closeDetachedUiHost();return}const r=ensureRoot();mountUiInSurface(true);r.style.setProperty('display',want?'grid':'none','important');if(!want){document.getElementById(SETTINGS)?.remove();closeDetachedUiHost()}}
...
async function setFeatureEnabled(v){
  ...
  suppressUntilBell=true;document.getElementById(ROOT)?.remove();scheduleInspect(0);return true
}
...
function hideImmediately(force=false){
  if(!force&&keepDockDuringScan())return false;
  notificationSessionActive=false;
...
function onShellPointerDown(e){
  ...
  if(leavingNotificationHref(target))hideImmediately()
}
...
function readerScanning(){const api=reader();try{return Boolean(api&&typeof api.isScanning==='function'&&api.isScanning())}catch{return false}}
function keepDockDuringScan(){
  if(!readerScanning())return false;
  notificationSessionActive=true;suppressUntilBell=false;dockVisible=true;
  const r=ensureRoot();mountUiInSurface(true);r.style.setProperty('display','grid','important');
  return true
}
...
function onStatus(e){const d=e.detail||{};if(d.scanning){notificationSessionActive=true;suppressUntilBell=false;keepDockDuringScan()}if(editingOutsideNotification()&&!d.scanning){hideImmediately();hideProgress();return}if((suppressUntilBell||!featureEnabled)&&!d.scanning){document.getElementById(ROOT)?.remove();hideProgress();return}const actual=findSurface();if(!actual&&!bellIntent()&&!d.scanning){hideProgress();scheduleInspect(20);return}const r=ensureRoot();r.style.setProperty('display','grid','important');dockVisible=true;const b=r.querySelector('[data-a="read"]');if(!b)return;b.classList.toggle('err',d.kind==='error');b.textContent=d.scanning?'読込中':d.kind==='error'?'再読込':'読込';if(d.message)b.title=String(d.message)}
async function openInsight(){const id=await account(),u=new URL('https://mumei-s.github.io/note-insight/?insightMode=notifications#dashboard');if(id)u.searchParams.set('notificationAccount',id);intentUntil=0;markSurface(null);hideImmediately(true);location.assign(u.href)}
...
async function activate(next){if(!await initEnabled()||suppressUntilBell){hideImmediately();return}notificationSessionActive=true;clearTimeout(hideTimer);hideTimer=0;markSurface(next);showRoot(true);await Promise.all([initFilter(),initMode()]);if(suppressUntilBell){hideImmediately();return}scheduleFilter();if(rows(next).length){await window.__mumeiV3Checkpoint325?.restore?.();if(suppressUntilBell){hideImmediately();return}window.__mumeiV3Checkpoint325?.mark?.();maybeAuto()}}
async function activateIntent(){if(!await initEnabled()||suppressUntilBell){hideImmediately();return}notificationSessionActive=true;clearTimeout(hideTimer);hideTimer=0;showRoot(true);await Promise.all([initFilter(),initMode()]);if(suppressUntilBell)hideImmediately()}
function confirmHide(){hideTimer=0;if(suppressUntilBell){hideImmediately();return}if(notificationRoute()){notificationSessionActive=true;suppressUntilBell=false;void activateIntent();return}const next=findSurface();if(next){void activate(next);return}if(notificationSessionActive){showRoot(true);return}if(bellIntent()){void activateIntent();return}markSurface(null);showRoot(false);cleanupVisuals()}
function scheduleHide(delay=750){if(hideTimer)return;hideTimer=setTimeout(confirmHide,delay)}
async function inspect(){if(!await initEnabled()){disableFeatureNow();return}if(notificationRoute()){notificationSessionActive=true;suppressUntilBell=false}if(editingOutsideNotification()){hideImmediately();return}if(suppressUntilBell){if(shell||dockVisible||document.getElementById(ROOT))hideImmediately();return}const next=findSurface();if(next){await activate(next);return}if(notificationSessionActive){await activateIntent();return}if(bellIntent()){await activateIntent();return}if(shell||dockVisible){scheduleHide();return}cleanupVisuals()}
function scheduleInspect(ms=100){clearTimeout(inspectTimer);inspectTimer=setTimeout(()=>void inspect(),ms)}
function onTopClick(e){const target=e.target instanceof Element?e.target:null;if(!featureEnabled)return;if(!bellTrigger(target,e)){if(shell&&notificationLeaveAction(target))hideImmediately();else if(notificationSessionActive&&!notificationRoute()&&target&&!target.closest('#'+ROOT)&&shell&&!shell.contains(target))hideImmediately();return}if(notificationSessionActive&&dockVisible&&!notificationRoute()){hideImmediately();return}suppressUntilBell=false;notificationSessionActive=true;const openNow=Boolean(findSurface())||notificationRoute()||dockVisible;if(openNow){intentUntil=0;setTimeout(()=>scheduleInspect(0),100);setTimeout(()=>scheduleInspect(0),500);return}autoDoneForSession=false;intentUntil=Date.now()+7000;void activateIntent();for(const ms of[0,40,100,180,350,700,1200,2200,4000,6500])setTimeout(()=>scheduleInspect(0),ms)}
...
function boot(){ensureStyle();
  new MutationObserver(()=>{if(!featureEnabled||!dockVisible)return;const r=document.getElementById(ROOT);if(!r)return;mountUiInSurface(false);r.style.setProperty('position','fixed','important');r.style.setProperty('top','auto','important');r.style.setProperty('bottom','calc(env(safe-area-inset-bottom,0px) + 10px)','important');r.style.setProperty('left','8px','important');r.style.setProperty('right','8px','important')}).observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['style','class']});window.addEventListener('mumei-v3-reader-status',onStatus);document.addEventListener('mumei-v3-reader-status',onStatus);document.addEventListener('mumei-v3-reader-stopped',()=>{notificationSessionActive=true;suppressUntilBell=false;showRoot(true);setTimeout(()=>scheduleInspect(0),120)});window.addEventListener('focusin',onEditorFocus,true);window.addEventListener('pointerdown',onSettingsPressStart,true);window.addEventListener('pointerup',onSettingsPressEnd,true);window.addEventListener('touchstart',onSettingsPressStart,true);window.addEventListener('touchend',onSettingsPressEnd,true);window.addEventListener('mousedown',onSettingsPressStart,true);window.addEventListener('mouseup',onSettingsPressEnd,true);window.addEventListener('click',onSettingsClick,true);window.addEventListener('click',onTopClick,true);addEventListener('popstate',()=>{intentUntil=0;if(notificationRoute()){notificationSessionActive=true;suppressUntilBell=false}else hideImmediately();scheduleInspect(20);setTimeout(()=>scheduleInspect(0),320)});addEventListener('hashchange',()=>{if(notificationRoute()){notificationSessionActive=true;suppressUntilBell=false}else hideImmediately();scheduleInspect(20)});for(const name of['pushState','replaceState']){const original=history[name];if(!original.__mumei328){const wrapped=function(...a){const r=original.apply(this,a);if(notificationRoute()){notificationSessionActive=true;suppressUntilBell=false}else hideImmediately();scheduleInspect(20);setTimeout(()=>scheduleInspect(0),320);return r};wrapped.__mumei328=true;history[name]=wrapped}}void initEnabled().then(on=>{if(on){document.getElementById(ROOT)?.remove();void window.__mumeiV3Checkpoint325?.ready?.finally?.(()=>scheduleInspect(0));scheduleInspect(0)}else disableFeatureNow()});setInterval(()=>{if(featureEnabled)scheduleInspect(0)},800)}
