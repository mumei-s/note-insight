...
function installStyle(){if(document.getElementById(STYLE))return;const s=document.createElement('style');s.id=STYLE;s.textContent=`[data-mumei-v2936-front-scroll="1"]{overscroll-behavior-y:contain!important;overscroll-behavior-x:none!important;-webkit-overflow-scrolling:touch!important}`;document.documentElement.append(s)}
...
document.addEventListener('touchmove',e=>{if(!touch||!filterOn())return;const p=e.touches?.[0];if(!p)return;const {root,box,y,top}=touch;if(!root.isConnected||!box.isConnected){touch=null;return}const max=Math.max(0,box.scrollHeight-box.clientHeight),delta=y-p.clientY,next=Math.max(0,Math.min(max,top+delta));e.preventDefault();e.stopPropagation();box.scrollTop=next;if(delta>8&&max<=2)scheduleOlder(40);},{capture:true,passive:false});
...
document.addEventListener('click',e=>{const b=e.target instanceof Element?e.target.closest(`#${RAIL} .filter`):null;if(!b)return;for(const ms of [80,260])setTimeout(()=>markScroller(shell()),ms)},true);
