...
const HIDE='mumei-v2958-hide',FRAME='mumei-v2948-frame',READER='mumei-notice-reader-v2963',SHELL='[data-mumei-notice-shell-v2958="1"]',FALLBACK_SENTINEL='data-mumei-fallback-disabled-v2970';
...
function notificationOpen(){if(/^\/notifications(?:\/|$)/.test(location.pathname.toLowerCase()))return true;const marked=document.querySelector(SHELL);return Boolean(marked&&shown(marked)&&tabLike(marked))}
function disableLegacyFallback(){let x=document.getElementById(READER);if(x?.hasAttribute?.(FALLBACK_SENTINEL))return x;if(x)x.remove();if(!document.documentElement)return null;x=document.createElement('span');x.id=READER;x.setAttribute(FALLBACK_SENTINEL,'1');x.setAttribute('aria-hidden','true');x.hidden=true;x.style.setProperty('display','none','important');x.style.setProperty('pointer-events','none','important');document.documentElement.appendChild(x);return x}
function guardPrimary(target=document.activeElement){disableLegacyFallback();const p=document.getElementById(FRAME);if(p&&(!notificationOpen()||editableTarget(target)))p.style.setProperty('display','none','important')}
...
function repair(root=document){
  disableLegacyFallback();guardPrimary();
...
document.addEventListener('focusin',e=>{guardPrimary(e.target);schedule(0)},true);
document.addEventListener('focusout',()=>schedule(0),true);
document.addEventListener('click',e=>{guardPrimary(e.target);schedule(0)},true);
