...
function dockInfo(f){try{const title=clean(f.title||''),d=f.contentDocument,body=clean(d?.body?.textContent||''),read=d?.getElementById('read'),settings=d?.getElementById('settings'),ins=d?.getElementById('ins'),filter=d?.getElementById('filter');return{isDock:(/INSIGHT.*通知/u.test(title)||/INSIGHT【通知】/u.test(body))&&(/前回の続きから読込/u.test(body)||Boolean(read)),legacyThree:Boolean((read||/前回の続きから読込/u.test(body))&&(settings||/設定/u.test(body))&&(ins||/INSIGHT【通知】/u.test(body))&&!filter&&!/フィルター(?:ON|OFF|設定)/u.test(body))}}catch{return{isDock:false,legacyThree:false}}}
function purgeLegacyThreeButton(){let removed=0;for(const f of [...document.querySelectorAll('iframe')]){const info=dockInfo(f);if(info.legacyThree){try{f.remove();removed++}catch{}}}return removed}
function looksLikeInsightDock(f){return dockInfo(f).isDock}
...
function guardNotificationDock(forceTyping=false){purgeLegacyThreeButton();const block=forceTyping||activeEditing()||!strictNoticeContext();for(const el of dockNodes()){if(block)hideDock(el);else releaseDock(el)}return block}
function repair(root=document){
  const removed=purgeLegacyThreeButton(),blocked=guardNotificationDock(false);
  ...
  if((restored||removed)&&health&&!blocked){health.textContent=removed?`旧3ボタン通知パネル ${removed}件を削除`:`フィルター安全補正 ✓ 親コンテナ ${restored}件を復元`;health.className='health done'}
}
...
function bind(){purgeLegacyThreeButton();repair(document);mo.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['class','style','hidden','aria-hidden','inert']})}
