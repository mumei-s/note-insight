...
const HIDE='mumei-v2958-hide',FRAME='mumei-v2948-frame',READER='mumei-notice-reader-v2963',SHELL='[data-mumei-notice-shell-v2958="1"]',SENTINEL='data-mumei-notice-sentinel-v2969';
...
function sentinel(){const x=document.getElementById(FRAME);return x?.hasAttribute?.(SENTINEL)?x:null}
function installSentinel(){if(document.getElementById(FRAME)||!document.body)return;const s=document.createElement('span');s.id=FRAME;s.setAttribute(SENTINEL,'1');s.setAttribute('aria-hidden','true');s.hidden=true;s.style.setProperty('display','none','important');s.style.setProperty('pointer-events','none','important');document.body.appendChild(s)}
function removeSentinel(){sentinel()?.remove()}
function guardReaderFallback(target=document.activeElement){const typing=editableTarget(target),allowed=readerAllowed(),blocked=typing||!allowed;const fallback=document.getElementById(READER);if(fallback&&blocked)fallback.remove();if(blocked){const primary=document.getElementById(FRAME);if(primary&&!primary.hasAttribute(SENTINEL))primary.style.setProperty('display','none','important');else if(!primary)installSentinel()}else removeSentinel()}
...
function repair(root=document){
  ...
  const f=document.getElementById(FRAME),health=!f?.hasAttribute?.(SENTINEL)?f?.contentDocument?.getElementById('health'):null;
...
document.addEventListener('focusout',()=>schedule(0),true);
document.addEventListener('click',e=>{if(editableTarget(e.target))guardReaderFallback(e.target);else schedule(0)},true);
window.addEventListener('pageshow',()=>schedule(0));window.addEventListener('focus',()=>schedule(0));window.addEventListener('pagehide',()=>{document.getElementById(READER)?.remove();removeSentinel()});window.addEventListener('popstate',()=>schedule(0));window.addEventListener('hashchange',()=>schedule(0));document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden'){document.getElementById(READER)?.remove();removeSentinel()}else schedule(0)});
})();