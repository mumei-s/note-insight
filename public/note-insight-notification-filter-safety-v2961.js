...
const HIDE='mumei-v2958-hide',FRAME='mumei-v2948-frame',LEGACY='mumei-notice-reader-v2963',SHELL='[data-mumei-notice-shell-v2958="1"]',SHIELD='data-mumei-input-shield-hidden';
...
function dockNodes(){const out=[...document.querySelectorAll(`#${FRAME},#${LEGACY}`)];for(const f of document.querySelectorAll('iframe')){if(out.includes(f))continue;let legacy=false;try{const t=clean(f.title||'');const d=f.contentDocument,body=clean(d?.body?.textContent||'');legacy=/INSIGHT.*通知/u.test(t)&&/前回の続きから読込/u.test(body)&&/INSIGHT【通知】/u.test(body)}catch{}if(legacy)out.push(f)}return out}
...
const mo=new MutationObserver(muts=>{for(const m of muts){if(m.type==='attributes'){const t=m.target;if(t instanceof HTMLElement&&(t.id===FRAME||t.id===LEGACY||t.getAttribute(SHIELD)==='1'||t.classList.contains(HIDE))){schedule(0);return}}if(m.type==='childList'){for(const x of m.addedNodes){if(!(x instanceof Element))continue;if(x.id===FRAME||x.id===LEGACY||x.querySelector?.(`#${FRAME},#${LEGACY}`)){schedule(0);return}}schedule();return}}});
