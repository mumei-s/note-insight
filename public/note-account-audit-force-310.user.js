...
// @downloadURL https://raw.githubusercontent.com/mumei-s/note-insight/main/public/note-account-audit-force-310.user.js
// @grant none
...
async function controlProfiles(ids,set){
 var cache={};try{cache=JSON.parse(localStorage.getItem(CONTROL_KEY)||'{}')||{}}catch(e){}
 var uniq=Array.from(new Set((ids||[]).filter(Boolean))),now=Date.now();
 try{
  var db=await idbOpen(),rows=await idbAll(db,'people');
  rows.forEach(function(x){if(x&&x.id&&x.profile&&uniq.indexOf(x.id)>=0)cache[x.id]={ok:true,id:x.id,profile:x.profile,cachedAt:now}})
 }catch(e){}
 var need=uniq.filter(function(id){var x=cache[id];return !x||!x.profile||!x.cachedAt||now-x.cachedAt>43200000});
 var chunks=[];for(var i=0;i<need.length;i+=50)chunks.push(need.slice(i,i+50));var next=0,done=0;
 async function worker(){
  while(true){
   var k=next++;if(k>=chunks.length)return;var ch=chunks[k];
   try{
    var j=await auditServer({action:'profile_batch',ids:ch}),rs=Array.isArray(j.results)?j.results:[];
    rs.forEach(function(x){if(x&&x.id&&x.profile)cache[x.id]=Object.assign({},x,{cachedAt:Date.now()})})
   }catch(e){}
   done+=ch.length;set('🧪 対照群プロフィール '+Math.min(done,need.length)+'/'+need.length)
  }
 }
 await Promise.all([worker(),worker()]);
 try{localStorage.setItem(CONTROL_KEY,JSON.stringify(cache))}catch(e){}
 return new Map(uniq.map(function(id){return[id,cache[id]&&cache[id].profile||null]}))
}
function statsFromProfiles(ids,pmap){
 var ps=(ids||[]).map(function(id){return pmap.get(id)}).filter(Boolean),n0=ps.length;
 return{
  total:(ids||[]).length,known:n0,
  mass:ps.filter(function(x){return x.mass}).length,
  lowNotes:ps.filter(function(x){return n(x.notes)<=1}).length,
...
function cohortRate(creators,pmap,pred){
 var all=creators.filter(function(x){var p=pmap.get(x.id);return p&&pred(p)}),f=all.filter(function(x){return x.follower});
 return{total:all.length,followers:f.length,rate:pct1(f.length,all.length)}
}
async function controlCohort(lg,followers,set){
 var creators=Array.from(lg.creatorMap&&lg.creatorMap.values?lg.creatorMap.values():[]),
     followerIds=creators.filter(function(x){return x.follower}).map(function(x){return x.id}),
     nonFollowerIds=creators.filter(function(x){return !x.follower}).map(function(x){return x.id}),
     allIds=creators.map(function(x){return x.id}),
     pmap=await controlProfiles(allIds,set),
     followerStats=statsFromProfiles(followerIds,pmap),nonStats=statsFromProfiles(nonFollowerIds,pmap),
     one=creators.filter(function(x){return x.count===1}),multi=creators.filter(function(x){return x.count>=2}),
     oneF=one.filter(function(x){return x.follower}).length,multiF=multi.filter(function(x){return x.follower}).length,
     noLike=(followers||[]).filter(function(x){return !(lg.creatorMap&&lg.creatorMap.has&&lg.creatorMap.has(x.id))}),
     groups={
      mass:cohortRate(creators,pmap,function(p){return p.mass}),
      nonMass:cohortRate(creators,pmap,function(p){return !p.mass}),
      follow1000:cohortRate(creators,pmap,function(p){return n(p.followings)>=1000}),
      followLt1000:cohortRate(creators,pmap,function(p){return n(p.followings)<1000}),
      lowNotes:cohortRate(creators,pmap,function(p){return n(p.notes)<=1}),
      notes2plus:cohortRate(creators,pmap,function(p){return n(p.notes)>=2}),
      ratio10:cohortRate(creators,pmap,function(p){return n(p.followers)>0&&n(p.followings)/n(p.followers)>=10}),
      ratioLt10:cohortRate(creators,pmap,function(p){return !(n(p.followers)>0&&n(p.followings)/n(p.followers)>=10)})
     };
 return{
  totalLikedCreators:creators.length,converted:followerIds.length,notConverted:nonFollowerIds.length,conversionRate:pct1(followerIds.length,creators.length),
  oneTouch:{total:one.length,followers:oneF,rate:pct1(oneF,one.length)},
  multiTouch:{total:multi.length,followers:multiF,rate:pct1(multiF,multi.length)},
  followerStats:followerStats,nonStats:nonStats,groups:groups,
  noVisibleLikeFollowers:noLike.length,noVisibleLikeIds:noLike.map(function(x){return x.id}).slice(0,50)
 }
}
...
function render(r){
 ...
 '<div class="naa-conclusion">'+
   ...
   '<em>'+(act.complete
      ...
      :'活動日データの取得中')+'</em>'+
   '<div class="naa-conclusion-sub">スキ先→フォロー化 '+ctl.conversionRate+'%｜1回だけスキ→フォロー '+ctl.oneTouch.rate+'%｜2回以上→ '+ctl.multiTouch.rate+'%｜一方向 '+lg.followerOneWayCreators+'人｜販売型 '+market.score+'/100</div>'+
 '</div>'+
 '<div class="naa-hyp">'+
   '<b>⏸ 停止アカウント仮説</b>'+
 ...
 '</div>'+
 '<div class="naa-tabs">'+
 '<button data-tab="answer" class="on">答え</button><button data-tab="likes">スキ</button><button data-tab="activity">停止</button><button data-tab="deep">深掘り</button><button data-tab="control">対照</button><button data-tab="net">関係網</button><button data-tab="market">購入</button><button data-tab="ext">外部</button>'+
 '</div>'+
 ...
 '</section>'+'<section class="naa-pane" data-pane="control">'+
 '<h3>🧪 同じ「まこからスキされた人」で比較</h3>'+
 '<div class="naa-note">母集団はまこがスキした '+ctl.totalLikedCreators+'人。現在フォローあり '+ctl.converted+'人、なし '+ctl.notConverted+'人。両群とも「まこからスキされた」条件を揃えた内部対照群。</div>'+
 '<div class="naa-cards">'+
 card('フォロー化率',ctl.conversionRate+'%',ctl.converted+'/'+ctl.totalLikedCreators+'人')+
 card('1回だけスキ',ctl.oneTouch.rate+'%',ctl.oneTouch.followers+'/'+ctl.oneTouch.total+'人がフォロー')+
 card('2回以上スキ',ctl.multiTouch.rate+'%',ctl.multiTouch.followers+'/'+ctl.multiTouch.total+'人がフォロー')+
 card('現在フォロワーだがスキ痕跡なし',ctl.noVisibleLikeFollowers+'人','現在確認できる公開スキ履歴では未一致')+
 '</div>'+
 '<h4>フォローした側 vs しなかった側</h4>'+
 '<div class="naa-compare"><div><b>フォローあり '+ctl.followerStats.known+'人</b><small>大量フォロー型 '+pct1(ctl.followerStats.mass,ctl.followerStats.known)+'%<br>投稿0〜1 '+pct1(ctl.followerStats.lowNotes,ctl.followerStats.known)+'%<br>フォロー1000+ '+pct1(ctl.followerStats.follow1000,ctl.followerStats.known)+'%<br>比率10倍+ '+pct1(ctl.followerStats.ratio10,ctl.followerStats.known)+'%<br>中央値 フォロー '+ctl.followerStats.medFollowings+' / フォロワー '+ctl.followerStats.medFollowers+'</small></div><div><b>フォローなし '+ctl.nonStats.known+'人</b><small>大量フォロー型 '+pct1(ctl.nonStats.mass,ctl.nonStats.known)+'%<br>投稿0〜1 '+pct1(ctl.nonStats.lowNotes,ctl.nonStats.known)+'%<br>フォロー1000+ '+pct1(ctl.nonStats.follow1000,ctl.nonStats.known)+'%<br>比率10倍+ '+pct1(ctl.nonStats.ratio10,ctl.nonStats.known)+'%<br>中央値 フォロー '+ctl.nonStats.medFollowings+' / フォロワー '+ctl.nonStats.medFollowers+'</small></div></div>'+
 '<h4>どんな人がフォロー化しやすいか</h4>'+
 '<div class="naa-cards">'+
 card('大量フォロー型',ctl.groups.mass&&ctl.groups.mass.rate+'%',ctl.groups.mass?(ctl.groups.mass.followers+'/'+ctl.groups.mass.total+'人'):'')+
 card('非・大量フォロー型',ctl.groups.nonMass&&ctl.groups.nonMass.rate+'%',ctl.groups.nonMass?(ctl.groups.nonMass.followers+'/'+ctl.groups.nonMass.total+'人'):'')+
 card('フォロー1000+',ctl.groups.follow1000&&ctl.groups.follow1000.rate+'%',ctl.groups.follow1000?(ctl.groups.follow1000.followers+'/'+ctl.groups.follow1000.total+'人'):'')+
 card('フォロー1000未満',ctl.groups.followLt1000&&ctl.groups.followLt1000.rate+'%',ctl.groups.followLt1000?(ctl.groups.followLt1000.followers+'/'+ctl.groups.followLt1000.total+'人'):'')+
 card('投稿0〜1',ctl.groups.lowNotes&&ctl.groups.lowNotes.rate+'%',ctl.groups.lowNotes?(ctl.groups.lowNotes.followers+'/'+ctl.groups.lowNotes.total+'人'):'')+
 card('投稿2+',ctl.groups.notes2plus&&ctl.groups.notes2plus.rate+'%',ctl.groups.notes2plus?(ctl.groups.notes2plus.followers+'/'+ctl.groups.notes2plus.total+'人'):'')+
 '</div>'+
 '<details><summary>現在フォロワーだが公開スキ痕跡なし（'+ctl.noVisibleLikeFollowers+'人）</summary><div class="naa-note">'+(ctl.noVisibleLikeIds&&ctl.noVisibleLikeIds.length?ctl.noVisibleLikeIds.map(function(x){return'@'+esc(x)}).join(' / '):'該当なし')+'</div></details>'+
