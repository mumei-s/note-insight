...
function makeLiveDataset(manifest,meta){
  ...
  return {version:'18.9.20',datasetId,count:rows.length,rows,preparedBatch:false,liveBatch:true,
  ...
  try{
    ...
    if(frozenResume){
      ...
      if(!stagedData||stagedData.datasetId!==dataset.datasetId||!run||run.datasetId!==dataset.datasetId){
        stagedData=dataset;
        run={version:'18.9.20',articleKey:articleKey(),datasetId:dataset.datasetId,stage:'images_building',images:{},pendingImage:null,cardKeys:[],savedCardCount:0};
...
async function runWaitResumeCycle(){
  // v18.9.20: automatic recovery retries are intentionally disabled.
...
function adoptCardsThroughCheckpoint(view,dataset,run){
  ...
  const adopted=[];
  // Adopt every target card that is actually visible in the current editor.
  // This includes cards created after るるちゃん just before the latest 403,
  // even when the local run record did not get updated yet.
  for(let i=0;i<dataset.rows.length;i++){
    const row=dataset.rows[i];
  ...
  // Rows 1..166 are user-confirmed complete and are never regenerated even if
  // note's current DOM parser fails to recognize one of those historical cards.
  const adoptedUrls=new Set(adopted.map(x=>normalize(x.url)));
  let resumeIndex=checkpoint+1;
  while(resumeIndex<dataset.rows.length&&adoptedUrls.has(normalize(dataset.rows[resumeIndex].url)))resumeIndex++;
  run.cardKeys=adopted;
  run.savedCardCount=Math.min(Number(run.savedCardCount||0),adopted.length);
  run.pendingCard=null;
  run.cardCheckpoint={url:CARD_CHECKPOINT_URL,index:checkpoint+1,label:CARD_CHECKPOINT_LABEL,adoptedAt:Date.now(),resumeIndex:resumeIndex+1};
  write(runKey(),run);
  return {checkpoint:checkpoint+1,count:adopted.length,resumeIndex};
}
async function buildCards(){
  ...
  try{
    ...
    if(hold){
      const waitMs=Math.max(0,Number(hold.until||0)-Date.now());
      if(waitMs>0){
        setStatus('note通信制限中｜あと約'+Math.ceil(waitMs/1000)+'秒待ってから「カード続き」を押してください。現在のカードは保持',true);
        try{safety().capture?.()}catch(_){}
        return;
      }
      try{safety().resumeNetwork()}catch(error){
        setStatus('通信停止解除待ち：'+(error?.message||String(error)),true);
        return;
      }
    }
    ...
    setStatus('カード専用再開 ✅ 現在見えているカード '+adopted.count+'件を採用｜'+(adopted.resumeIndex+1)+'番から残りだけ');
    let sessionCreated=0;

    for(let i=adopted.resumeIndex;i<dataset.rows.length;i++){
      const row=dataset.rows[i];
      ...
      try{
        ...
        if(recovered){
          ...
          sessionCreated++;
          await sleep(3000);
          if(sessionCreated%10===0){
            setStatus('カード '+run.cardKeys.length+'/'+dataset.count+'｜403回避の30秒休止');
            await sleep(30000);
          }
          continue;
        }
        ...
        try{safety().capture?.()}catch(_){}
        setStatus('カード '+run.cardKeys.length+'/'+dataset.count+'｜'+row.creator+' で停止 '+(code?'HTTP '+code:'通信エラー')+'。現在の本文をローカル控えに保存済み。通信制限が解けてから「カード続き」で再開',true);
      ...
      try{safety().capture?.()}catch(_){}
      sessionCreated++;
      setStatus('通知カード '+run.cardKeys.length+'/'+dataset.count+' ✅｜カードだけ安全速度で続行');
      await sleep(3000);
      if(sessionCreated%10===0&&i<dataset.rows.length-1){
        setStatus('カード '+run.cardKeys.length+'/'+dataset.count+'｜403回避の30秒休止（本文は保持）');
        await sleep(30000);
      }
    }
    await sleep(2500);
...
async function resumeAfterBennett(){
  ...
  try{
    ...
    const run={version:'18.9.20',articleKey:articleKey(),datasetId:dataset.datasetId,stage:'images_building',images:{},pendingImage:null,cardKeys:[],savedCardCount:0,
...
function mount(){
  ...
  if(!p){
    ...
    p.innerHTML='<div class="title" style="display:flex;align-items:center;gap:6px;font-weight:900;margin-bottom:6px;cursor:grab;user-select:none"><span style="flex:1">極薄＋通知 Fresh <span style="font-size:10px">v'+VERSION+'</span></span><button data-a="min" type="button" style="width:32px;min-height:28px;padding:2px 6px">−</button></div>'+
      ...
      '<button data-a="cards" type="button" style="grid-column:1 / -1">カード続き（現在位置から）</button><button data-a="delete" type="button" style="grid-column:1 / -1">投稿後カード一括削除</button></div>'+
      '<div data-progress style="margin-top:5px;font-size:10px;color:#9fdcff">極薄 0/0｜カード 0/0</div>'+
      '<div id="'+STATUS+'" style="margin-top:4px;font-size:10px">極薄は完了済み｜本文に見えているカードを採用 → その次だけ</div></div>';
