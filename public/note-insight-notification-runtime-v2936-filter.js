...
function leadId(el,lead,st){const links=creatorLinks(el);if(!links.length)return'';if(lead){const textHit=links.find(x=>x.txt&&nameMatches(lead,x.txt));if(textHit)return textHit.id;const profHit=st.profiles.find(p=>p.name&&nameMatches(lead,p.name));if(profHit)return profHit.id}
const first=links[0];if(!first)return'';const p=st.profiles.find(x=>x.id===first.id);if(!lead)return first.id;if(first.txt&&nameMatches(lead,first.txt))return first.id;if(p?.name&&nameMatches(lead,p.name))return first.id;return''}
...
async function reconcile(){if(busy)return;busy=true;try{const root=shell();if(!root)return;const st=await state();if(!st)return;for(const e of root.querySelectorAll(`.${MUTED}`))e.classList.remove(MUTED);if(!st.enabled||!st.ids.size)return;for(const el of logicalRows(root)){const t=clean(el.textContent);if(!magazineNoise(t))continue;const lead=leadTextName(t);if(!lead)continue;let hit=st.profiles.some(p=>p.name&&nameMatches(lead,p.name));if(!hit){const id=leadId(el,lead,st);hit=Boolean(id&&st.ids.has(id))}if(!hit)continue;const row=outerRow(el,root);row.classList.add(MUTED);if(row!==el)el.classList.add(MUTED)}}finally{busy=false}}
function schedule(ms=35){clearTimeout(timer);timer=setTimeout(()=>void reconcile(),ms)}
const obs=new MutationObserver(ms=>{if(ms.some(m=>m.type==='childList'||(m.type==='attributes'&&m.attributeName==='class')))schedule(20)});obs.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['class']});
document.addEventListener('click',e=>{const b=e.target instanceof Element?e.target.closest(`#${RAIL} .filter`):null;schedule(b?80:35)},true);
document.addEventListener('scroll',()=>schedule(30),true);addEventListener('focus',()=>schedule(50));addEventListener('pageshow',()=>schedule(50));document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')schedule(50)});
schedule(120);
})();
