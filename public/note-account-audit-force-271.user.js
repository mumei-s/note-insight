...
// @downloadURL https://raw.githubusercontent.com/mumei-s/note-insight/main/public/note-account-audit-force-271.user.js
// @grant none
// @run-at document-start
// ==/UserScript==
(function(){
'use strict';window.__noteAccountAuditV1=1;window.__noteAccountAuditCanonical='2.7.1';
...
var LIKED_KEY='note-account-audit:liked:v27:koakumako';
async function targetLikedArticles(id,set){
 set('☁ まこの公開スキをGraphQLで全件取得中…');
 try{
  var j=await auditServer({action:'liked_all',id:id}),items=Array.isArray(j.items)?j.items:[];
  var map={};
  items.forEach(function(x,i){
   if(!x||!x.creator)return;
   var k=c(x.key)||('gql-'+i+'-'+x.creator+'-'+c(x.title));
   map[k]={key:k,creator:c(x.creator).toLowerCase(),creatorName:c(x.creatorName||x.creator),title:c(x.title),publishedAt:x.publishedAt||null,likeCount:n(x.likeCount),commentCount:n(x.commentCount)};
  });
  var state={items:map,pages:n(j.pages),at:Date.now(),complete:j.complete===true,source:c(j.source||'note-graphql')};
  localStorage.setItem(LIKED_KEY,JSON.stringify(state));
  NAA_AUTO.lastLikeDiag={ok:true,count:Object.keys(map).length,pages:state.pages,source:state.source};var box=document.getElementById('naa-diag');if(box){box.style.display='block';box.innerHTML='✅ GraphQL全件取得<br>'+Object.keys(map).length+'件 / '+state.pages+'ページ'};set('❤️ まこの公開スキ '+Object.keys(map).length+'件｜GraphQL全件');
  return{items:Object.values(map),pages:state.pages,at:state.at,complete:state.complete,source:state.source};
 }catch(serverErr){
  var em=String(serverErr&&serverErr.message||serverErr);NAA_AUTO.lastLikeDiag={ok:false,error:em};set('❌ GraphQL失敗｜'+em);var box=document.getElementById('naa-diag');if(box){box.style.display='block';box.innerHTML='❌ GraphQL全件取得NG<br>'+esc(em)+'<br>現在の数字はHTML参考値'};
  var state={items:{},pages:0,at:0,complete:false,source:'html-partial'},noNew=0;
  for(var pg=1;pg<=50;pg++){
   set('まこの公開スキ参考値 '+Object.keys(state.items).length+'件｜p'+pg);
   try{
    ...
    root.querySelectorAll('a[href]').forEach(function(el){
     ...
     if(x&&x.creator&&x.creator!==id)state.items[x.key]={key:x.key,creator:x.creator,url:x.url,title:c(el.textContent)||x.key};
    });
    var re=/https?:\/\/note\.com\/([A-Za-z0-9_-]+)\/n\/(n[A-Za-z0-9]+)/g,m;
    while((m=re.exec(norm))){var cr=c(m[1]).toLowerCase(),k=c(m[2]);if(cr&&k&&cr!==id)state.items[k]=state.items[k]||{key:k,creator:cr,url:'https://note.com/'+cr+'/n/'+k,title:k}}
  ...
  return{items:Object.values(state.items),pages:state.pages,at:state.at,complete:false,source:'html-partial',error:String(serverErr&&serverErr.message||serverErr)};
 }
}
...
function render(r){
 ...
 '<div class="naa-conclusion">'+
   '<small>結論</small>'+
   '<b>'+(lead?'@'+esc(lead.id):'本人アカウントは未特定')+'</b>'+
   '<span>'+esc(leadStatus)+'</span>'+
   '<em>'+esc(lead?lead.reasons.join(' / '):'本人自身が公開した別アカウント情報は確認できていません')+'</em>'+
   '<div class="naa-conclusion-sub">1000人走査完了｜まこ→スキ '+lg.articles+'件｜フォロワーへのスキ '+lg.followerArticles+'件｜相互 '+lg.mutualCreators+'人｜'+esc(marketText)+' ('+market.score+'/100)</div>'+
   (lead?'<a target="_blank" href="'+esc(lead.urls[0]||('https://note.com/'+lead.id))+'">この根拠を確認</a>':'')+
 '</div>'+
 '<div class="naa-cards">'+
 card('公開根拠候補',pc.length+'件',lead?('@'+lead.id+'｜'+leadNote):'明示根拠なし')+
 card('まこ→スキ',(lg.complete?'':'>=')+lg.articles+'件',(lg.complete?'GraphQL全件｜':'参考値｜')+lg.creators+'人へ')+
 ...
 '<h3>❤️ まこの公開スキ行動</h3>'+
 '<div class="naa-mini">'+(lg.complete?'公開スキ全件':'公開スキ参考値')+' <b>'+lg.articles+'</b>件 ／ 相手 <b>'+lg.creators+'</b>人 ／ フォロワー相手 <b>'+lg.followerCreators+'</b>人 ／ 相互スキ <b>'+lg.mutualCreators+'</b>人<br><small>取得: '+esc(lg.source||'不明')+'</small></div>'+
 '<h4>スキ先 上位</h4>'+
