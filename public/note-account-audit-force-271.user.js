 ...
 for(var attempt=1;attempt<=3;attempt++){
  try{
   ...
   items.forEach(function(x,i){
    if(!x)return;
    var k=c(x.key)||('gql-'+i);
    map[k]={key:k,creator:c(x.creator).toLowerCase()};
   });
...
function analyzeLikeGraph(outgoing,followers,candidates){
 ...
 outgoing.forEach(function(x){if(!x.creator)return;var q=cmap.get(x.creator)||{id:x.creator,count:0,follower:fs.has(x.creator),incoming:incoming.get(x.creator)||0};q.count++;cmap.set(x.creator,q)});
 var creators=Array.from(cmap.values()).sort(function(a,b){return b.count-a.count}),fArts=outgoing.filter(function(x){return x.creator&&fs.has(x.creator)}).length,fCreators=creators.filter(function(x){return x.follower}),mutual=creators.filter(function(x){return x.incoming>0});
