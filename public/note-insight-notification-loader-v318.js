...
if(window.__mumeiNotificationLoader320)return;window.__mumeiNotificationLoader320=true;
const VERSION='3.2.0';
...
function dashboardRoute(){return !IS_NOTE||/^\/(?:sitesettings\/stats|dashboard)(?:\/|$)/.test(location.pathname)}
...
async function componentText(name){const url=BASE+name+'?v=320&ts='+Date.now();let last=null;for(let i=0;i<2;i++){try{const t=await gmRequest(url);if(t.trim()){await setValue(CACHE+name,t);return t}}catch(e){last=e}try{const t=await fetchText(url);if(t.trim()){await setValue(CACHE+name,t);return t}}catch(e){last=e}await sleep(180+i*220)}const cached=String(await getValue(CACHE+name,'')||'');if(cached.trim())return cached;throw last||new Error('EMPTY_'+name)}
function transform(name,code){let s=String(code||'');if(name.includes('runtime-v2958')){s=s.replaceAll('__mumeiNotificationRuntime2958','__mumeiNotificationRuntimeV320').replaceAll('mumei-v2948-frame',V3_FRAME).replaceAll('mumei-notice-shell-v2958','mumei-notice-shell-v3');s=s.replace(/const VERSION='[^']*'/,"const VERSION='3.2.0'")}else if(name.includes('autoscan-v2970')){s=s.replaceAll('__mumeiNotificationAutoscan2970','__mumeiNotificationAutoscanV320').replaceAll('mumei-v2948-frame',V3_FRAME).replaceAll('data-mumei-notice-shell-v2958','data-mumei-notice-shell-v3');s=s.replace(/const VERSION='[^']*',PROTOCOL='[^']*'/,"const VERSION='3.2.0',PROTOCOL='3.2.0'")}else if(name.includes('bootstrap-v2966')){s=s.replaceAll('mumei-v2948-frame',V3_FRAME);s=s.replace(/const VERSION='[^']*'/,"const VERSION='3.2.0'");if(versionCheckRequested)s=s.replace('if(handleVersionCheck())return;','')}else if(name.includes('filter-restore-v2962')){s=s.replaceAll('__mumeiNotificationFilterRestore2962','__mumeiNotificationFilterRestoreV320').replaceAll('mumei-v2948-frame',V3_FRAME)}else if(name.includes('filter-safety-v2961')){s=s.replaceAll('__mumeiNotificationFilterSafety2961','__mumeiNotificationFilterSafetyV320').replaceAll('mumei-v2948-frame',V3_FRAME).replaceAll('data-mumei-notice-shell-v2958','data-mumei-notice-shell-v3')}return s}
...
async function ensureDashboard(){if(!dashboardRoute()||dashboardLoaded)return;try{await loadComponent('note-insight-dashboard-integrated-v318.js');dashboardLoaded=true;try{localStorage.setItem('mumei-dashboard-integrated-runtime','3.2.0')}catch{}}catch(e){console.warn('[INSIGHT] ダッシュボード接続失敗',e)}}
...
function installRouteWatch(){for(const name of['pushState','replaceState']){const orig=history[name];if(typeof orig!=='function'||orig.__mumeiLoader320Wrapped)continue;const fn=function(){const out=orig.apply(this,arguments);queueMicrotask(routeChanged);return out};fn.__mumeiLoader320Wrapped=true;history[name]=fn}addEventListener('popstate',routeChanged);addEventListener('hashchange',routeChanged)}
installRouteWatch();
