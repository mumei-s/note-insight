...
// @version      3.3.0
// @description  成功済みProseMirror DIRECT URL設定版。進捗・再開＋note公式通知カード作成/記録/一括削除
...
(function(){
'use strict';
if(window.__MUMEI_DIRECT_SUCCESS_330__) return;
window.__MUMEI_DIRECT_SUCCESS_330__=true;
...
function saveRegistry(a){const out=[];const seen=new Set();for(const x of a){if(!x?.url||!x?.type)continue;const k=x.url+'|'+x.type;if(seen.has(k))continue;seen.add(k);out.push({url:x.url,type:x.type})}localStorage.setItem(REGISTRY_KEY,JSON.stringify(out));return out}
function rememberNotify(url,type){const a=getRegistry();a.push({url,type});return saveRegistry(a)}
function migrateLegacy(){let old=null;try{old=JSON.parse(localStorage.getItem(TEST_KEY)||'null')}catch(_){}if(old?.url&&old?.type)rememberNotify(old.url,old.type)}
...
function findView(){
  ...
  while(q.length&&steps<12000){
    ...
    for(const k of keys){
      ...
      if(d<7&&x&&(typeof x==='object'||typeof x==='function')){if(x===window||x===document)continue;push(x,d+1)}
    }
  }
  return null;
}
...
function trySetAttr(view,pos,node,url){const keys=attrKeys(node);if(!keys.length)return false;for(const key of keys){try{const attrs={...node.attrs,[key]:url};view.dispatch(view.state.tr.setNodeMarkup(pos,node.type,attrs,node.marks));const fresh=view.state.doc.nodeAt(pos);if(nodeHasLink(fresh,url))return true}catch(_){}}return false}
function trySetMark(view,pos,node,url){const type=linkMarkType(view.state.schema);if(!type)return false;let mark;try{mark=type.create(buildLinkAttrs(type,url))}catch(_){return false}try{const marks=(node.marks||[]).filter(m=>m.type!==type).concat(mark);view.dispatch(view.state.tr.setNodeMarkup(pos,node.type,node.attrs,marks));const fresh=view.state.doc.nodeAt(pos);if(nodeHasLink(fresh,url))return true}catch(_){}try{view.dispatch(view.state.tr.addMark(pos,pos+node.nodeSize,mark));const fresh=view.state.doc.nodeAt(pos);if(nodeHasLink(fresh,url))return true}catch(_){}return false}
...
function nodeList(view,img){const hit=findNodeForImg(view,img);if(!hit)return[];return[hit,...parentCandidates(view,hit.pos)]}
function alreadyLinked(view,img,url){return nodeList(view,img).some(c=>nodeHasLink(c.node,url))}
function setDirect(view,img,url){const list=nodeList(view,img);if(!list.length)return{ok:false,reason:'node'};const hit=list[0];for(const c of list){if(trySetAttr(view,c.pos,c.node,url))return{ok:true,mode:'attr',type:c.node.type.name}}for(const c of list){if(trySetMark(view,c.pos,c.node,url))return{ok:true,mode:'mark',type:c.node.type.name}}return{ok:false,reason:'schema',type:hit.node.type?.name||'?',attrs:Object.keys(hit.node.attrs||{}),marks:Object.keys(view.state.schema.marks||{})}}
...
async function run(){
  ...
  try{
    ...
    status('EditorViewを探しています…');const view=findView();if(!view){status('DIRECT停止：EditorViewを取得できません',true);emit('mumei-direct-stopped',{index:0,ok:0,reason:'view'});return}
    let ok=0;
    for(let i=0;i<10;i++){
      const index=i+1;if(alreadyLinked(view,imgs[i],URLS[i])){ok++;status(`URL書き込み ${index}/10（設定済み）`);emit('mumei-direct-progress',{index,ok,skipped:true});continue}
      status(`URL書き込み ${index}/10…`);emit('mumei-direct-progress',{index,ok,skipped:false});let r={ok:false,reason:'unknown'};
...
function findEmbed(view,url,typeName=null){let hit=null;view.state.doc.descendants((node,pos)=>{if(hit)return false;if(typeName&&node.type?.name!==typeName)return;if(nodeCarriesUrl(node,url)&&!node.isTextblock&&!/image|picture|photo/i.test(node.type?.name||'')){hit={node,pos};return false}});return hit}
function setSelectionAtEnd(view){try{const Sel=view.state.selection.constructor;view.dispatch(view.state.tr.setSelection(Sel.atEnd(view.state.doc)));return true}catch(_){return false}}
async function waitEmbed(view,url,ms=8000){const end=Date.now()+ms;while(Date.now()<end){const hit=findEmbed(view,url);if(hit)return hit;await sleep(250)}return null}
async function notifyTest(){
  ...
  try{
    ...
    const old=findEmbed(view,TEST_URL);if(old){nstatus('同じ記事の既存埋め込みがあります。削除対象には登録しません',true);return}
    const p=view.state.schema.nodes.paragraph;if(!p){nstatus('通知テスト停止：paragraphなし',true);return}
    nstatus('通知カード 1/1 作成中…');const start=view.state.doc.content.size;
    view.dispatch(view.state.tr.insert(start,p.create(null,view.state.schema.text(TEST_URL))));setSelectionAtEnd(view);view.focus();await sleep(120);
    ...
    if(!hit){try{const pos=Math.min(start+1+TEST_URL.length,view.state.doc.content.size);const Sel=view.state.selection.constructor;if(typeof Sel.near==='function')view.dispatch(view.state.tr.setSelection(Sel.near(view.state.doc.resolve(pos),-1)))}catch(_){}view.focus();try{view.dom.dispatchEvent(new InputEvent('beforeinput',{inputType:'insertParagraph',bubbles:true,cancelable:true}))}catch(_){}hit=await waitEmbed(view,TEST_URL,5500)}
    if(!hit){nstatus('末尾URLの後で改行を1回押して',true);return}
    localStorage.setItem(TEST_KEY,JSON.stringify({url:TEST_URL,type:hit.node.type.name}));rememberNotify(TEST_URL,hit.node.type.name);
    nstatus('通知カード 1/1 ✅ 公開して通知確認 → 後で一括削除');updateCleanButton();
  }catch(e){nstatus('通知テスト停止：'+(e?.message||String(e)),true)}finally{if(nb)nb.disabled=false}
}
function collectRegisteredHits(view,registry){
  const hits=[];
  for(const rec of registry){
    view.state.doc.descendants((node,pos)=>{
      if(node.type?.name!==rec.type)return;
      if(node.isTextblock||/image|picture|photo/i.test(node.type?.name||''))return;
      if(nodeCarriesUrl(node,rec.url))hits.push({node,pos,url:rec.url,type:rec.type});
    });
  }
  const uniq=[];const seen=new Set();for(const h of hits){const k=h.pos+'|'+h.node.nodeSize;if(seen.has(k))continue;seen.add(k);uniq.push(h)}return uniq;
}
function updateCleanButton(){
  const c=document.getElementById(N_CLEAN);if(!c)return;const n=getRegistry().length;
  c.textContent=n?`通知カード一括削除（${n}件）`:'通知カード一括削除';
...
function cleanAllNotifyCards(){
  ...
  try{
    ...
    const registry=getRegistry();if(!registry.length){nstatus('削除対象 0件');return}
    const hits=collectRegisteredHits(view,registry);
    if(!hits.length){nstatus(`記録 ${registry.length}件／本文内 0件。削除するものはありません`);return}
    nstatus(`通知カード削除中 0/${hits.length}…`);
    let tr=view.state.tr;let done=0;
    for(const h of hits.sort((a,b)=>b.pos-a.pos)){tr=tr.delete(h.pos,h.pos+h.node.nodeSize);done++;nstatus(`通知カード削除中 ${done}/${hits.length}…`)}
    view.dispatch(tr);saveRegistry([]);localStorage.removeItem(TEST_KEY);
    nstatus(`通知カード ${done}/${hits.length} 一括削除 ✅ 極薄カードは残しています`);updateCleanButton();
  }catch(e){nstatus('削除停止：'+(e?.message||String(e)),true)}finally{if(c)c.disabled=false}
}
function mountNotify(){
  if(!document.body)return;migrateLegacy();
  ...
  let c=document.getElementById(N_CLEAN);if(!c){c=document.createElement('button');c.id=N_CLEAN;c.type='button';c.textContent='通知カード一括削除';c.addEventListener('click',cleanAllNotifyCards);document.body.appendChild(c)}
