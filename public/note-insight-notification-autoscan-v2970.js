...
function resumeSignature(cp){return String(cp?.resumeSignatureV2970||cp?.resumeSignatureV2960||cp?.boundarySignature||'')}
...
async function restoreCheckpoint(panel,host,cp){const wanted=resumeSignature(cp);if(!wanted)return'';health('前回保存位置を探しています…','saving');const savedTop=Number(cp?.resumeScrollTopV2970||cp?.resumeScrollTopV2960||0);if(savedTop>0){try{host.scrollTop=Math.min(savedTop,Math.max(0,host.scrollHeight-host.clientHeight))}catch{}await sleep(180)}for(let pass=0;pass<24&&!stop;pass++){for(const el of rows(panel)){const d=rowData(el);if(d&&sig(d)===wanted){try{el.scrollIntoView({block:'center'})}catch{}await sleep(120);return wanted}}const max=Math.max(0,Number(host.scrollHeight||0)-Number(host.clientHeight||0));if(Number(host.scrollTop||0)>=max-2)break;try{host.scrollTop=Math.min(max,Number(host.scrollTop||0)+Math.max(220,Number(host.clientHeight||300)*.82))}catch{}await sleep(250);await yieldUi()}return''}
