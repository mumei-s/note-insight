...
function transform(name,code){
  ...
  if(name.includes('runtime-v2958')){
    ...
    s=s.replace(/const VERSION='[^']*'/,"const VERSION='3.1.0'");
    ...
    s=s.replace(/const VERSION='[^']*',PROTOCOL='[^']*'/,"const VERSION='3.1.0',PROTOCOL='3.1.0'");
    ...
    s=s.replace(/const VERSION='[^']*'/,"const VERSION='3.1.0'").replaceAll('mumei-v2948-frame',V3_FRAME);
...
async function loadLatest(){
  ...
  if(essentialOk){
    ready=true;retryCount=0;clearLegacyStatus();
...
window.addEventListener('focus',()=>{clearLegacyStatus();if(!ready)startLoad()});
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'){clearLegacyStatus();if(!ready)startLoad()}});
startLoad();
})();
