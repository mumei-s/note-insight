...
function fatal(e){console.error('[INSIGHT V3.2.9]',e);try{localStorage.setItem('mumei-notification-v3-error',String(e?.message||e))}catch{}}
...
function showRescue(on){const r=ensureRescue();r.style.setProperty('display',on?'grid':'none','important');r.style.setProperty('pointer-events',on?'auto':'none','important');if(on)void paintRescueFilter()}
...
function startRescue(){if(!isNote)return;window.__mumeiV3ParentDock=true;ensureRescue();window.addEventListener('click',handleRescueClick,true);window.addEventListener('click',handleNotificationIntent,true);document.addEventListener('mumei-v3-reader-status',onRescueStatus);const hook=name=>{const orig=history[name];history[name]=function(){const out=orig.apply(this,arguments);setTimeout(refreshRescue,0);return out}};hook('pushState');hook('replaceState');addEventListener('popstate',refreshRescue);setInterval(refreshRescue,700);refreshRescue()}
