...
function ensureLauncher(){
  ...
  b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();if(manualVisible)closeIntent();else{autoStarted=false;openIntent('INSIGHT本人通知パネルを開きました')}} ,true);
  (document.body||document.documentElement).appendChild(b);return b
}
function paintLauncher(){const b=ensureLauncher();b.textContent=manualVisible?'× 通知パネル':'🔔 INSIGHT';b.setAttribute('aria-label',manualVisible?'INSIGHT本人通知パネルを閉じる':'INSIGHT本人通知パネルを開く');b.style.setProperty('background',manualVisible?'#12364a':'#07151f','important');b.style.setProperty('border-color',manualVisible?'#8feaff':'#5ddcf1','important')}
function ensureDock(){
...
function showDock(msg='通知一覧を確認中…'){
  ...
  f.style.setProperty('bottom','max(56px,calc(48px + env(safe-area-inset-bottom,0px)))','important');
...
function hideDock(){
  const f=primary?.isConnected?primary:null;if(f){f.dataset.mumeiDockVisible='0';f.style.setProperty('display','none','important')}
  paintLauncher()
}
...
addEventListener('focus',()=>{ensureLauncher();if(manualVisible){showDock();scheduleAutoStart()}});
