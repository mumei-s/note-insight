...
document.addEventListener('click',e=>{if(!likelyBell(e.target))return;intentUntil=Date.now()+6000;forceDock('🔔を検出しました。通知一覧を確認中…');clearTimeout(timer);timer=setTimeout(pulse,120)},true);
new MutationObserver(ms=>{for(const m of ms)for(const n of m.addedNodes){if(!(n instanceof Element))continue;if(notificationSurface(n)){intentUntil=Date.now()+6000;forceDock();clearTimeout(timer);timer=setTimeout(pulse,120);return}}}).observe(document.documentElement,{subtree:true,childList:true});
addEventListener('pageshow',()=>{if(document.querySelector(ITEM)){intentUntil=Date.now()+3500;forceDock();clearTimeout(timer);timer=setTimeout(pulse,120)}});
addEventListener('focus',()=>{if(document.querySelector(ITEM)){intentUntil=Date.now()+2500;forceDock();clearTimeout(timer);timer=setTimeout(pulse,120)}});
})();
