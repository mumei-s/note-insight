...
let primary=null,autoStarted=false,resetTimer=0,startTimers=[];
...
function hideDock(){
  const f=primary?.isConnected?primary:null;if(!f||f.dataset.mumeiDockVisible==='0')return;
...
function scheduleAutoStart(){clearStartTimers();for(const ms of [80,260,650,1200])startTimers.push(setTimeout(()=>{if(anySurface()){showDock();tryAutoStart()}},ms))}
function scheduleHide(){clearTimeout(resetTimer);resetTimer=setTimeout(()=>{if(anySurface())return;clearStartTimers();autoStarted=false;hideDock()},900)}
function openIntent(msg){clearTimeout(resetTimer);showDock(msg);scheduleAutoStart()}
document.addEventListener('click',e=>{if(likelyBell(e.target)){autoStarted=false;openIntent('🔔を検出しました。自動読込を準備中…')}},true);
new MutationObserver(ms=>{let hit=false;for(const m of ms)for(const n of m.addedNodes){if(!(n instanceof Element))continue;if(n instanceof HTMLIFrameElement&&n.id===FRAME&&primary&&n!==primary){n.remove();continue}if(notificationSurface(n))hit=true}if(hit||anySurface()){clearTimeout(resetTimer);showDock(hit?'通知一覧を検出しました。自動読込を準備中…':'');scheduleAutoStart()}else scheduleHide()}).observe(document.documentElement,{subtree:true,childList:true});
addEventListener('pageshow',()=>{if(anySurface()){autoStarted=false;openIntent('通知一覧を確認中…')}});
addEventListener('focus',()=>{if(anySurface()){showDock();scheduleAutoStart()}});
