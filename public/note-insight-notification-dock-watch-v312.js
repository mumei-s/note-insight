...
if(window.__mumeiNotificationFixedDock319)return;
...
async function boot(){removeOld();const mo=new MutationObserver(()=>removeOld());mo.observe(document.documentElement,{subtree:true,childList:true});try{run(await fetchFixed());removeOld()}catch(e){console.warn('[INSIGHT通知] 4列固定パネル hotfix retry',e);setTimeout(async()=>{try{run(await fetchFixed());removeOld()}catch(err){console.warn('[INSIGHT通知] 4列固定パネル hotfix failed',err)}},1800)}}
