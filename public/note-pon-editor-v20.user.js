// ==UserScript==
// @name         note ポン出し v21｜固定記事厳密・マガジン＋固定記事カード
// @namespace    https://github.com/mumei-s/note-insight
// @version      21.0.0
// @description  最新版1本。参加中共同マガジンを回数順に整理し、各オーナーのプロフィール「固定された記事」1件だけを取得。マガジンURLと固定記事URLを両方noteカード化。貼付一覧だけ全消し。
// @author       無名S note
// @match        https://editor.note.com/*
// @grant        GM_xmlhttpRequest
// @connect      note.com
// @connect      raw.githubusercontent.com
// @require      https://raw.githubusercontent.com/mumei-s/note-insight/main/public/note-pon-editor-v14.user.js?v=21.0.0
...
(() => {
'use strict';
if (window.__MUMEI_PON_V21__) return;
window.__MUMEI_PON_V21__ = true;
['__MUMEI_PON_V15_ADDON__','__MUMEI_PON_V16_ADDON__','__MUMEI_PON_V171_ADDON__','__MUMEI_PON_V172_ADDON__','__MUMEI_PON_V173_ADDON__','__MUMEI_PON_V18__','__MUMEI_PON_V181__','__MUMEI_PON_V19__','__MUMEI_PON_V20__'].forEach(k=>window[k]=true);
...
function exactPinned(dom,ownerName){const all=[...dom.querySelectorAll('body *')];const marker=all.find(e=>e.children.length===0&&(e.textContent||'').trim()==='固定された記事');if(!marker)return'';const links=[...dom.querySelectorAll('a[href]')];for(const a of links){if(!(marker.compareDocumentPosition(a)&Node.DOCUMENT_POSITION_FOLLOWING))continue;const u=artUrl(a);if(u&&own(u)===ownerName)return u}return''}
async function pinnedFor(ownerName){try{const html=await get(`https://note.com/${ownerName}`);return exactPinned(new DOMParser().parseFromString(html,'text/html'),ownerName)}catch{return''}}
...
async function inspect(row){const k=mkey(row.url),o=own(row.url);let title=row.title,d='',pin='',pt='';try{const html=await get(row.url),dom=new DOMParser().parseFromString(html,'text/html');title=[...dom.querySelectorAll('h1')].map(x=>(x.textContent||'').trim()).find(Boolean)||title;d=desc(html,k,dom);pin=await pinnedFor(o);if(pin)try{pt=articleText(await get(pin))}catch{pt=''}}catch{}let rule=limit(pt)||limit(d)||fallback(k,title,o);if(k==='m752f734f7a1c')rule={type:'count',count:4};if(k==='m8d6e2d4322c8')rule={type:'count',count:2};if(k==='ma4dad1f25900')rule={type:'count',count:3};if(['mbe79c0d9105c','m9f1b6d83fe39','ma7a2c6649fa2'].includes(k)&&rule.type==='none')rule={type:'count',count:1};return{...row,title,display:o==='ss_yr'?`👑 ${title}`:title,pin,rule,paid:PAID.has(k)||paid(pt+' '+d)}}
async function inspectAll(input,show){const out=new Array(input.length);let cur=0,done=0;await Promise.all(Array.from({length:5},async()=>{while(true){const i=cur++;if(i>=input.length)return;out[i]=await inspect(input[i]);show(`投稿回数を確認 ${++done}/${input.length}`)}}));return out.filter(Boolean)}
...
async function oneCard(v,row){const f=factory();if(!f||!cursor(v,row.pos))return false;try{const c=f(row.url);if(typeof c!=='function')return false;c(v.state,tr=>v.dispatch(tr),v)}catch{return false}const end=Date.now()+7000;while(Date.now()<end){if(!exists(v,row.url))return true;await sleep(160)}return false}
async function cards(show){const v=findView();if(!v)throw Error('EditorViewなし');for(let pass=0;pass<6;pass++){const list=rawRows(v);if(!list.length)return 0;show(`カード化 ${list.length}件残り｜マガジン＋固定記事`);for(let i=0;i<list.length;i++){show(`カード化 ${i+1}/${list.length}｜マガジン＋固定記事`);await oneCard(v,list[i]);await sleep(230);if((i+1)%12===0)await sleep(500)}await sleep(700)}return rawRows(v).length}
...
function install(){const root=document.getElementById('__mumei_pon_v14_root__');if(!root)return setTimeout(install,250);const panel=root.querySelector('#ponPanel14'),src=root.querySelector('#ponSrc14'),add=root.querySelector('#ponAdd14'),st=root.querySelector('#ponStatus14'),head=root.querySelector('#ponDrag14 b');if(!panel||!src||!add||!st)return setTimeout(install,250);if(head)head.textContent='↔️ ポン出し v21';['ponMags19','ponClearList21','ponMake21','ponCards21'].forEach(id=>document.getElementById(id)?.remove());const say=t=>st.textContent=t;
 const del=document.createElement('button');del.id='ponClearList21';del.textContent='🧹 ポン出し貼付分を全部消す';del.style.cssText='display:block;width:100%;border:1px solid #ff8d8d;border-radius:8px;padding:9px 5px;background:#5b1f29;color:#fff;font-weight:900;font-size:11px;margin-bottom:5px';panel.insertBefore(del,src);del.onclick=()=>{try{clearList(src,say)}catch(e){say('❌ '+(e?.message||e))}};
 const make=document.createElement('button');make.id='ponMake21';make.textContent='📚 回数整理→マガジン＋固定記事を全カード';make.style.cssText='display:block;width:100%;border:0;border-radius:8px;padding:9px 5px;background:#ffd54a;color:#261f00;font-weight:900;font-size:11px;margin-bottom:5px';panel.insertBefore(make,src);make.onclick=async()=>{if(busy)return;busy=true;make.disabled=true;try{say('参加中一覧を取得…');const list=await rows();const items=await inspectAll(list,say);src.value=build(items);say(`固定記事 ${items.filter(x=>x.pin).length}件確認 → 貼付中`);add.click()}catch(e){say('❌ '+(e?.message||e));busy=false;make.disabled=false}};
 const card=document.createElement('button');card.id='ponCards21';card.textContent='🃏 残りURLを全部カード化';card.style.cssText='display:block;width:100%;border:0;border-radius:8px;padding:9px 5px;background:#72f1c9;color:#032b25;font-weight:900;font-size:11px;margin-bottom:5px';panel.insertBefore(card,src);card.onclick=async()=>{if(busy)return;busy=true;card.disabled=true;try{const left=await cards(say);say(left?`⚠️ 生URL残り ${left}件｜もう一度カード化`:'✅ マガジン＋固定記事カード化完了')}catch(e){say('❌ '+(e?.message||e))}finally{busy=false;card.disabled=false;make.disabled=false}};
 let timer=null;new MutationObserver(()=>{if(/^✅ 完了/.test(st.textContent||'')){clearTimeout(timer);timer=setTimeout(async()=>{try{const left=await cards(say);say(left?`⚠️ 生URL残り ${left}件`:'✅ マガジン＋固定記事カード化完了')}catch(e){say('❌ '+(e?.message||e))}finally{busy=false;make.disabled=false}},700)}}).observe(st,{childList:true,subtree:true,characterData:true});
}
install();
})();
