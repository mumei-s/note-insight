...
// @version      2.9.49
// @description  前回保存位置まで通知欄を自動走査して保存。公開データ更新とは分離し、本人通知でしか取れない履歴だけを補完します。
...
// @require      https://raw.githubusercontent.com/mumei-s/note-insight/main/public/note-insight-notification-runtime-v2948.js?v=2948a
// @require      https://raw.githubusercontent.com/mumei-s/note-insight/main/public/note-insight-notification-autoscan-v2949.js?v=2949a
...
async function get(k,d){if(modern()&&typeof GM.getValue==='function')return GM.getValue(k,d);if(typeof GM_getValue==='function')return GM_getValue(k,d);return d}async function set(k,v){if(modern()&&typeof GM.setValue==='function')return GM.setValue(k,v);if(typeof GM_setValue==='function')return GM_setValue(k,v)}
...
function safeInsightReturn(value){try{const u=new URL(String(value||''));return u.origin==='https://mumei-s.github.io'&&u.pathname.startsWith('/note-insight/')?u.href:''}catch{return''}}function creatorId(v){try{const raw=String(v||'').trim(),u=new URL(/^https?:\/\//i.test(raw)?raw:`https://note.com/${raw.replace(/^@/,'')}`),id=u.pathname.split('/').filter(Boolean)[0]||'';return u.hostname==='note.com'&&/^[A-Za-z0-9_-]+$/.test(id)?id.toLowerCase():''}catch{return''}}function decode64(raw){const b=String(raw||'').replace(/-/g,'+').replace(/_/g,'/').padEnd(Math.ceil(String(raw||'').length/4)*4,'=');return JSON.parse(decodeURIComponent(escape(atob(b))))}
...
async function openMatchingInsight(){const a=await currentAccount(),u=new URL(INSIGHT_ENTRY);u.searchParams.set('from','note');u.searchParams.set('insightMode','notifications');if(a?.id)u.searchParams.set('account',a.id);u.hash='dashboard';location.assign(u.href)}function bindDockInsightButton(tries=0){try{const f=document.getElementById(DOCK_FRAME),b=f?.contentDocument?.getElementById('ins');if(b){if(b.dataset.mumeiAccountRoute==='1')return;b.dataset.mumeiAccountRoute='1';b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();void openMatchingInsight()},true);return}}catch{}if(tries<40)setTimeout(()=>bindDockInsightButton(tries+1),250)}
