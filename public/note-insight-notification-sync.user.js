...
// @version      1.1.0
// @description  noteトップから本物の通知ベルを開き、本人通知をINSIGHTへ自動同期します
...
(function(){
  ...
  const invoked=pageUrl.searchParams.get('mumei_notify')==='1'||pageUrl.searchParams.has('mumei_pair');
  if(!invoked)return;
  ...
  async function pairIfNeeded(){
    ...
    if(code){toast('INSIGHT通知同期を連携しています…','normal',true);try{const p=await request(PAIR,{action:'pair-exchange',code});token=p.ingestToken||'';if(!token)throw new Error('TOKEN_NOT_RETURNED');await GM_setValue(TOKEN_KEY,token);toast('INSIGHT通知同期：連携完了','ok')}catch(e){toast('連携できませんでした。INSIGHTから連携をやり直してください。','error',true);return ''}}
    return token;
  }
  ...
  async function openRealNotifications(){
    for(let i=0;i<24;i++){
      ...
      if(bell){bell.click();await sleep(1000);return true}
      await sleep(250);
    }
    toast('noteの通知ベルを自動で見つけられません。右上のベルを1回タップしてください。','error',true);
    return false;
  }
  ...
  function scrollBox(root){
    ...
    return all.find(el=>{const s=getComputedStyle(el);return el.scrollHeight>el.clientHeight+40&&/(auto|scroll)/.test(s.overflowY)})||root;
  }

  async function collect(){
    let root=null;
    for(let i=0;i<20;i++){root=notificationRoot();if(root)break;await sleep(250)}
    ...
    for(let i=0;i<50;i++){
      ...
      const h=box.scrollHeight,c=all.size;
      if((sawPrevious&&i>=2)||(h===lastHeight&&c===lastCount))stable++;else stable=0;
      if(stable>=3||c>=1200)break;
      lastHeight=h;lastCount=c;box.scrollTop=box.scrollHeight;await sleep(350);
    }
  ...
  async function sync(){
    ...
    const opened=await openRealNotifications();if(!opened)return;
    toast('本人通知をINSIGHTへ同期中…','normal',true);
    try{
      const found=await collect(),items=[...found.values()];let inserted=0;
      ...
      const u=new URL(location.href);u.searchParams.delete('mumei_pair');u.searchParams.delete('mumei_notify');history.replaceState(null,'',u.pathname+(u.search?'?'+u.searchParams.toString():'')+u.hash);
