...
async function choose(c,m){if(c.tagArticle)return{...c,...c.tagArticle,sourceKeys:c.sourceKeys||[],sourceTags:c.sourceTags||[],articleSource:'hashtag'};try{if(m==='oldest')return oldest(c);if(m==='fixed')return article(list(await contents(c.urlname,1,false))[0],c);if(m==='todayYesterday'){const a=list(await contents(c.urlname,1,true)).map(r=>article(r,c)).filter(Boolean),days=new Set([ymd(Date.now()),ymd(Date.now()-86400000)]),hit=a.find(x=>days.has(ymd(x.publishAt)));return hit||article(list(await contents(c.urlname,1,false)).find(n=>(n.note||n).isPinned===true||(n.note||n).is_pinned===true),c)||a[0]}return article(list(await contents(c.urlname,1,true))[0],c)}catch{return null}}
