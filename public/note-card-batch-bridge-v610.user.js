...
// @version      6.5.0
// @description  成功済み10枚一括挿入＋記事別ON/OFF＋最終107枚/URL対応表の事前検証
...
(function(){
'use strict';
if(window.__MUMEI_BATCH_BRIDGE_650__) return;
window.__MUMEI_BATCH_BRIDGE_650__=true;
...
function mount(){if(!document.body)return;const on=enabled(),oldPanel=document.getElementById(PANEL),oldButton=document.getElementById(BTN);if(!on){if(oldPanel)oldPanel.style.display='none';if(oldButton)oldButton.style.display='none';return}hideDirectUI();let p=oldPanel;if(!p){p=document.createElement('div');p.id=PANEL;p.textContent='COMPLETE 6.5｜この記事だけON';document.body.appendChild(p)}Object.assign(p.style,{position:'fixed',right:'8px',top:'42%',zIndex:'2147483646',maxWidth:'250px',padding:'6px 8px',borderRadius:'8px',background:'#111827',color:'#fff',fontSize:'10px',lineHeight:'1.3',boxShadow:'0 3px 12px rgba(0,0,0,.25)',pointerEvents:'none',display:'block'});let b=oldButton;if(!b){b=document.createElement('button');b.id=BTN;b.type='button';b.textContent='10枚 COMPLETE 6.5';b.addEventListener('click',mainAction);document.body.appendChild(b)}Object.assign(b.style,{position:'fixed',right:'8px',top:'48%',zIndex:'2147483647',border:'0',borderRadius:'10px',padding:'10px 13px',background:'#059669',color:'#fff',fontSize:'13px',fontWeight:'800',boxShadow:'0 4px 14px rgba(0,0,0,.28)',touchAction:'manipulation',display:'block'})}
...
async function preflightFinal(){if(!enabled()||finalChecked)return;finalChecked=true;try{const raw=await xhr(FINAL_MANIFEST,'text'),m=JSON.parse(raw),items=Array.isArray(m.items)?m.items:[],urls=items.map(x=>x?.url).filter(Boolean);if(m.count!==107||m.width!==860||m.height!==140||items.length!==107||new Set(urls).size!==107||items.some((x,i)=>x.index!==i+1||x.width!==860||x.height!==140||!x.title||!x.creator||!x.cardPath))throw new Error('最終107件データ不整合');window.__MUMEI_SUMMER_107_MANIFEST__=m;document.dispatchEvent(new CustomEvent('mumei-final-107-ready',{detail:{count:107,width:860,height:140}}));if(!busy&&!armed&&!consumed&&mode==='idle')status('10件通知テスト待機｜最終107枚 READY')}catch(e){finalChecked=false;if(!busy&&!armed&&!consumed&&mode==='idle')status('最終107枚の確認停止：'+(e?.message||String(e)),true)}}
...
async function inject(input){
 if(!enabled()||!armed||consumed||!files.length||!imageInput(input)||beforeInputs.has(input))return false;consumed=true;armed=false;if(timer)clearTimeout(timer);timer=null;const send=files;files=[];const currentMode=mode;const expected=currentMode==='test'?1:10;
...
document.addEventListener('mumei-direct-success-done',e=>{if(!enabled())return;const ok=e.detail?.ok||0;consumed=false;retryMode=false;retryIndex=0;status(`完成：画像10/10・URL ${ok}/10 ✅`,ok!==10);buttonText('10枚 COMPLETE 6.5');showButton(true);hideDirectUI()});
...
async function retryDirect(){if(!enabled())return;const d=document.getElementById(DIRECT_BTN);if(!d){status('DIRECT SUCCESSが見つかりません',true);return}showButton(false);status(retryIndex?`URL ${retryIndex}/10から再開…`:'URL再開…');d.click()}
...
function mainAction(){if(!enabled())return;if(retryMode)return retryDirect();return arm()}
