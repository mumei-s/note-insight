...
if(page.__MUMEI_SEND3_ADDON_112__)return;
page.__MUMEI_SEND3_ADDON_112__=true;
...
const ACTIVE_KEY='mumei_send3_active_v112';
const ROWS_KEY='mumei_send3_rows_v112';
const BTN_ID='mumei-send3-v112';
const STATUS_ID='mumei-send3-status-v112';
const W=860,H=140;
...
function status(t,bad=false){let s=document.getElementById(STATUS_ID);if(!s)return;s.textContent=t;s.dataset.bad=bad?'1':'0'}
...
function webpackRequire(){const chunks=page.webpackChunk_N_E;if(!chunks||typeof chunks.push!=='function')return null;let req=null;const id=980000000+Math.floor(Math.random()*10000000);try{chunks.push([[id],{},r=>{req=r}])}catch(_){}return req}
...
function xhr(url,type='text'){return new Promise((res,rej)=>GM_xmlhttpRequest({method:'GET',url,responseType:type,timeout:30000,onload:r=>r.status>=200&&r.status<300?res(r.response):rej(new Error('取得失敗 '+r.status)),onerror:()=>rej(new Error('通信失敗')),ontimeout:()=>rej(new Error('通信タイムアウト'))}))}
function meta(html,key){const d=new DOMParser().parseFromString(html,'text/html');return d.querySelector(`meta[property="${key}"]`)?.content||d.querySelector(`meta[name="${key}"]`)?.content||''}
async function bitmap(blob){if('createImageBitmap'in page)return page.createImageBitmap(blob);return new Promise((res,rej)=>{const i=new page.Image(),u=URL.createObjectURL(blob);i.onload=()=>{URL.revokeObjectURL(u);res(i)};i.onerror=()=>{URL.revokeObjectURL(u);rej(new Error('画像読込失敗'))};i.src=u})}
...
function lines(c,text,max,maxLines){const out=[];let line='';for(const ch of[...text]){const t=line+ch;if(line&&c.measureText(t).width>max){out.push(line);line=ch;if(out.length===maxLines-1)break}else line=t}if(out.length<maxLines&&line){let rest=[...text].slice(out.join('').length).join('');while(rest&&c.measureText(rest+'…').width>max)rest=rest.slice(0,-1);out.push(rest+(rest.length<text.length?'…':''))}return out.slice(0,maxLines)}
async function makeFile(url,index){const html=await xhr(url),imgUrl=meta(html,'og:image'),title=meta(html,'og:title')||url.split('/').pop(),author=meta(html,'author')||'sashisashi';if(!imgUrl)throw new FatalError('サムネ取得失敗 '+index);const img=await bitmap(await xhr(imgUrl,'blob')),cv=document.createElement('canvas');cv.width=W;cv.height=H;const c=cv.getContext('2d');c.fillStyle='#fff';c.fillRect(0,0,W,H);c.strokeStyle='#d9dde3';c.lineWidth=1.5;rounded(c,1,1,W-2,H-2,12);c.stroke();const tw=320,th=124,tx=W-tw-8,ty=8,textX=16,textW=tx-textX-12;c.textBaseline='top';c.fillStyle='#171b21';c.font='700 18px system-ui';lines(c,title,textW,3).forEach((x,i)=>c.fillText(x,textX,12+i*24));c.fillStyle='#626975';c.font='14px system-ui';c.fillText(author,textX,110);const iw=img.width||img.naturalWidth,ih=img.height||img.naturalHeight,sc=Math.min(tw/iw,th/ih),dw=iw*sc,dh=ih*sc;c.fillStyle='#f7f8fa';rounded(c,tx,ty,tw,th,8);c.fill();c.save();rounded(c,tx,ty,tw,th,8);c.clip();c.drawImage(img,tx+(tw-dw)/2,ty+(th-dh)/2,dw,dh);c.restore();if(img.close)img.close();const out=await new Promise((res,rej)=>cv.toBlob(b=>b?res(b):rej(new Error('画像生成失敗')),'image/png',1));return{file:new page.File([out],`${String(index).padStart(2,'0')}_send3.png`,{type:'image/png'}),title,author}}
...
async function runSend3(){if(busy||!isEdit())return;busy=true;try{const v=findView();if(!v)throw new FatalError('EditorViewなし');core();captureTemplate(v);const hits=[];for(const u of URLS){hits.push(...officialCards(v,u),...exactUrlParagraphs(v,u));const im=findImage(v,u);if(im)hits.push(im)}deleteHits(v,hits);ensureEnd(v);await saveDraft(v,'3送の旧データを消して保存中…',()=>{for(const u of URLS)if(officialCards(v,u).length||exactUrlParagraphs(v,u).length||findImage(v,u))throw new FatalError('初期化残り')});if(!validTemplate(getTemplate()))throw new FatalError('標準カード学習データなし。本文に既存のnote標準カードを1枚残してから再実行');status('3件の過去サムネイルを取得・生成中…');const made=await Promise.all(URLS.map((u,i)=>makeFile(u,i+1)));const rows=made.map((m,i)=>({index:i+1,url:URLS[i],title:m.title,author:m.author,nodeId:''}));saveRows(rows);arm={view:v,rows,files:made.map(x=>x.file),used:false,beforeIds:new Set(imageNodes(v).map(e=>String(e.node.attrs?.id||'')).filter(Boolean)),beforeInputs:new Set(document.querySelectorAll('input[type="file"]')),timer:null};install();arm.timer=setTimeout(()=>{status('画像選択待機が3分を超えました',true);cancelArm();busy=false},180000);status('準備OK｜本文をタップ→「＋」→「画像」を1回だけ')}catch(e){status('3送停止：'+(e?.message||String(e)),true);busy=false}}
