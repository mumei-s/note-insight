...
function notificationShell(){return commonShell(exact('通知'),exact('お知らせ'))}
...
function moveIndependent(shell){const rail=document.getElementById(RAIL),settings=document.getElementById(SETTINGS);if(rail&&rail.parentElement!==document.body)document.body.append(rail);if(settings&&settings.parentElement!==document.body)document.body.append(settings);for(const el of document.querySelectorAll(`[${HOST_ATTR}]`)){if(el!==shell)el.removeAttribute(HOST_ATTR)}if(shell)shell.setAttribute(HOST_ATTR,'1');if(rail)rail.hidden=!shell;if(!shell&&settings)settings.remove();stripBoundaryText()}
let frame=0;
...
window.addEventListener(EVT_MANUAL,e=>{const shell=notificationShell();if(!shell){e.stopImmediatePropagation();window.dispatchEvent(new CustomEvent(EVT_STATUS,{detail:{state:'error',text:'🔔の「通知」一覧を開いてから押してください',manual:true,version:'2.9.35'}}));return}if(e.detail&&typeof e.detail==='object')e.detail.root=shell},true);
