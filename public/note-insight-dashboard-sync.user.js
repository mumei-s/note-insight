...
// @version      1.4.1
// @description  INSIGHTの読込ボタンからアカウント照合・公式Dashboard読込・INSIGHT復帰まで自動実行。本人通知とは独立し、通常時パネルは完全非表示です。
...
// @require      https://raw.githubusercontent.com/mumei-s/note-insight/main/public/note-insight-dashboard-sync-core-v1.1.0.js?v=141a
// ==/UserScript==
...
(() => {
  ...
  async function pairPending(){const p=await loadPending();if(!p)return false;ensureCorePanel();setCoreStatus('INSIGHTアカウント照合中…');const current=await currentNoteIdV141(),expected=String(p.noteId||'').toLowerCase();if(!current)throw new Error('NOTE_LOGIN_REQUIRED');if(current!==expected)throw new Error(`DASHBOARD_ACCOUNT_MISMATCH：note @${current} / INSIGHT @${expected}`);const x=await request({action:'pair-exchange',code:String(p.code||'')});const issued=String(x.noteId||'').toLowerCase();if(issued!==current)throw new Error(`DASHBOARD_ACCOUNT_MISMATCH：note @${current} / INSIGHT @${issued}`);localStorage.setItem(TOKEN_KEY,String(x.ingestToken||''));localStorage.setItem(NOTE_KEY,current);localStorage.setItem('mumei-dashboard-tool-version',VERSION);const back=safeReturn(p.returnTo);if(back)localStorage.setItem(RETURN_KEY,back);sessionStorage.setItem(FLOW_KEY,'1');await gmDel(HANDOFF_KEY);setCoreStatus(`✓ @${current} アカウント一致。公式Dashboardを読み込みます`,'ok');return true}
  ...
  async function boot(){try{installHideStyle(false);const pairedNow=await pairPending(),flow=sessionStorage.getItem(FLOW_KEY)==='1';if(!pairedNow&&!flow){panel()?.remove();return}if(!looksDashboard()){setCoreStatus('公式Dashboardへ移動中…');if(location.pathname!=='/sitesettings/stats'){location.assign('https://note.com/sitesettings/stats');return}}await startRead()}catch(e){ensureCorePanel();showPanel();setCoreStatus(`⚠ ${e?.message||e}｜自動読込に失敗した時だけこの復旧パネルを表示します`,'warn')}}
