// ==UserScript==
// @name         無名S note 極薄カード COMPLETE 5.0
// @namespace    https://github.com/mumei-s/note-insight/card-complete-500
// @version      5.0.0
// @description  極薄カード10枚を一括生成・挿入し、各画像へnote記事URLをDIRECT設定
...
(function () {
  'use strict';
  if (window.__MUMEI_CARD_COMPLETE_500__) return;
  window.__MUMEI_CARD_COMPLETE_500__ = true;
  ...
  const CARD_W = 860;
  const CARD_H = 140;
  const BTN = 'mumei-card500-btn';
  const PANEL = 'mumei-card500-panel';
  const OLD_PREFIXES = ['mumei-batch10','mumei-url-repair','mumei-thin-card-auto','mumei-direct-url-300'];
  let armed = false;
  let consumed = false;
  let busy = false;
  let preparedFiles = [];
  let beforeInputs = new Set();
  let beforeImages = new Set();
  let timer = null;
  const sleep = ms => new Promise(r => setTimeout(r, ms));

  function pageDoc() {
    try { return unsafeWindow && unsafeWindow.document ? unsafeWindow.document : document; }
    catch (_) { return document; }
  }

  function editor() {
    const d = pageDoc();
    return d.querySelector('.ProseMirror[contenteditable="true"]') || d.querySelector('.ProseMirror');
  }

  function hideOld() {
    for (const el of [...document.querySelectorAll('[id]')]) {
      if (OLD_PREFIXES.some(p => el.id.startsWith(p))) {
  ...
  function status(text, bad=false) {
    ...
    if (!p) return;
    p.textContent = text;
    p.style.background = bad ? '#991b1b' : '#065f46';
    p.style.display = 'block';
  }

  function showButton(show=true) {
  ...
  function mount() {
    ...
    if (!panel) {
      ...
      panel.id = PANEL;
      panel.textContent = 'COMPLETE 5.0：860×140固定';
      document.body.appendChild(panel);
    }
    Object.assign(panel.style, {
      position:'fixed', right:'8px', bottom:'58px', zIndex:'2147483646',
      maxWidth:'220px', padding:'5px 7px', borderRadius:'7px',
      background:'#065f46', color:'#fff', fontSize:'10px', lineHeight:'1.25',
      boxShadow:'0 3px 10px rgba(0,0,0,.22)', pointerEvents:'none'
    });
    ...
    if (!button) {
      ...
      button.id = BTN;
      button.type = 'button';
      button.textContent = '10枚 COMPLETE 5.0';
      button.addEventListener('click', arm);
      document.body.appendChild(button);
    }
    Object.assign(button.style, {
      position:'fixed', right:'8px', bottom:'10px', zIndex:'2147483647',
      border:'0', borderRadius:'9px', padding:'9px 12px',
      background:'#059669', color:'#fff', fontSize:'12px', fontWeight:'800',
      boxShadow:'0 3px 12px rgba(0,0,0,.25)', touchAction:'manipulation'
    });
  }
  ...
  function fitLines(c,text,maxWidth,maxLines) {
    ...
    for (; cursor<chars.length; cursor++) {
      ...
      if (c.measureText(test).width > maxWidth && line) {
        lines.push(line);
        line = chars[cursor];
        if (lines.length === maxLines-1) { cursor++; break; }
      } else line = test;
    }
    if (lines.length < maxLines) {
      let rest = line + chars.slice(cursor).join('');
      if (c.measureText(rest).width > maxWidth) {
        while (rest && c.measureText(rest+'…').width > maxWidth) rest = rest.slice(0,-1);
        rest += '…';
      }
  ...
  async function makeCard(item) {
    ...
    c.strokeStyle='#d9dde3'; c.lineWidth=1.5;
    roundedRect(c,1,1,CARD_W-2,CARD_H-2,12); c.stroke();
    ...
    c.drawImage(bmp,thumbX+(thumbW-dw)/2,thumbY+(thumbH-dh)/2,dw,dh);
    c.restore();
  ...
  function looksLikeView(v) {
    try { return !!v && typeof v==='object' && v.state?.doc && v.state?.schema && typeof v.dispatch==='function' && v.dom && typeof v.posAtDOM==='function'; }
  ...
  function findView() {
    ...
    const push=(v,d=0)=>{
      if (!v || seen.has(v) || d>7) return;
      if (typeof v!=='object' && typeof v!=='function') return;
      seen.add(v); q.push([v,d]);
    };
    ...
    while (q.length && steps<12000) {
      ...
      for (const k of keys) {
        ...
        if (d<7 && x && (typeof x==='object'||typeof x==='function')) push(x,d+1);
      }
    }
    return null;
  }
  ...
  function norm(u) {
    try { const x=new URL(u,location.href); x.search=''; x.hash=''; return x.href; }
  ...
  function buildLinkAttrs(type,url) {
    const spec=type?.spec?.attrs||{}, attrs={};
  ...
  function nodeHasLink(node,url) {
    ...
    for (const m of node.marks||[]) {
      if (/link/i.test(m.type?.name||'') && Object.values(m.attrs||{}).map(String).includes(url)) return true;
    }
  ...
  function trySetAttr(view,pos,node,url) {
    for (const key of attrKeys(node)) {
      try {
  ...
  function parentCandidates(view,pos) {
    ...
    try {
      ...
      for (let d=$p.depth;d>=1;d--) {
        const node=$p.node(d);
        let p;
        try { p=$p.before(d); } catch (_) { continue; }
        out.push({node,pos:p});
      }
    } catch (_) {}
    return out;
  ...
  function setDirect(view,img,url) {
    const hit=findNodeForImg(view,img);
    if (!hit) return false;
    const list=[hit,...parentCandidates(view,hit.pos)];
    for (const c of list) if (trySetAttr(view,c.pos,c.node,url)) return true;
    for (const c of list) if (trySetMark(view,c.pos,c.node,url)) return true;
    return false;
  }
  ...
  async function directAll(imgs) {
    ...
    const view=findView();
    if (!view) throw new Error('EditorView取得失敗');
    let ok=0;
    for (let i=0;i<Math.min(10,imgs.length);i++) {
      status(`DIRECT URL ${i+1}/10…`);
      if (setDirect(view,imgs[i],ITEMS[i].url)) ok++;
      else throw new Error(`${i+1}枚目URL設定失敗`);
      await sleep(80);
    }
    return ok;
  }
  ...
  async function waitInserted(timeout=45000) {
    ...
    while (Date.now()<end) {
      const added=[...root.querySelectorAll('img')].filter(x=>!beforeImages.has(x));
      if (added.length>=10) return added;
      await sleep(350);
    }
  ...
  async function inject(input) {
    ...
    consumed=true; armed=false;
    if (timer) clearTimeout(timer);
    timer=null;
    const send=preparedFiles;
    preparedFiles=[];
    try {
      const dt=new DataTransfer();
      send.forEach(f=>dt.items.add(f));
      ...
      consumed=false;
      status('停止：'+(e?.message||String(e)),true);
      showButton(true);
      return false;
    }
  }
  ...
  const observer=new MutationObserver(ms=>{
    ...
    for (const m of ms) for (const node of m.addedNodes) {
      ...
      for (const input of node.querySelectorAll?.('input[type="file"]')||[]) {
        if (imageInput(input)) { inject(input); return; }
      }
    }
  });

  function startObs() {
    if (!document.documentElement) return setTimeout(startObs,50);
    observer.observe(document.documentElement,{childList:true,subtree:true});
  }

  const nativeClick=HTMLInputElement.prototype.click;
  HTMLInputElement.prototype.click=function(...args) {
    if (armed && !consumed && imageInput(this) && !beforeInputs.has(this)) { inject(this); return; }
  ...
  async function arm() {
    ...
    try {
      hideOld();
      const root=editor();
      if (!root) throw new Error('note本文欄が見つかりません');
      ...
      armed=true;
      consumed=false;
      status('準備OK → 本文タップ →「＋」→「画像」を1回');
      timer=setTimeout(()=>{
        if (armed) {
          armed=false;
          preparedFiles=[];
          status('時間切れ。もう一度押して',true);
          showButton(true);
        }
      },60000);
    } catch (e) {
