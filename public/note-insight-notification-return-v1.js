...
const VERSION='1.0.0',RETURN='mumei_insight_return_bell_v1';
...
function openNotice(){
 for(const root of document.querySelectorAll('[role="dialog"],[role="menu"],[popover],section,aside')){
  if(!shown(root))continue;const t=String(root.textContent||'').replace(/\s+/g,' ');
  if(/通知/.test(t)&&/お知らせ/.test(t)&&/(?:たった今|昨日|\d+\s*(?:分|時間|日)前|スキ|フォロー|コメント|返信|追加|メンバー|購入)/u.test(t))return true
 }
 return false
}
function clickBell(){
