...
function saveGuard(id,g){as(id,GUARD,g)}
function clearStrikes(id){const g=guard(id);if((g.strikes||[]).length||g.reason){g.strikes=[];g.reason='';if(Number(g.until||0)<=Date.now())g.until=0;saveGuard(id,g)}}
function strike403(id,key){const now=Date.now(),g=guard(id),recent=(Array.isArray(g.strikes)?g.strikes:[]).filter(x=>now-Number(x.t||0)<120000&&x.key!==key);recent.push({t:now,key});const unique=[...new Map(recent.map(x=>[x.key,x])).values()];if(unique.length>=3){g.until=now+30*60*1000;g.reason='異なる相手で403が3回続いたため30分休止';g.strikes=[]}else{g.strikes=unique;g.reason=''}saveGuard(id,g);return{cooldown:Number(g.until||0)>now,wait:Math.max(0,Number(g.until||0)-now),count:unique.length}}
...
async function safeLike(btn){if(busy)return;busy=true;btn.disabled=true;try{const me=await who(),s=al(me.id,'session',null),q=Array.isArray(s?.queue)?s.queue:[],i=Math.max(0,Number(s?.index)||0),c=q[i];if(!c?.key)throw new Error('現在の記事を特定できません');
  const known=resultMap(me.id).find(x=>x.key===c.key);if(known){markQueueState(me.id,s,i,known.state);clearStrikes(me.id);setStatus(known.state==='preexisting'?'💗 最初からスキ済み｜解除せず次へ':'✅ 今回スキ済み｜二重送信せず次へ');advance();return}
  if(c.liked===true){markQueueState(me.id,s,i,'preexisting');addResult(me.id,c,'preexisting');clearStrikes(me.id);setStatus('💗 最初からスキ済み｜何も送らず完了');advance();return}
  ...
  if(current===true){markQueueState(me.id,s,i,'preexisting');addResult(me.id,c,'preexisting');clearStrikes(me.id);setStatus('💗 最初からスキ済み｜何も送らず完了');advance();return}
  setStatus(`❤️ ${c.title||c.key} にスキ…`);
  try{await api(`/api/v3/notes/${encodeURIComponent(c.key)}/likes`,{method:'POST',headers:{'content-type':'application/json','x-requested-with':'XMLHttpRequest'},body:'{}'});const t=Date.now();const likes=history(me.id,'likes');likes.push({t,key:c.key,urlname:c.urlname,source:'v521'});as(me.id,'likes',likes.slice(-1600));const acts=history(me.id,'actions');acts.push({t,type:'like',key:c.key,urlname:c.urlname,source:'v521'});as(me.id,'actions',acts.slice(-1600));const out=history(me.id,'outbound');out.push({t,key:c.key,urlname:c.urlname,source:'v521'});as(me.id,'outbound',out.slice(-1600));markQueueState(me.id,s,i,'liked_now');addResult(me.id,c,'liked_now');clearStrikes(me.id);setStatus('❤️ 今回スキ完了');advance()}catch(e){const body=String(e.body||'');if(/already|liked/i.test(body)){markQueueState(me.id,s,i,'preexisting');addResult(me.id,c,'preexisting');clearStrikes(me.id);setStatus('💗 すでにスキ済み｜解除せず完了');advance();return}if(e.status===429){const wait=Math.max(60000,Number(e.retryAfter)||15*60*1000),until=Date.now()+wait,gg=guard(me.id);gg.until=until;gg.reason='429保護休止';gg.strikes=[];saveGuard(me.id,gg);scheduleResumeLabel(me.id,until,'429保護休止');return}if(e.status===403){const st=strike403(me.id,String(c.key));if(st.cooldown){scheduleResumeLabel(me.id,Date.now()+st.wait,'403保護休止');return}setStatus(`⚠ 403 (${st.count}/3)。この人は未完了のまま保持｜次の人へ進むなら「次へ」`,true);return}if(e.status===401){setStatus('🛑 401認証エラー｜未完了のまま保持',true);return}throw e}
