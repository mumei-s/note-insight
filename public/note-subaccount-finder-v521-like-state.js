...
function likedFrom(j){const d=j?.data??j??{},n=d.note||d;for(const k of ['isLiked','is_liked','liked','hasLiked'])if(typeof n?.[k]==='boolean')return n[k];return false}
...
async function safeLike(btn){if(busy)return;busy=true;btn.disabled=true;try{const me=await who(),s=al(me.id,'session',null),q=Array.isArray(s?.queue)?s.queue:[],i=Math.max(0,Number(s?.index)||0),c=q[i];if(!c?.key)throw new Error('現在の記事を特定できません');
  ...
  let current=false;try{current=likedFrom(await api(`/api/v3/notes/${encodeURIComponent(c.key)}`))}catch(e){if(e.status===401)throw e}
  if(current){markQueueState(me.id,s,i,'preexisting');addResult(me.id,c,'preexisting');setStatus('💗 最初からスキ済み｜何も送らず完了');advance();return}
