...
(() => {
  ...
  function ensureCorePanel(){if(document.getElementById('mumei-dashboard-sync'))return true;if(document.readyState!=='loading'){try{window.dispatchEvent(new Event('DOMContentLoaded'))}catch{}}return!!document.getElementById('mumei-dashboard-sync')}
  ...
  async function pairPending(){const p=await loadPending();if(!p)return false;ensureCorePanel();setCoreStatus('INSIGHT本人確認中…');const current=await currentNoteIdV130(),expected=String(p.noteId||'').toLowerCase();if(!current)throw new Error('NOTE_LOGIN_REQUIRED');if(current!==expected)throw new Error(`DASHBOARD_ACCOUNT_MISMATCH：note @${current} / INSIGHT @${expected}`);const x=await request({action:'pair-exchange',code:String(p.code||'')});const issued=String(x.noteId||'').toLowerCase();if(issued!==current)throw new Error(`DASHBOARD_ACCOUNT_MISMATCH：note @${current} / INSIGHT @${issued}`);localStorage.setItem(TOKEN_KEY,String(x.ingestToken||''));localStorage.setItem(NOTE_KEY,current);localStorage.setItem('mumei-dashboard-tool-version',VERSION);const back=safeReturn(p.returnTo);if(back)localStorage.setItem(RETURN_KEY,back);sessionStorage.setItem(FLOW_KEY,'1');await gmDel(HANDOFF_KEY);setCoreStatus(`✓ @${current} 本人連携完了。公式Dashboardを読み込みます`,'ok');return true}
  async function waitPanel(){for(let i=0;i<25;i++){if(ensureCorePanel())return true;await sleep(120)}return false}
  function watchCompletion(){const done=()=>{const s=document.querySelector('#mumei-dashboard-sync .status'),t=s?.textContent||'';if(!/同期完了/.test(t))return false;sessionStorage.removeItem(FLOW_KEY);markVersion();const back=safeReturn(localStorage.getItem(RETURN_KEY)||'');localStorage.removeItem(RETURN_KEY);if(back){const u=new URL(back);u.searchParams.set('dashboardSync','ok');u.searchParams.set('dashboardVersion',VERSION);u.searchParams.set('dashboardAt',new Date().toISOString());setTimeout(()=>location.assign(u.href),700)}return true};if(done())return;const o=new MutationObserver(()=>{if(done())o.disconnect()});o.observe(document.documentElement,{subtree:true,childList:true,characterData:true});setTimeout(()=>o.disconnect(),90000)}
  async function startRead(){if(!await waitPanel())throw new Error('DASHBOARD_TOOL_CORE_NOT_READY');const s=document.querySelector('#mumei-dashboard-sync .status');if(s&&!/読込中|同期完了/.test(s.textContent||''))s.textContent=`Dashboard同期 v${VERSION}`;watchCompletion();await sleep(1300);const btn=document.getElementById('mumei-dash-read');if(!btn)throw new Error('DASHBOARD_READ_BUTTON_NOT_FOUND');btn.click()}
  async function boot(){try{const pairedNow=await pairPending(),flow=sessionStorage.getItem(FLOW_KEY)==='1';if(!pairedNow&&!flow){if(!looksDashboard())setTimeout(()=>document.getElementById('mumei-dashboard-sync')?.remove(),120);return}if(!looksDashboard()){setCoreStatus('公式Dashboardへ移動中…');if(location.pathname!=='/sitesettings/stats'){location.assign('https://note.com/sitesettings/stats');return}}await startRead()}catch(e){ensureCorePanel();setCoreStatus(`⚠ ${e?.message||e}`,'warn')}}
