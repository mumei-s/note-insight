...
test("v2.9.47 bootstrap keeps one stable core runtime and account-aware handoff",async()=>{
  const boot=await read("public/note-insight-notification-sync.user.js");
  assert.match(boot,/@version\s+2\.9\.47/);
...
test("v2.9.47 recovers checkpoint from server-confirmed saved overlap, never note unread state",async()=>{
  const boot=await read("public/note-insight-notification-sync.user.js");
  ...
  assert.doesNotMatch(boot,/\bunread\b|aria-unread|is-unread/i);
  assert.doesNotMatch(boot,/scrollTop\s*=|scrollTo\(/);
});

test("v2945 stable core remains CSP-safe, bottom-docked, filter-session-only and scroll-neutral",async()=>{
...
test("v2.9.47 installer verifies the actual running version",async()=>{
  ...
  assert.match(setup,/最新版は v2\.9\.47/);
...
test("INSIGHT and本人通知 release tracks remain independent",async()=>{
  ...
  assert.equal(manifest.appVersion,"2026.09.08.4");
  assert.equal(manifest.notificationVersion,"2.9.47");
  assert.match(release,/CURRENT_INSIGHT_APP_VERSION = "2026\.09\.08\.4"/);
});
