...
test("V3.3.6 loads bottom-up reader, persistent checkpoint and fixed five-panel dock",async()=>{
  ...
  assert.match(v3,/@version\s+3\.3\.6/);
  assert.match(v3,/@require\s+https:\/\/raw\.githubusercontent\.com\/mumei-s\/note-insight\/main\/public\/note-insight-notification-reader-v323\.js\?v=3360/);
  ...
  assert.match(v3,/@require\s+https:\/\/raw\.githubusercontent\.com\/mumei-s\/note-insight\/main\/public\/note-insight-notification-network-v3300\.js\?v=3360/);assert.match(v3,/@require\s+https:\/\/raw\.githubusercontent\.com\/mumei-s\/note-insight\/main\/public\/note-insight-notification-runtime-v327\.js\?v=3360/);
  ...
  has(checkpoint,["mumei_insight_notification_checkpoint_local_v325:","localStorage.setItem","async function restore(","async function persist(","ここまで保存済み"]);assert.doesNotMatch(checkpoint,/new MutationObserver/);has(reader,["checkpointFor(","通知履歴の最下部まで到達できませんでした","通知一覧の先頭まで確認できませんでした","historyComplete:true"]);assert.doesNotMatch(reader,/start\.rebase/);
  has(runtime,["notification-filter-settings.html","notificationAccount","location.replace(u.href)","openNotificationBell","isNotificationOpen","mumei_insight_notification_feature_enabled_v1","setFeatureEnabled","hideImmediately","suppressUntilBell","notificationLeaveAction","onGlobalPointerDown","function showRoot(on){const want=Boolean(on&&featureEnabled&&!suppressUntilBell&&panelSessionActive)","function onStatus(e){if(suppressUntilBell||!featureEnabled)","function confirmHide(){hideTimer=0;if(suppressUntilBell)","event?.composedPath","mumei-v325-dock","data-a=\"read\"","data-a=\"mode\"","data-a=\"filter\"","data-a=\"settings\"","data-a=\"ins\"","mumei_insight_notification_auto_v325:","async function initMode(","async function toggleMode(","function maybeAuto(","autoDoneForSession","async function safeScan(","mountUiToViewport","leadDisplayName","profileCandidates","filterRows","runDockAction","pointerdown","pointerup","leadCreatorId","ids.has(lead)","creatorProfile","profileImageUrl","nickname"]);
  ...
  has(reader,["__mumeiNotificationReader323","__mumeiV3Reader323","readyToScan","confirmedClientSignatures","bottom-up-from-saved-line","saved-line-bottom-up-v324","ここまで保存済み","boundarySignature","boundaryEventIdentity","保存済みラインから先頭まで追加分を確認します","findSavedRecoveryElement","persistRecoveredBoundary","通知履歴の最下部まで到達できませんでした","通知一覧の先頭まで確認できませんでした","historyComplete:true"]);
...
test("release tracks V3.3.6 without putting the version in the user-facing label",async()=>{
  const manifest=JSON.parse(await read("public/insight-release.json")),release=await read("src/insight-release.ts"),v3=await read("public/note-insight-notification-v3.user.js");assert.equal(manifest.appVersion,"2026.09.21.3");assert.equal(manifest.notificationVersion,"3.3.6");assert.equal(manifest.notificationLabel,"本人通知");assert.equal(manifest.dashboardVersion,"1.4.4");assert.match(release,/CURRENT_INSIGHT_APP_VERSION = "2026\.09\.21\.3"/);assert.match(release,/CURRENT_NOTIFICATION_VERSION = "3\.3\.6"/);assert.match(v3,/@version\s+3\.3\.6/);
});
