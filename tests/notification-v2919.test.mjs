...
test("v2.9.69 bootstrap uses bottom-up incremental reader, safety and filter restore",async()=>{
  ...
  assert.match(boot,/@version\s+2\.9\.69/);has(boot,["runtime-v2958.js?v=2970","notification-filter-safety-v2961.js?v=2970","notification-filter-restore-v2962.js?v=2970","notification-autoscan-v2970.js?v=2970","notification-bootstrap-v2966.js?v=2970"]);assert.equal((boot.match(/\/\/ @require\s+/g)||[]).length,5);assert.doesNotMatch(boot,/notification-reader-v2963\.js\?v=2970/);has(bridge,["async function openMatchingInsight()","const VERSION='2.9.69'","notificationInstalled","notificationCheckedAt"]);has(safety,["unsafeContainer","distinctNotificationDescendants","purgeLegacyThreeButton","legacyThree","data-mumei-filter-container-restored"]);has(restore,["mumei_filter_export","mumei_existing","normalizeGroups","既存フィルター","exportTo"]);has(reader,["const VERSION='2.9.69'","note-notification-manual-sync-v2968","verified-shell-bottom-to-top","confirmedClientSignatures","scrollHost","sendBatch","slice().reverse()"]);
});

test("notification V3 is a stable remote-core loader with a new identity",async()=>{
  const v3=await read("public/note-insight-notification-v3.user.js");
  assert.match(v3,/@name\s+無名S note INSIGHT 本人通知 V3/);assert.match(v3,/@namespace\s+https:\/\/github\.com\/mumei-s\/note-insight\/notification-v3/);assert.match(v3,/@version\s+3\.0\.1/);assert.match(v3,/@run-at\s+document-start/);has(v3,["note-insight-notification-runtime-v2958.js","note-insight-notification-autoscan-v2970.js","note-insight-notification-filter-safety-v2961.js","note-insight-notification-bootstrap-v2966.js","mumei-v3-notification-frame","checked-v3","loadLatest","gmText","transform","data-mumei-v3-neutralized"]);assert.doesNotMatch(v3,/\/\/ @require\s+/);
});

test("notification runtime remains notification-only and compact",async()=>{
  const r=await read("public/note-insight-notification-runtime-v2958.js");has(r,["function messagingContext()","function findNoticeShell()","REACTION_RE","スキした人","リアクション一覧","SHELL='mumei-notice-shell-v2958'","下から読込","フィルター設定","INSIGHT【通知】"]);for(const x of ["年月日指定","modeDate","rangePanel"])assert.doesNotMatch(r,new RegExp(x));assert.doesNotMatch(r,/setInterval\(/);assert.match(r,/capture:true/);
});
...
test("notification installer now migrates to V3 and keeps file-import fallback",async()=>{
  const redirect=await read("public/notification-update.html"),update=await read("public/notification-update-v29665.html"),legacy=await read("public/notification-update-v2966.html"),setupRedirect=await read("public/notification-setup.html"),setup=await read("public/notification-setup-v2966.html");has(redirect,["notification-update-v29665.html","v3.0.1"]);has(update,["本人通知 V3","最新版 v3.0.1","note-insight-notification-v3.user.js","V3ファイルを保存","ファイルからインポート","mumei_insight_version_check","notificationInstalled","note側でV3を確認","旧2.xは削除しなくて大丈夫","mumeiV3Install","recoverRawInstall","mumei-v3-verified"]);assert.match(update,/window\.open\(scriptUrl\.href,'mumeiV3Install'\)/);assert.match(update,/e\.origin!==location\.origin/);has(legacy,["V3 v3.0.1","notification-update-v29665.html"]);has(setupRedirect,["notification-setup-v2966.html","V3 v3.0.1"]);has(setup,["3.0.1","通知フィルター設定","notificationInstalled","localStorage.setItem(KEY,checked)","通常のUserscript更新は不要"]);
});

test("filter settings restore existing entries and accept profile/article URLs",async()=>{
  const page=await read("public/notification-filter.html"),restore=await read("public/note-insight-notification-filter-restore-v2962.js");has(page,["登録済みをnoteから復元","プロフィールURL・記事URL・@ID・ID","mumei_existing","mergeGroups","削除せずマージ","note URL / 記事URL / @ID / ID","mumei_filter_export"]);has(restore,["mumei_existing","mumei_filter_export","mumei_insight_notification_groups_v1:","mumei_insight_magazine_mute_ids_v5:"]);assert.match(page,/u\.pathname\.split\('\/'\)\.filter\(Boolean\)\[0\]/);
});
...
test("notification history keeps one shared date filter and all required categories",async()=>{
  const ui=await read("src/member-insight-notifications-final.tsx"),css=await read("src/member-insight-notifications-final.css"),feed=await read("supabase/functions/insight-notification-feed-final/index.ts");has(ui,["selectedDay","type=\"date\"","📅 表示日","全期間に戻す","この日付は下の全カテゴリ共通です。","reply_self","reply_other","membership_reaction_self","membership_reaction_joined","membership_join","purchase","tip","quote","other"]);has(css,["minf-date-filter","color-scheme:dark","grid-template-columns:minmax(150px,1fr) auto auto"]);has(feed,["function jstDay","selectedDay:day||null","categoryCounts","dated=day?"]);
});

test("notification categories stay one panel with persisted long-press reorder",async()=>{
  const ux11=await read("src/insight-ux-v11.ts"),enh=await read("src/insight-notification-enhancements-v17.ts");has(ux11,["mumei-category-panel","mumei-category-toggle","mumei-category-open","mumeiSinglePanelBound","通知項目：","mumei-all-tab"]);for(const x of ["CAT_ORDER_KEY","mumei-moving","holdTimer"])assert.doesNotMatch(ux11,new RegExp(x));has(enh,["mumei-notification-category-order-v17","HOLD_MS=520","bindCategoryReorder","saveOrder","applyCategoryOrder","mumei-cat-held","長押しで並替"]);
});

test("comment body recovery uses feed body, canonical comment history and delegated binding",async()=>{
  const old=await read("src/insight-ux-v12.ts"),enh=await read("src/insight-notification-enhancements-v17.ts"),feed=await read("supabase/functions/insight-notification-feed-final/index.ts");has(old,["mumei-comment-body","コメント本文","返信本文","insight-comment-events"]);has(enh,["bindCommentBody","ensureCommentBody","feedBodies","meta?.body","insight-comment-events","通常コメント履歴から照合","通知feedの保存本文","mumei-comment-open"]);has(feed,["mergeMeta","meta:mergeMeta","comment_body","meta?.body"]);
});

test("final TOP is stable normal-data, analysis, notification with detail beside account switch",async()=>{
  const live=await read("src/member-insight-live-v2.tsx"),top=await read("src/insight-top-install-v16.ts"),release=await read("src/insight-release.ts"),guide=await read("src/insight-update-guide-v18.ts"),ux13=await read("src/insight-ux-v13.ts"),unified=await read("src/member-insight-unified-v4.tsx");has(live,["miv5-source-card normal","🔔 本人通知","📊 分析","🔎 詳細分析","manualDataRefresh","aria-label=\"連携データを更新\""]);has(top,["FINAL TOP: this is the only active TOP layout manager","miv5-source-card.normal{display:grid","miv5-source-card.dashboard{display:grid","miv5-source-card.notice{display:grid","miv5-source-card.detail{display:none","grid-template-columns:repeat(3,minmax(0,1fr))","mumei-detail-analysis-proxy","mumei-public-refresh-proxy","mumei-top-tool-install","mumei-canonical-install","dashboard-setup.html","notification-update.html","インストール / 更新","order:1","order:2","order:3"]);has(release,["insight-notification-enhancements-v17","insight-top-install-v16","insight-update-guide-v18"]);has(guide,["⬆ 更新があります","必要な項目だけ表示","INSIGHT本体","本人通知","Dashboard同期","更新案内を開く"]);assert.doesNotMatch(release,/insight-mobile-v14|insight-mobile-v15|insight-top-install-dedupe-v1/);has(ux13,["📊 分析","Dashboard同期＋本人通知 必須","Dashboard分析はDashboard同期＋本人通知の2ツール必須"]);has(unified,["アカウント切替","miu-topactions"]);
});

test("public refresh remains abortable and cannot permanently disable the visible control",async()=>{
  const live=await read("src/member-insight-live-v2.tsx"),top=await read("src/insight-top-install-v16.ts"),detail=await read("public/install-free-analysis-v2.html");has(live,["AbortController","PUBLIC_SYNC_TIMEOUT","MANUAL_UI_TIMEOUT","publicSyncController","manualRefreshRunning","finally{manualRefreshRunning.current=false;setDataBusy(false)}"]);has(top,["miv5-source-card.normal:not(.mumei-has-app-update)","mumei-has-app-update"]);has(detail,["watchdog=setTimeout","更新待ちを解除しました","busy=false;refresh.disabled=false"]);
});

test("notification-free detail analysis is compact, explicit, graphed and independently refreshable",async()=>{
  const detail=await read("public/install-free-analysis-v2.html");has(detail,["<details class=\"section\"","分析をすべて開く","分析をすべて収納","1記事あたり平均スキ数","1記事あたり平均コメント数","1記事あたり平均反応数","平均反応数","反応＝スキ＋コメント","PVではありません","recentSpark","cadenceChart","daysChart","hoursChart","wordsChart","evergreenChart","articlesChart","backgroundRefresh(true)","post('sync',18000)","本人通知・Dashboard同期なし","overflow-x:hidden"]);
});

test("Dashboard analysis uses Pro V3, rejects impossible PV conversion and stays collapsible",async()=>{
  const hub=await read("src/member-insight-analysis-hub.tsx"),pro=await read("src/member-insight-analytics-pro-v3.tsx"),css=await read("src/member-insight-analytics-pro-v3.css"),ux13=await read("src/insight-ux-v13.ts"),baseCss=await read("src/member-insight-analytics-base.css"),live=await read("src/member-insight-live-v2.tsx");has(hub,["Dashboard同期＋本人通知をそろえるとPV・流入・収益と人物反応を横断分析します。","公式Dashboardクロス分析は2つのツールを準備してください。","公式Dashboard＋INSIGHT Pro分析","MemberInsightAnalyticsProV3","miah-fold","NOTIFICATION DEEP ANALYSIS","人物別反応","メンシプ","購入/支援","topActorShare","kind:\"all\"","詳細分析"]);has(pro,["INSIGHT PRO ANALYTICS V3","PV>Impは集計範囲不一致として率計算しない","以前の572%・1431.9%のような表示は出しません","PV改善余地","反応/1,000PV","流入分散度","本人通知 × PV クロス分析","記事総合ランキング","分析をすべて開く","すべて収納","conversion=comparable?pageViews/impressions*100:null"]);has(css,[".mipro-fold",".mipro-fold summary",".mipro-scatter",".mipro-table"]);has(ux13,["Dashboard分析はDashboard同期＋本人通知の2ツール必須","旧v13の「本人通知なし/あり」2分岐DOM上書きは廃止"]);assert.doesNotMatch(ux13,/本人通知なしで読込/);assert.doesNotMatch(baseCss,/本人通知なし/);assert.match(live,/MemberInsightAnalysisHub/);
});
