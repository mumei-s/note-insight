...
test("V3.2.90 loads bottom-up reader, persistent checkpoint and fixed five-panel dock",async()=>{
  ...
  assert.match(v3,/@version\s+3\.2\.90/);
  ...
  assert.match(v3,/@require\s+https:\/\/raw\.githubusercontent\.com\/mumei-s\/note-insight\/main\/public\/note-insight-notification-runtime-v327\.js\?v=3290/);
...
test("installer is isolated, versionless and browser-specific",async()=>{
  ...
  assert.doesNotMatch(setup,/本人通知 V\d|V3\.2\.90をインストール|script_installation\.php#url=/);
...
test("release tracks V3.2.90 without putting the version in the user-facing label",async()=>{
  const manifest=JSON.parse(await read("public/insight-release.json")),release=await read("src/insight-release.ts"),v3=await read("public/note-insight-notification-v3.user.js");assert.equal(manifest.appVersion,"2026.09.19.1");assert.equal(manifest.notificationVersion,"3.2.90");assert.equal(manifest.notificationLabel,"本人通知");assert.equal(manifest.dashboardVersion,"1.4.4");assert.match(release,/CURRENT_INSIGHT_APP_VERSION = "2026\.09\.19\.1"/);assert.match(release,/CURRENT_NOTIFICATION_VERSION = "3\.2\.90"/);assert.match(v3,/@version\s+3\.2\.90/);
});
