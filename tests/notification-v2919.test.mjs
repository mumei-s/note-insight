...
test("release tracks V3.5.6 without putting the version in the user-facing label",async()=>{
  ...
  assert.equal(manifest.appVersion,"2026.09.21.21");assert.equal(manifest.notificationVersion,"3.5.6");assert.equal(manifest.dmVersion,"1.3.2");assert.equal(manifest.notificationLabel,"本人通知");assert.equal(manifest.dashboardVersion,"1.4.4");
