...
test("V3.3.10 loads panel-free automatic notification reader",async()=>{
  ...
  assert.ok(v3.includes("note-insight-notification-autoscan-v2970.js?v=33100"));
  assert.ok(v3.includes("note-insight-notification-bootstrap-v2966.js?v=33100"));assert.ok(v3.includes("note-insight-notification-runtime-v2939-filter.js?v=33100"));
  assert.doesNotMatch(v3,/note-insight-notification-runtime-v2958\.js\?v=|note-insight-notification-runtime-v327\.js\?v=/);
  has(reader,["function directPanel","function findPanel(){return directPanel()}","actor_image_url:img","function scheduleAuto","historyComplete","mumei_insight_notification_avatar_repair_v338:","mumei-inline-notification-tools-v339","INSIGHT【通知】","✓全履歴確認","✓追加確認"]);
});
...
test("release tracks V3.3.10 without putting the version in the user-facing label",async()=>{
  ...
  assert.equal(manifest.appVersion,"2026.09.21.7");assert.equal(manifest.notificationVersion,"3.3.10");assert.equal(manifest.notificationLabel,"本人通知");assert.equal(manifest.dashboardVersion,"1.4.4");
  assert.match(release,/CURRENT_INSIGHT_APP_VERSION = "2026\.09\.21\.7"/);assert.match(release,/CURRENT_NOTIFICATION_VERSION = "3\.3\.10"/);assert.match(v3,/@version\s+3\.3\.10/);
});
