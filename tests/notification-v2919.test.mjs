...
test("V3.2.46 loads bottom-up reader, persistent checkpoint and fixed five-panel dock",async()=>{
  ...
  assert.match(v3,/@version\s+3\.2\.46/);
  ...
  assert.match(v3,/@require\s+https:\/\/raw\.githubusercontent\.com\/mumei-s\/note-insight\/main\/public\/note-insight-notification-runtime-v327\.js\?v=3246/);
  ...
  has(runtime,["通知フィルター登録","data-addg","note URL / @ID / ID","event?.composedPath","mumei-v325-dock","data-a=\"read\"","data-a=\"mode\"","data-a=\"filter\"","data-a=\"settings\"","data-a=\"ins\"","mumei_insight_notification_auto_v325:","async function initMode(","async function toggleMode(","function maybeAuto(","autoDoneForSession","async function safeScan(","mountUiInSurface","leadDisplayName","profileCandidates","filterRows","runPrimaryDockAction","pointerdown","pointerup","leadCreatorId","ids.has(lead)"]);
