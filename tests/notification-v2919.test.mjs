...
test("V3.2.37 loads bottom-up reader, persistent checkpoint and fixed five-panel dock",async()=>{
  ...
  assert.match(v3,/@version\s+3\.2\.37/);
  assert.match(v3,/@require\s+https:\/\/raw\.githubusercontent\.com\/mumei-s\/note-insight\/main\/public\/note-insight-notification-reader-v323\.js\?v=3237/);
  assert.match(v3,/@require\s+https:\/\/raw\.githubusercontent\.com\/mumei-s\/note-insight\/main\/public\/note-insight-notification-checkpoint-v325\.js\?v=3237/);
  assert.match(v3,/@require\s+https:\/\/raw\.githubusercontent\.com\/mumei-s\/note-insight\/main\/public\/note-insight-notification-runtime-v327\.js\?v=3237/);
  ...
  has(v3,["mumei-notification-tool-version","mumei-notification-v3-loader","bottom-up-saved-line-v3237"]);
  ...
  has(runtime,["notification-filter.html","mumei-v337-read-panel","通知を読み込む","mumei-v325-dock","data-a=\"read\"","data-a=\"mode\"","data-a=\"filter\"","data-a=\"settings\"","data-a=\"ins\"","mumei_insight_notification_auto_v325:","async function initMode(","async function toggleMode(","function maybeAuto(","async function safeScan("]);
...
test("installer stays versionless and resolves latest notification version from release manifest",async()=>{
  const setup=await read("public/tool-setup.html"),top=await read("src/insight-top-install-v16.ts"),route=await read("src/insight-notification-update-route-v1.ts"),bridge=await read("public/note-insight-bridge.user.js");has(setup,["INSIGHT インストール / 更新","INSIGHTをインストール / 更新","本人通知ツール","最新版をインストール / 更新","ブラウザ別インストール","最新版かどうか自動確認します","insight-release.json","notificationVersion","mumei-s.github.io/note-insight/note-insight-notification-v3.user.js","mumei-notification-v3-loader","mumei-notification-tool-version","ユーザースクリプトの更新を確認","URLを貼り付ける操作はありません","確認ボタンも不要"]);assert.doesNotMatch(setup,/本人通知 V\d|V3\.2\.37をインストール/);assert.doesNotMatch(setup,/script_installation\.php#url=/);assert.doesNotMatch(setup,/本人通知を実働確認|ダッシュボードを読み込む|Import from URL|https:\/\/note\.com\/notifications|mumei_insight_version_check|2\.9\.27/);has(top,["./tool-setup.html?from=top"]);has(route,["notification-update.html"]);has(bridge,["互換停止版"]);
});

test("release tracks V3.2.37 without putting the version in the user-facing label",async()=>{
  const manifest=JSON.parse(await read("public/insight-release.json")),release=await read("src/insight-release.ts"),v3=await read("public/note-insight-notification-v3.user.js");assert.equal(manifest.appVersion,"2026.09.17.2");assert.equal(manifest.notificationVersion,"3.2.37");assert.equal(manifest.notificationLabel,"本人通知");assert.equal(manifest.dashboardVersion,"1.4.4");assert.match(release,/CURRENT_INSIGHT_APP_VERSION = "2026\.09\.17\.2"/);assert.match(release,/CURRENT_NOTIFICATION_VERSION = "3\.2\.37"/);assert.match(v3,/@version\s+3\.2\.37/);
});
