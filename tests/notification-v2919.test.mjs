...
test("V3.6.7 loads split notification architecture",async()=>{
  ...
  assert.match(v3,/@version\s+3\.6\.7/);
  for(const p of ["note-insight-notification-network-v3300.js?v=3640","note-insight-notification-reader-v4.js?v=3660","note-insight-notification-controls-v1.js?v=138","note-insight-notification-filter-v4.js?v=411","note-insight-notification-return-v1.js?v=120","note-insight-notification-status-bridge-v1.js?v=110","note-insight-notification-settings-bridge-v1.js?v=110","note-insight-notification-account-pair-v1.js?v=100"])assert.ok(v3.includes(p),p);
  assert.doesNotMatch(v3,/notification-autoscan-v2970\.js\?v=|notification-bootstrap-v2966\.js\?v=|runtime-v2939-filter\.js\?v=/);
  has(reader,["function directPanel","function findPanel(){return directPanel()}","actor_image_url:img","function scheduleAuto","MAX_NOTICES=300","net.syncCurrent","automatic:true"]);
...
test("automatic notification reading is Reader-only and does not require controls",async()=>{
  ...
  const network=await read("public/note-insight-notification-network-v3300.js");assert.match(network,/async function syncHistory/);assert.match(network,/writeJournal/);assert.match(reader,/net\.syncCurrent/);assert.match(reader,/new MutationObserver/);
...
test("installer is isolated, versionless and browser-specific",async()=>{
  ...
  has(top,["./tool-setup.html?from=top","mumei-notice-controls","mumei-notice-feature-toggle","mumei-notification-feature-ui-v1","mumei-notification-feature-bridge-v1","note公式🔔のINSIGHTパネル・自動読込をOFFにする","note公式🔔のINSIGHTパネル・自動読込をONにする"]);
  assert.match(top,/grid-template-columns:minmax\(0,2.4fr\) minmax\(22px,.8fr\)/);
  has(route,["notification-update.html"]);
  has(bridge,["互換停止版"]);
});

test("release tracks V3.6.7 without putting the version in the user-facing label",async()=>{
  ...
  assert.equal(manifest.appVersion,"2026.09.22.12");assert.equal(manifest.notificationVersion,"3.6.7");assert.equal(manifest.dmVersion,"1.4.6");assert.equal(manifest.notificationLabel,"本人通知");assert.equal(manifest.dashboardVersion,"1.4.9");
  assert.match(release,/CURRENT_INSIGHT_APP_VERSION = "2026\.09\.22\.12"/);assert.match(release,/CURRENT_NOTIFICATION_VERSION = "3\.6\.7"/);assert.match(v3,/@version\s+3\.6\.7/);
});
