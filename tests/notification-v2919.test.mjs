...
test("v2.9.66 bootstrap keeps complete-row reader, safety and filter restore",async()=>{
  ...
  assert.match(boot,/@version\s+2\.9\.66/);has(boot,["runtime-v2958.js?v=2966","notification-filter-safety-v2961.js?v=2966","notification-filter-restore-v2962.js?v=2966","notification-reader-v2963.js?v=2966","notification-bootstrap-v2966.js?v=2966"]);assert.equal((boot.match(/\/\/ @require\s+/g)||[]).length,5);has(bridge,["async function openMatchingInsight()","const VERSION='2.9.66'","notificationInstalled","notificationCheckedAt"]);has(safety,["unsafeContainer","distinctNotificationDescendants","data-mumei-filter-container-restored","親コンテナ"]);has(restore,["mumei_filter_export","mumei_existing","normalizeGroups","既存フィルター","exportTo"]);
});
...
test("notification installer is popup-free and exposes every browser path",async()=>{
  const redirect=await read("public/notification-update.html"),update=await read("public/notification-update-v2966.html"),setupRedirect=await read("public/notification-setup.html"),setup=await read("public/notification-setup-v2966.html");has(redirect,["notification-update-v2966.html","v2.9.66"]);has(update,["最新版 v2.9.66","Android Edge","Android Firefox","Android Chrome / Chromium系","Samsung Internet","Yahooブラウザ / Yahooアプリ内ブラウザ","iPhone / iPad Safari","iPhone / iPad Chrome・Edge・Firefox","macOS Safari","PC Chrome / Edge / Firefox / Chromium","Edgeで開く","Firefoxで開く","Userscriptsを入れる","Tampermonkey公式","Raw直結","mumei_insight_version_check","notificationInstalled","localStorage.setItem(KEY,checked)","autoVerify","target=\"_blank\""]);assert.doesNotMatch(update,/window\.open\(/);has(setupRedirect,["notification-setup-v2966.html","v2.9.66"]);has(setup,["2.9.66","通知フィルター設定","notificationInstalled","localStorage.setItem(KEY,checked)"]);
});
