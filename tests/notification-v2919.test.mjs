...
test("V3.5.2 loads split notification architecture",async()=>{
...
test("release tracks V3.5.2 without putting the version in the user-facing label",async()=>{
  ...
  assert.equal(manifest.appVersion,"2026.09.21.14");assert.equal(manifest.notificationVersion,"3.5.2");assert.equal(manifest.dmVersion,"1.1.0");assert.equal(manifest.notificationLabel,"本人通知");assert.equal(manifest.dashboardVersion,"1.4.4");
  assert.match(release,/CURRENT_INSIGHT_APP_VERSION = "2026\.09\.21\.14"/);assert.match(release,/CURRENT_NOTIFICATION_VERSION = "3\.5\.2"/);assert.match(v3,/@version\s+3\.5\.2/);
});
