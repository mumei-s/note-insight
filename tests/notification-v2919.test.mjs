...
test("V3.6.2 loads split notification architecture",async()=>{
  ...
  assert.match(v3,/@version\s+3\.6\.2/);
  for(const p of ["note-insight-notification-network-v3300.js?v=3620","note-insight-notification-reader-v4.js?v=3620","note-insight-notification-controls-v1.js?v=136","note-insight-notification-filter-v4.js?v=401","note-insight-notification-return-v1.js?v=120","note-insight-notification-status-bridge-v1.js?v=110","note-insight-notification-settings-bridge-v1.js?v=100","note-insight-notification-account-pair-v1.js?v=100"])assert.ok(v3.includes(p),p);
...
test("release tracks V3.6.2 without putting the version in the user-facing label",async()=>{
  ...
  assert.equal(manifest.appVersion,"2026.09.22.3");assert.equal(manifest.notificationVersion,"3.6.2");assert.equal(manifest.dmVersion,"1.4.2");assert.equal(manifest.notificationLabel,"本人通知");assert.equal(manifest.dashboardVersion,"1.4.5");
  assert.match(release,/CURRENT_INSIGHT_APP_VERSION = "2026\.09\.22\.3"/);assert.match(release,/CURRENT_NOTIFICATION_VERSION = "3\.6\.2"/);assert.match(v3,/@version\s+3\.6\.2/);
});
