...
test("V3.6.8 loads split notification architecture",async()=>{
  ...
  assert.match(v3,/@version\s+3\.6\.8/);
...
test("release tracks V3.6.8 without putting the version in the user-facing label",async()=>{
  ...
  assert.equal(manifest.appVersion,"2026.09.22.12");assert.equal(manifest.notificationVersion,"3.6.8");assert.equal(manifest.dmVersion,"1.4.6");assert.equal(manifest.notificationLabel,"本人通知");assert.equal(manifest.dashboardVersion,"1.4.9");
  assert.match(release,/CURRENT_INSIGHT_APP_VERSION = "2026\.09\.22\.12"/);assert.match(release,/CURRENT_NOTIFICATION_VERSION = "3\.6\.8"/);assert.match(v3,/@version\s+3\.6\.8/);
});
