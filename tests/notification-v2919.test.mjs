...
test("notification installer now migrates to V3 and keeps file-import fallback",async()=>{
  const redirect=await read("public/notification-update.html"),update=await read("public/notification-update-v29665.html"),legacy=await read("public/notification-update-v2966.html"),setupRedirect=await read("public/notification-setup.html"),setup=await read("public/notification-setup-v2966.html");has(redirect,["notification-update-v29665.html","v3.0.0"]);has(update,["本人通知 V3","最新版 v3.0.0","note-insight-notification-v3.user.js","V3ファイルを保存","ファイルからインポート","mumei_insight_version_check","notificationInstalled","note側でV3を確認","旧2.xは削除しなくて大丈夫"]);assert.doesNotMatch(update,/window\.open\(/);has(legacy,["V3 v3.0.0","notification-update-v29665.html"]);has(setupRedirect,["notification-setup-v2966.html","V3 v3.0.0"]);has(setup,["3.0.0","通知フィルター設定","notificationInstalled","localStorage.setItem(KEY,checked)","通常のUserscript更新は不要"]);
});
