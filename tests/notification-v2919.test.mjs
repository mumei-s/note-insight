...
test("V3.2.24 loads reader, inline runtime and fallback dock",async()=>{
  const v3=await read("public/note-insight-notification-v3.user.js"),runtime=await read("public/note-insight-notification-runtime-v324.js"),dock=await read("public/note-insight-notification-dock-watch-v312.js"),reader=await read("public/note-insight-notification-reader-v323.js"),loader=await read("public/note-insight-notification-loader-v318.js");
  assert.match(v3,/@version\s+3\.2\.24/);
  assert.match(v3,/@require\s+https:\/\/raw\.githubusercontent\.com\/mumei-s\/note-insight\/main\/public\/note-insight-notification-reader-v323\.js\?v=3224/);
  assert.match(v3,/@require\s+https:\/\/raw\.githubusercontent\.com\/mumei-s\/note-insight\/main\/public\/note-insight-notification-runtime-v324\.js\?v=3224/);
  assert.match(v3,/@require\s+https:\/\/raw\.githubusercontent\.com\/mumei-s\/note-insight\/main\/public\/note-insight-notification-dock-watch-v312\.js\?v=3224/);
  has(v3,["mumei-notification-tool-version","mumei-notification-v3-loader","runtime-checked-v3224"]);
  has(runtime,["mumei-v324-dock","手動読み込み","フィルターOFF","フィルター登録","INSIGHT","通知フィルター登録","function findSurface(","async function openSettings(","showRoot(Boolean(shell))"]);
...
test("notification filter registration is inline on the note notification surface",async()=>{
  const runtime=await read("public/note-insight-notification-runtime-v324.js"),page=await read("public/notification-filter.html"),setup=await read("public/tool-setup.html");
...
test("installer remains a compact automatic-confirmation V3.2.24 update route",async()=>{
  ...
  has(setup,["INSIGHT インストール / 更新","INSIGHTをインストール / 更新","本人通知 V3.2.24","この画面で更新結果を確認","mumei-s.github.io/note-insight/note-insight-notification-v3.user.js","mumei-notification-v3-loader","mumei-notification-tool-version","ユーザースクリプトの更新を確認","URLを貼り付ける操作はありません","確認ボタンも不要"]);
...
test("release tracks V3.2.24",async()=>{
  ...
  assert.equal(manifest.appVersion,"2026.09.17.2");assert.equal(manifest.notificationVersion,"3.2.24");assert.equal(manifest.dashboardVersion,"1.4.4");
  assert.match(release,/CURRENT_INSIGHT_APP_VERSION = "2026\.09\.17\.2"/);assert.match(release,/CURRENT_NOTIFICATION_VERSION = "3\.2\.24"/);assert.match(v3,/@version\s+3\.2\.24/);
});
