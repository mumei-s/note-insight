...
test("V3.4.1 loads split notification architecture",async()=>{
  ...
  for(const p of ["note-insight-notification-reader-v4.js?v=3400","note-insight-notification-controls-v1.js?v=110","note-insight-notification-filter-v4.js?v=400","note-insight-notification-return-v1.js?v=110","note-insight-notification-status-bridge-v1.js?v=100","note-insight-notification-settings-bridge-v1.js?v=100","note-insight-notification-account-pair-v1.js?v=100"])assert.ok(v3.includes(p),p);
...
test("automatic notification reading is Reader-only and does not require controls",async()=>{
  ...
  assert.match(reader,/for\(let i=0;i<1200&&!stop;i\+\+\)/);assert.match(reader,/steps\+\+<1200/);assert.match(reader,/new MutationObserver/);
...
test("release tracks V3.4.1 without putting the version in the user-facing label",async()=>{
  ...
  assert.equal(manifest.appVersion,"2026.09.21.11");assert.equal(manifest.notificationVersion,"3.4.1");assert.equal(manifest.dmVersion,"1.1.0");assert.equal(manifest.notificationLabel,"本人通知");assert.equal(manifest.dashboardVersion,"1.4.4");
  assert.match(release,/CURRENT_INSIGHT_APP_VERSION = "2026\.09\.21\.11"/);assert.match(release,/CURRENT_NOTIFICATION_VERSION = "3\.4\.1"/);assert.match(v3,/@version\s+3\.4\.1/);
});
