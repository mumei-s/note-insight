...
test("V3.5.8 loads split notification architecture",async()=>{
...
test("release tracks V3.5.8 without putting the version in the user-facing label",async()=>{
  ...
  assert.equal(manifest.appVersion,"2026.09.21.26");assert.equal(manifest.notificationVersion,"3.5.8");assert.equal(manifest.dmVersion,"1.3.5");assert.equal(manifest.notificationLabel,"本人通知");assert.equal(manifest.dashboardVersion,"1.4.4");
  assert.match(release,/CURRENT_INSIGHT_APP_VERSION = "2026\.09\.21\.26"/);assert.match(release,/CURRENT_NOTIFICATION_VERSION = "3\.5\.8"/);assert.match(v3,/@version\s+3\.5\.8/);
});
