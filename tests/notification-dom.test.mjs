...
const panel=read('note-insight-notification-dock-watch-v312.js');
...
test('single panel ignores background rows and recognizes the opened notification shell after top-bar intent',async()=>{const{dom,w}=env();const api=expose(w,panel,['findShell','frameHtml','showDock','topClick']);try{assert.equal(api.findShell(),null);const bell=w.document.getElementById('bell'),popup=w.document.getElementById('popup');api.topClick({target:bell});popup.hidden=false;w.document.getElementById('list').innerHTML='<a class="m-navbarNoticeItem">Aさんがコメントしました3分前</a>';assert.equal(api.findShell().id,'popup');assert.match(api.frameHtml(),/repeat\(4,minmax\(0,1fr\)\)/);api.showDock(true);assert.ok(w.document.getElementById('mumei-v3-notification-frame'));popup.hidden=true;assert.equal(api.findShell(),null)}finally{dom.window.close()}});
