...
test('fixed runtime declares exactly five bottom actions including auto mode',()=>{
  ...
  assert.match(runtime,/notification-filter\.html/);assert.match(runtime,/location\.replace\(u\.href\)/);assert.doesNotMatch(runtime,/new MutationObserver/);
  assert.match(runtime,/autoMode\?'自動':'手動'/);
});

test('legacy settings UIs are retired and canonicalized',()=>{for(const src of [legacy325,legacy324]){assert.match(src,/notification-filter\.html/);assert.match(src,/location\.replace\(u\.href\)/);assert.doesNotMatch(src,/通知フィルター登録<\/h3>/)}assert.match(legacyRoute,/notification-filter\.html/);assert.match(legacyRoute,/location\.replace\(u\.href\)/);assert.doesNotMatch(legacyRoute,/new MutationObserver/)});
...
test('reader starts from the lower side, marks the completion line, and next run reads only above it',()=>{
  ...
  assert.match(reader,/findSavedRecoveryElement/);assert.match(reader,/kind!=='done'&&kind!=='error'/);assert.match(reader,/2500/);assert.match(reader,/persistRecoveredBoundary/);assert.match(reader,/完了ラインを再作成しています/);assert.doesNotMatch(reader,/if\(needBoundary\)return\{ok:false,boundaryFound:false,reason:'BOUNDARY_NOT_FOUND_SAFE_STOP'\}/);
...
test('V3.2.42 wrapper activates bottom-up reader and fixed five-panel runtime',()=>{
  const meta=v3.split('// ==/UserScript==')[0];assert.match(v3,/@version\s+3\.2\.42/);assert.match(meta,/note-insight-notification-reader-v323\.js\?v=3242/);assert.match(meta,/note-insight-notification-checkpoint-v325\.js\?v=3242/);assert.match(meta,/note-insight-notification-runtime-v327\.js\?v=3242/);assert.doesNotMatch(meta,/note-insight-notification-settings-route-v332\.js/);assert.doesNotMatch(meta,/note-insight-notification-dock-watch-v312\.js/);assert.match(v3,/bottom-up-saved-line-v3242/);assert.match(runtime,/function maybeAuto/);assert.match(runtime,/autoDoneForSession/);assert.doesNotMatch(runtime,/lastAutoAt/);assert.match(runtime,/async function safeScan/);assert.match(runtime,/event\?\.composedPath/);assert.match(runtime,/li,\[role="listitem"\]/);assert.doesNotMatch(runtime,/new MutationObserver/)
});

test('installer opens canonical userscript and keeps visible copy versionless',async()=>{
  const html=read('tool-setup.html'),dom=new JSDOM(html,{url:'https://mumei-s.github.io/note-insight/tool-setup.html',runScripts:'outside-only'}),w=dom.window;try{w.fetch=async()=>({ok:true,json:async()=>({notificationVersion:'3.2.42'})});w.eval(w.document.querySelector('script').textContent);await new Promise(resolve=>setTimeout(resolve,0));const install=w.document.getElementById('install'),u=new URL(install.href);assert.equal(u.origin,'https://raw.githubusercontent.com');assert.equal(u.pathname,'/mumei-s/note-insight/main/public/note-insight-notification-v3.user.js');assert.equal(w.document.querySelector('.toolname').textContent,'本人通知ツール');assert.match(install.textContent,/最新版をインストール \/ 更新/);assert.match(html,/insight-release\.json/);assert.doesNotMatch(html,/本人通知 V\d/);assert.match(html,/ブラウザ別インストール/);assert.match(html,/5パネル/);assert.match(html,/次回からはその保存位置を境界/);assert.match(html,/URLを貼り付ける操作はありません/);assert.match(html,/確認ボタンも不要/)}finally{dom.window.close()}
});