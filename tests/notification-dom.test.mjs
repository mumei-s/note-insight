...
test('installer is isolated behind a redirect shell and stays versionless',async()=>{
  ...
  const w=dom.window;
  try{
    w.fetch=async()=>({ok:true,json:async()=>({notificationVersion:'3.2.66'})});
    ...
    assert.doesNotMatch(html,/本人通知 V\d|V3\.2\.66/);
...
test("body-fixed notification dock cannot be reparented into note notification DOM",async()=>{
 ...
 assert.match(runtime,/const host=document\.body\|\|document\.documentElement/);
