...
test('V3.6.10 wrapper activates only split modules',()=>{
  const meta=v3.split('// ==/UserScript==')[0];
  assert.match(v3,/@version\s+3\.6\.10/);
  for(const part of [
    'note-insight-notification-network-v3300.js?v=3660','note-insight-notification-reader-v4.js?v=3660','note-insight-notification-controls-v1.js?v=138',
...
test('installer is isolated behind a redirect shell and stays versionless',async()=>{
  ...
  const w=dom.window;
  try{
    w.fetch=async()=>({ok:true,json:async()=>({notificationVersion:'3.6.10'})});
...
test('network Reader is primary while legacy DOM parser remains isolated',async()=>{
  ...
  assert.ok(meta.includes('note-insight-notification-network-v3300.js?v=3660'));assert.ok(meta.includes('note-insight-notification-reader-v4.js?v=3660'));
