...
test('clean panel ignores background rows and recognizes only the opened notification shell',async()=>{const{dom,w}=env();const api=expose(w,panel,['strictShell','currentShell','frameHtml','show']);try{assert.equal(api.strictShell(),null);const popup=w.document.getElementById('popup');popup.hidden=false;w.document.getElementById('list').innerHTML='<a class="m-navbarNoticeItem">Aさんがコメントしました3分前</a>';assert.equal(api.strictShell().id,'popup');assert.equal(api.currentShell().id,'popup');assert.match(api.frameHtml(),/repeat\(4,minmax\(0,1fr\)\)/);assert.ok(w.document.getElementById('mumei-v3-panel-clean-v1'));popup.hidden=true;assert.equal(api.strictShell(),null)}finally{dom.window.close()}});
