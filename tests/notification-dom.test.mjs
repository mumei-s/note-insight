...
test('V3.6.8 wrapper activates only split modules',()=>{
  const meta=v3.split('// ==/UserScript==')[0];
  assert.match(v3,/@version\s+3\.6\.8/);
  for(const part of [
    'note-insight-notification-network-v3300.js?v=3650','note-insight-notification-reader-v4.js?v=3660','note-insight-notification-controls-v1.js?v=138',
    'note-insight-notification-filter-v4.js?v=411','note-insight-notification-return-v1.js?v=120',
    'note-insight-notification-status-bridge-v1.js?v=110','note-insight-notification-feature-bridge-v1.js?v=110',
    'note-insight-notification-settings-bridge-v1.js?v=110','note-insight-notification-account-pair-v1.js?v=100'
  ...
  assert.match(splitReader,/function scheduleAuto/);assert.doesNotMatch(splitReader,/autoReadVisibleWindow/);assert.match(splitReader,/async function scan/);
...
test('installer is isolated behind a redirect shell and stays versionless',async()=>{
  ...
  const w=dom.window;
  try{
    w.fetch=async()=>({ok:true,json:async()=>({notificationVersion:'3.6.8'})});
...
test('manual full-read is not preloaded; split Reader owns automatic full and delta scans',async()=>{
  ...
  assert.doesNotMatch(splitReader,/scrollTop\s*=/);
  assert.match(splitReader,/automatic:true/);
});
...
test('inline controls are independently notification-surface-owned',()=>{
  assert.match(controls,/document\.body\.appendChild\(bar\)/);assert.match(controls,/window\.addEventListener\(ev,interceptToolbar,\{capture:true,passive:ev!==\x27click\x27\}\)/);assert.match(controls,/data-action="read"/);
...
test('network Reader is primary while legacy DOM parser remains isolated',async()=>{
  ...
  assert.ok(meta.includes('note-insight-notification-network-v3300.js?v=3650'));assert.ok(meta.includes('note-insight-notification-reader-v4.js?v=3660'));
  assert.doesNotMatch(meta,/note-insight-notification-autoscan-v2970\.js\?v=/);
  assert.match(network,/async function syncHistory/);assert.match(network,/writeJournal/);
  assert.match(splitReader,/function rowData/);assert.match(splitReader,/actor_image_url:img/);
  assert.match(splitReader,/function scheduleAuto/);assert.match(splitReader,/net\.syncCurrent/);assert.doesNotMatch(splitReader,/async function scanDomLegacy/);
});
