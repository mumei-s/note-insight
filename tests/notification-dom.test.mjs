...
test('V3.6.5 wrapper activates only split modules',()=>{
  const meta=v3.split('// ==/UserScript==')[0];
  assert.match(v3,/@version\s+3\.6\.5/);
  for(const part of [
    'note-insight-notification-network-v3300.js?v=3630','note-insight-notification-reader-v4.js?v=3640','note-insight-notification-controls-v1.js?v=137',
    'note-insight-notification-filter-v4.js?v=410','note-insight-notification-return-v1.js?v=120',
    'note-insight-notification-status-bridge-v1.js?v=110','note-insight-notification-feature-bridge-v1.js?v=100',
...
test('installer is isolated behind a redirect shell and stays versionless',async()=>{
  ...
  const w=dom.window;
  try{
    w.fetch=async()=>({ok:true,json:async()=>({notificationVersion:'3.6.5'})});
...
test('network Reader is primary while legacy DOM parser remains isolated',async()=>{
  ...
  assert.ok(meta.includes('note-insight-notification-network-v3300.js?v=3630'));assert.ok(meta.includes('note-insight-notification-reader-v4.js?v=3640'));
