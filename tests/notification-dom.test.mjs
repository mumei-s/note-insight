...
test('installer is isolated behind a redirect shell and stays versionless',async()=>{
  ...
  const w=dom.window;
  try{
    w.fetch=async()=>({ok:true,json:async()=>({notificationVersion:'3.3.2'})});
    ...
    assert.doesNotMatch(html,/本人通知 V\d|V3\.3\.2/);
...
test('manual full-read stays isolated from the 05:48 runtime core',async()=>{
  ...
  assert.match(v3,/note-insight-notification-manual-full-v3284\.js\?v=3310/);
...
test('V3.3.2 dock is viewport-owned, never notification-surface-owned',()=>{
