...
test("installer entry redirects to isolated compact browser page",async()=>{
  ...
  assert.doesNotMatch(page,/script_installation\.php#url=|本人通知 V\d|V3\.2\.91/);
});

test("V3.2.91 preloads bottom-up reader, checkpoint and fixed five-panel runtime",async()=>{
  ...
  assert.match(v3,/@version\s+3\.2\.91/);assert.match(v3,/bottom-up-saved-line-v3245/);
  const meta=v3.split("// ==/UserScript==")[0];
  assert.match(meta,/note-insight-notification-reader-v323\.js\?v=3258/);assert.match(meta,/note-insight-notification-checkpoint-v325\.js\?v=3250/);assert.match(meta,/note-insight-notification-runtime-v327\.js\?v=3291/);assert.doesNotMatch(meta,/note-insight-notification-settings-route-v332\.js/);
