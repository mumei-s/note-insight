...
test("installer keeps participant steps compact with automatic confirmation",async()=>{
  ...
  assert.doesNotMatch(page,/本人通知 V\d|V3\.2\.38/);assert.match(page,/mumei-notification-v3-loader/);assert.match(page,/mumei-notification-tool-version/);assert.match(page,/確認ボタンも不要/);
});

test("V3.2.38 preloads bottom-up reader, checkpoint and fixed five-panel runtime",async()=>{
  ...
  assert.match(v3,/@version\s+3\.2\.38/);assert.match(v3,/bottom-up-saved-line-v3238/);
  const meta=v3.split("// ==/UserScript==")[0];
  assert.match(meta,/note-insight-notification-reader-v323\.js\?v=3238/);assert.match(meta,/note-insight-notification-checkpoint-v325\.js\?v=3238/);assert.match(meta,/note-insight-notification-runtime-v327\.js\?v=3238/);assert.doesNotMatch(meta,/note-insight-notification-settings-route-v332\.js/);
