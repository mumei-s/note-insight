...
test("installer entry redirects to isolated compact browser page",async()=>{
  ...
  assert.doesNotMatch(page,/script_installation\.php#url=|本人通知 V\d|V3\.5\.9/);
});

test("V3.5.9 preloads split runtime modules",async()=>{
  ...
  assert.match(network,/async function syncHistory/);assert.match(network,/syncDirectFullAscending/);assert.match(network,/MAX_NOTICES=300,MAX_PAGES=3/);assert.match(network,/direction:'bottom-up'/);assert.match(network,/needsDom:false/);assert.match(reader,/function directPanel/);assert.match(reader,/function scheduleAuto/);assert.match(reader,/autoReadVisibleWindow/);assert.match(reader,/fastVisibleSync/);assert.match(reader,/net\.syncCurrent/);assert.match(reader,/actor_image_url:img/);
...
test("automatic reading is owned only by Reader",async()=>{
  ...
  assert.match(reader,/scheduleAuto\(80\)/);assert.match(reader,/document\.addEventListener\('scroll'/);assert.match(reader,/MAX_NOTICES=300/);
...
test("V3.5.9 supports checkpoint stop without losing partial progress",async()=>{
