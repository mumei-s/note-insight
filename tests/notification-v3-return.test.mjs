...
test("installer keeps participant steps compact with automatic confirmation",async()=>{
  ...
  assert.doesNotMatch(page,/本人通知 V\d|V3\.2\.48/);assert.match(page,/mumei-notification-v3-loader/);assert.match(page,/mumei-notification-tool-version/);assert.match(page,/確認ボタンも不要/);
});

test("V3.2.48 preloads bottom-up reader, checkpoint and fixed five-panel runtime",async()=>{
  ...
  assert.match(meta,/note-insight-notification-reader-v323\.js\?v=3245/);assert.match(meta,/note-insight-notification-checkpoint-v325\.js\?v=3245/);assert.match(meta,/note-insight-notification-runtime-v327\.js\?v=3248/);assert.doesNotMatch(meta,/note-insight-notification-settings-route-v332\.js/);
