...
test("installer entry redirects to isolated compact browser page",async()=>{
  ...
  assert.doesNotMatch(page,/script_installation\.php#url=|本人通知 V\d|V3\.4\.3/);
});

test("V3.4.3 preloads split runtime modules",async()=>{
  ...
  assert.match(network,/async function syncHistory/);assert.match(network,/needsDom:false/);assert.match(reader,/function directPanel/);assert.match(reader,/function scheduleAuto/);assert.match(reader,/net\.syncCurrent/);assert.match(reader,/actor_image_url:img/);
...
test("notification Reader stays automatic while Controls are a separate module",async()=>{
  ...
  assert.match(controls,/mumei-inline-notification-controls-v1/);assert.match(controls,/panel\.appendChild\(bar\)/);assert.match(controls,/position:fixed/);
  assert.match(controls,/INSIGHT【通知】/);assert.match(controls,/フィルター ON|フィルター OFF/);
});
