...
test('フォローから記事・スキ順位へ移動し、戻る・進むでそのカテゴリと表示を復元する',async t=>{
 ...
 const cache=new Map();async function load(file){if(cache.has(file))return cache.get(file);const source=ts.transpileModule(readFileSync(file,'utf8'),{compilerOptions:{module:ts.ModuleKind.ESNext,target:ts.ScriptTarget.ES2022,jsx:ts.JsxEmit.ReactJSX}}).outputText;const m=new vm.SourceTextModule(source,{context:ctx,initializeImportMeta:meta=>meta.env={BASE_URL:'/note-insight/'}});cache.set(file,m);await m.link(async name=>{if(name==='./member-insight-unified-v4')return load('src/member-insight-unified-v4.tsx');const exports=data[name]||(name.endsWith('.css')||name==='./insight-ux-v12'?{}:null);if(!exports)throw new Error('Missing fixture '+name);return new vm.SyntheticModule(Object.keys(exports),function(){for(const[k,v]of Object.entries(exports))this.setExport(k,v)},{context:ctx})});return m}
