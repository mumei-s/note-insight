...
function aboveComposer(h){const r=h.panel.getBoundingClientRect(),composer=h.w.document.getElementById('composer').getBoundingClientRect(),send=h.w.document.getElementById('send').getBoundingClientRect();assert.ok(r.bottom<=composer.top-8);assert.ok(r.bottom<send.top);assert.equal(h.panel.hidden,false);assert.equal(h.panel.style.display,'flex')}

test('DMの入力欄と送信ボタンの上へ薄型パネルを配置する',t=>{const h=fixture(t);h.show();aboveComposer(h);assert.match(h.panel.textContent,/本文 40・保存 20/);assert.equal(h.panel.querySelector('button').style.minHeight,'32px')});
for(const contenteditable of [false,true,'plaintext-only'])test((contenteditable==='plaintext-only'?'plaintext-only':contenteditable?'contenteditable':'textarea')+'入力中は即座に隠れ、遅い保存応答でも復活せず下書きを保持する',async t=>{
 ...
 h.editor.blur();await h.flushLayout();aboveComposer(h);assert.match(h.panel.textContent,/保存 25/);
});
test('キーボード・拡大表示中は隠し、閉じた後に安全な位置へ戻す',async t=>{
 ...
 h.vv.height=800;h.vv.scale=1.5;h.vv.dispatchEvent(new h.w.Event('resize'));await h.flushLayout();assert.equal(h.panel.hidden,true);h.vv.scale=1;h.vv.dispatchEvent(new h.w.Event('resize'));await h.flushLayout();aboveComposer(h);
});
test('入力欄の高さ変更と横向き画面でもパネルが入力欄へ重ならない',async t=>{
 const h=fixture(t);h.show();h.geometry({composerTop:590,editorTop:604,editorHeight:130});h.resize();await h.flushLayout();aboveComposer(h);
 h.geometry({layoutHeight:400,composerTop:280,editorTop:294,editorHeight:56});h.vv.height=400;h.w.dispatchEvent(new h.w.Event('orientationchange'));await h.flushLayout();aboveComposer(h);
});
test('入力欄を検出できない時や上に場所がない時は表示を控える',async t=>{
 const h=fixture(t);h.show();h.editor.remove();await h.flushLayout();assert.equal(h.panel.hidden,true);h.w.document.getElementById('composer').insertAdjacentHTML('afterbegin','<textarea id="editor"></textarea>');h.geometry({composerTop:25,editorTop:35});await h.flushLayout();assert.equal(h.panel.hidden,true);
 h.geometry({composerTop:680,editorTop:694});h.resize();await h.flushLayout();aboveComposer(h);
});
test('VisualViewportがないブラウザでも入力中は隠れ、復帰後は入力欄の上に置く',async t=>{const h=fixture(t,{visualViewport:false});h.show();aboveComposer(h);h.editor.focus();assert.equal(h.panel.hidden,true);h.editor.blur();await h.flushLayout();aboveComposer(h)});
test('DM一覧では操作ボタンを残し、通知画面では遅い進捗でも復活させない',async t=>{
 const h=fixture(t,{list:true});h.w.document.querySelector('form').remove();h.show();assert.equal(h.panel.hidden,false);let runs=0;h.w.__mumeiInsightDmReaderV1Api.run=()=>{runs++;return true};h.panel.querySelector('button').click();assert.equal(runs,1);
