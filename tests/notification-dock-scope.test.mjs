...
test("V3.2.62 loads bottom-up reader and fixed five-panel runtime",async()=>{
 ...
 const reader=await read("public/note-insight-notification-reader-v323.js");
 assert.match(parent,/@version\s+3\.2\.62/);
 ...
 assert.match(parent,/note-insight-notification-runtime-v327\.js\?v=3262/);
...
test("tapping a notification row does not close the detached dock",async()=>{
 const runtime=await read("public/note-insight-notification-runtime-v327.js");
 assert.match(runtime,/function notificationLeaveAction\(target\)[\s\S]*return leavingNotificationHref\(target\)/);
 assert.doesNotMatch(runtime,/const row=target\.closest\(ITEM\+'\,li\,\[role="listitem"\]'\)/);
});


test("dock is locked visible for the full reader scan cycle",async()=>{
 const runtime=await read("public/note-insight-notification-runtime-v327.js");
 assert.match(runtime,/function readerScanning\(\)/);
 assert.match(runtime,/function keepDockDuringScan\(\)/);
 assert.match(runtime,/if\(!force&&keepDockDuringScan\(\)\)return false/);
 assert.match(runtime,/mumei-v3-reader-stopped/);
 assert.match(runtime,/d\.scanning\).*keepDockDuringScan/);
});
