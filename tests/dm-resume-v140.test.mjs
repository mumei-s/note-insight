...
function harness(){
 const gm=new Map(),local=new Map(),events=new EventTarget(),doc=new EventTarget(),visited=[];let fail='r3',now=Date.now();class Clock extends Date{static now(){return now}}
 doc.documentElement={};
 ...
 const context={URL,Event,Map,Set,Date:Clock,Math,String,Number,Boolean,JSON,Promise,location:{hostname:'note.com',pathname:'/messages/rooms',href:'https://note.com/messages/rooms',search:''},document:doc,GM:{getValue:async(k,d)=>gm.get(k)??d,setValue:async(k,v)=>gm.set(k,structuredClone(v))},localStorage:{getItem:k=>local.get(k)||null,setItem:(k,v)=>local.set(k,v)},sessionStorage:{removeItem(){}},MutationObserver:class{observe(){}},setTimeout:()=>1,setInterval:()=>1,clearInterval(){},clearTimeout(){},addEventListener:events.addEventListener.bind(events),fixtureFrame:async key=>{visited.push(key);return key===fail?{read:0,saved:0,error:'fixture-failure'}:{read:2,saved:2,complete:true}}};context.window=context;vm.createContext(context);vm.runInContext(injectable,context);
 return{api:context.__mumeiInsightDmReaderV1Api,gm,visited,network(value){context.__mumeiDmNetworkV2=value},recover(){fail='';now+=16000},status(){return gm.get('mumei_insight_dm_checkpoint_v1:tester')}};
}
test('DMの直接取得が可能ならHTMLやフレームを再読込しない',async()=>{const h=harness(),calls=[];h.network({readHistory:async key=>{calls.push(key);return{read:1,saved:1,complete:true}},readRoom:async()=>{throw new Error('unnecessary HTML fetch')}});await h.api.syncRoomsInBackground({id:'tester'},[{key:'r1',url:'https://note.com/messages/rooms/r1'}]);assert.deepEqual(calls,['r1']);assert.deepEqual(h.visited,[]);assert.equal(h.status().lastRunComplete,true);});
test('DMの1部屋でAPIエラーが起きても残りの部屋を取得する',async()=>{const h=harness(),calls=[];h.network({readHistory:async key=>{calls.push(key);if(key==='r2')throw new Error('DM_API_RETRY');return{read:1,saved:1,complete:true}}});await h.api.syncRoomsInBackground({id:'tester'},['r1','r2','r3'].map(key=>({key,url:'https://note.com/messages/rooms/'+key})));assert.deepEqual(calls,['r1','r2','r3']);assert.equal(h.status().currentThread,2);assert.equal(h.status().lastRunComplete,false);assert.match(h.status().lastError,/r2: DM_API_RETRY/);});
