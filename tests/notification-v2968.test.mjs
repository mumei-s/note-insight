...
test('active package uses V3.5.6 split full/delta Reader',()=>{
  ...
  assert.equal(manifest.notificationVersion,'3.5.6');assert.equal(manifest.notificationLabel,'本人通知');
...
test('notification and DM readers are hard separated with independent storage and APIs',()=>{
  ...
  assert.match(dmUser,/@version\s+1\.3\.2/);assert.match(dmUser,/note-insight-dm-account-pair-v1\.js\?v=100/);assert.match(dmUser,/note-insight-dm-network-v2\.js\?v=121/);assert.match(dmUser,/note-insight-dm-reader-v1\.js\?v=132/);
  ...
  assert.match(dmUi,/PRIVATE DIRECT MESSAGES/);assert.match(dmUi,/fetchInsightRelease/);assert.match(dmUi,/dmUpdateAvailable/);assert.match(dmUi,/DM同期 v\{latestDmVersion\}へ更新/);assert.match(dmUi,/通常のnote DM画面はそのまま/);assert.match(dmUi,/midm-history-panel/);assert.match(dmUi,/feed\("people"\)/);assert.match(dmUi,/person_messages/);assert.match(dmUi,/DM同期ツールをインストール/);assert.doesNotMatch(dm,/location\.replace|cloak\(true\)/);assert.match(dm,/syncRoomsInBackground/);assert.match(dm,/mumei_dm_bg/);assert.match(dm,/previewMessage/);assert.match(dm,/background-hidden/);assert.match(dm,/clearLegacyQueue/);assert.match(dm,/ROOM_ID_RE/);assert.doesNotMatch(dm,/threadKey===\'new\'/);
});
