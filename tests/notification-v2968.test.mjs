...
test('active package uses V3.6.1 split full/delta Reader',()=>{
  ...
  assert.equal(manifest.notificationVersion,'3.6.1');assert.equal(manifest.notificationLabel,'本人通知');
...
test('notification and DM readers are hard separated with independent storage and APIs',()=>{
  ...
  assert.match(dmUser,/@version\s+1\.4\.1/);assert.match(dmUser,/note-insight-dm-account-pair-v1\.js\?v=100/);assert.match(dmUser,/note-insight-dm-network-v2\.js\?v=141/);assert.match(dmUser,/note-insight-dm-reader-v1\.js\?v=141/);
