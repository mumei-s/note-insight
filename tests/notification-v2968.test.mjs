...
test('active package uses V3.2.89 with incremental resume, persistent checkpoint and fixed five-panel auto dock',()=>{
  ...
  assert.equal(manifest.notificationVersion,'3.2.89');assert.equal(manifest.notificationLabel,'本人通知');
  assert.match(v3,/@version\s+3\.2\.89/);assert.match(v3,/bottom-up-saved-line-v3245/);
  const meta=v3.split('// ==/UserScript==')[0];
  for(const part of ['note-insight-notification-reader-v323.js?v=3259','note-insight-notification-checkpoint-v325.js?v=3250','note-insight-notification-runtime-v327.js?v=3289'])assert.match(meta,new RegExp(part.replace(/[.?]/g,m=>'\\'+m)));
  assert.doesNotMatch(meta,/note-insight-notification-dock-watch-v312\.js/);
  assert.match(checkpoint,/mumei_insight_notification_checkpoint_local_v325:/);assert.match(checkpoint,/localStorage\.setItem/);assert.doesNotMatch(checkpoint,/new MutationObserver/);assert.match(reader,/checkpointFor/);assert.match(reader,/preserveBoundary/);assert.match(reader,/通常読込では過去へ潜りません/);
  ...
  assert.match(reader,/scan_mode:'incremental-top-until-saved'/);assert.match(reader,/top-until-first-saved-v329/);assert.match(reader,/ここまで保存済み/);assert.match(reader,/boundarySignature/);assert.match(reader,/新しい通知だけ確認しています/);assert.match(reader,/findSavedRecoveryElement/);assert.match(reader,/preserveBoundary/);assert.match(reader,/通常読込では過去へ潜りません/);assert.doesNotMatch(reader,/完了ラインを再作成しています/);
