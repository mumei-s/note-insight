...
test('active package uses V3.2.41 with bottom-up resume, persistent checkpoint and fixed five-panel auto dock',()=>{
  ...
  assert.equal(manifest.notificationVersion,'3.2.41');assert.equal(manifest.notificationLabel,'本人通知');
  assert.match(v3,/@version\s+3\.2\.41/);assert.match(v3,/bottom-up-saved-line-v3241/);
  const meta=v3.split('// ==/UserScript==')[0];
  for(const part of ['note-insight-notification-reader-v323.js?v=3241','note-insight-notification-checkpoint-v325.js?v=3241','note-insight-notification-runtime-v327.js?v=3241'])assert.match(meta,new RegExp(part.replace(/[.?]/g,m=>'\\'+m)));
  ...
  assert.match(runtime,/mumei_insight_notification_auto_v325:/);assert.match(runtime,/event\?\.composedPath/);assert.doesNotMatch(runtime,/new MutationObserver/);assert.match(runtime,/notification-filter\.html/);assert.doesNotMatch(runtime,/new MutationObserver/);assert.match(runtime,/maybeAuto/);assert.match(runtime,/autoDoneForSession/);assert.doesNotMatch(runtime,/lastAutoAt/);assert.match(runtime,/safeScan/);
  assert.match(reader,/scan_mode:'bottom-up-from-saved-line'/);assert.match(reader,/saved-line-bottom-up-v324/);assert.match(reader,/ここまで保存済み/);assert.match(reader,/boundarySignature/);assert.match(reader,/完了ラインから上方向へ、追加分だけ読み込みます/);assert.match(reader,/findSavedRecoveryElement/);assert.match(reader,/完了ラインを再作成しています/);
