...
test('active package uses V3.2.45 with bottom-up resume, persistent checkpoint and fixed five-panel auto dock',()=>{
  ...
  assert.equal(manifest.notificationVersion,'3.2.45');assert.equal(manifest.notificationLabel,'本人通知');
  assert.match(v3,/@version\s+3\.2\.45/);assert.match(v3,/bottom-up-saved-line-v3245/);
  const meta=v3.split('// ==/UserScript==')[0];
  for(const part of ['note-insight-notification-reader-v323.js?v=3245','note-insight-notification-checkpoint-v325.js?v=3245','note-insight-notification-runtime-v327.js?v=3245'])assert.match(meta,new RegExp(part.replace(/[.?]/g,m=>'\\'+m)));
  ...
  assert.match(runtime,/runPrimaryDockAction/);assert.match(runtime,/pointerdown/);assert.match(runtime,/pointerup/);assert.match(runtime,/leadCreatorId/);assert.match(runtime,/ids\.has\(lead\)/);assert.match(runtime,/mumei_insight_notification_auto_v325:/);assert.match(runtime,/event\?\.composedPath/);assert.doesNotMatch(runtime,/new MutationObserver/);assert.match(runtime,/通知フィルター登録/);assert.match(runtime,/data-addg/);assert.match(runtime,/note URL \/ @ID \/ ID/);assert.doesNotMatch(runtime,/notification-filter\.html|location\.replace\(u\.href\)/);assert.doesNotMatch(runtime,/new MutationObserver/);assert.match(runtime,/maybeAuto/);assert.match(runtime,/autoDoneForSession/);assert.doesNotMatch(runtime,/lastAutoAt/);assert.match(runtime,/safeScan/);
