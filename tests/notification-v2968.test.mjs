...
test('active package uses V3.2.31 with persistent checkpoint and fixed five-panel auto dock',()=>{
  ...
  assert.equal(manifest.notificationVersion,'3.2.31');assert.equal(manifest.notificationLabel,'本人通知');
  assert.match(v3,/@version\s+3\.2\.31/);assert.match(v3,/fixed-five-dock-v3231/);
  const meta=v3.split('// ==/UserScript==')[0];
  for(const part of ['note-insight-notification-reader-v323.js?v=3231','note-insight-notification-checkpoint-v325.js?v=3231','note-insight-notification-runtime-v327.js?v=3231'])assert.match(meta,new RegExp(part.replace(/[.?]/g,m=>'\\'+m)));
  ...
  assert.match(runtime,/mumei_insight_notification_auto_v325:/);assert.match(runtime,/maybeAuto/);assert.match(runtime,/safeScan/);
  assert.match(reader,/scan_mode:'incremental-top-to-checkpoint'/);assert.match(reader,/ここまで保存済み/);assert.match(reader,/boundarySignature/);
