...
test('active package uses V3.5.3 split full/delta Reader',()=>{
  ...
  assert.equal(manifest.notificationVersion,'3.5.3');assert.equal(manifest.notificationLabel,'本人通知');
...
test('notification and DM readers are hard separated with independent storage and APIs',()=>{
  ...
  assert.match(dmUser,/@version\s+1\.3\.0/);assert.match(dmUser,/note-insight-dm-account-pair-v1\.js\?v=100/);assert.match(dmUser,/note-insight-dm-network-v2\.js\?v=120/);assert.match(dmUser,/note-insight-dm-reader-v1\.js\?v=130/);
