...
test('active package uses V3.2.70 with bottom-up resume, persistent checkpoint and fixed five-panel auto dock',()=>{
  ...
  assert.equal(manifest.notificationVersion,'3.2.70');assert.equal(manifest.notificationLabel,'本人通知');
  assert.match(v3,/@version\s+3\.2\.70/);assert.match(v3,/bottom-up-saved-line-v3245/);
  const meta=v3.split('// ==/UserScript==')[0];
  for(const part of ['note-insight-notification-reader-v323.js?v=3245','note-insight-notification-checkpoint-v325.js?v=3250','note-insight-notification-runtime-v327.js?v=3270'])assert.match(meta,new RegExp(part.replace(/[.?]/g,m=>'\\'+m)));
