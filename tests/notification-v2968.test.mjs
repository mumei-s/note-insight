...
test('active package uses V3.5.8 split full/delta Reader',()=>{
  ...
  assert.equal(manifest.notificationVersion,'3.5.8');assert.equal(manifest.notificationLabel,'本人通知');
...
test('notification and DM readers are hard separated with independent storage and APIs',()=>{
  ...
  assert.match(dm,/for\(let i=0;i<list\.length;i\+=2\)/);assert.doesNotMatch(dm,/i\+=3/);assert.match(dmUser,/@version\s+1\.3\.5/);assert.match(dmUser,/note-insight-dm-reader-v1\.js\?v=135/);assert.match(unified,/onTabChange/);assert.match(live,/onTabChange=\{handleUnifiedTab\}/);
});
