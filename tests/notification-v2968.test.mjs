...
test('active package uses V3.3.0 with bottom-up resume, persistent checkpoint and fixed five-panel auto dock',()=>{
  ...
  assert.equal(manifest.notificationVersion,'3.3.0');assert.equal(manifest.notificationLabel,'本人通知');
  assert.match(v3,/@version\s+3\.3\.0/);assert.match(v3,/bottom-up-saved-line-v3245/);
  const meta=v3.split('// ==/UserScript==')[0];
  for(const part of ['note-insight-notification-network-v3300.js?v=3300','note-insight-notification-reader-v323.js?v=3258','note-insight-notification-checkpoint-v325.js?v=3293','note-insight-notification-runtime-v327.js?v=3300','note-insight-notification-manual-full-v3284.js?v=3300'])assert.match(meta,new RegExp(part.replace(/[.?]/g,m=>'\\'+m)));
